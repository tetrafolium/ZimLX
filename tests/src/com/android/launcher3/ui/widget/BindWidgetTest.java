...
public void setUp() throws Exception {
	...
	LauncherSettings.Settings.call(
		mResolver, LauncherSettings.Settings.METHOD_CREATE_EMPTY_DB);
	LauncherSettings.Settings.call(
		mResolver, LauncherSettings.Settings.METHOD_CLEAR_EMPTY_DB_FLAG);
}
...
public void tearDown() {
	...
	if (mSessionId > -1) {
		mTargetContext.getPackageManager().getPackageInstaller().abandonSession(
			mSessionId);
	}
}
...
public void testUnboundWidget_removed() throws Exception {
	...
	// Since there is no widget to verify, just wait until the workspace is
	// ready.
	...
	assertFalse(
		mDevice.findObject(new UiSelector().description(info.label)).exists());
}
...
public void testPendingWidget_withConfigScreen() throws Exception {
	// A non-restored widget with config screen get bound and shows a 'Click to
	// setup' UI.
	LauncherAppWidgetProviderInfo info = findWidgetProvider(true);
	...
	assertEquals(0, mCursor.getInt(mCursor.getColumnIndex(
					       LauncherSettings.Favorites.RESTORED)) &
	             LauncherAppWidgetInfo.FLAG_ID_NOT_VALID);
	assertNotNull(mWidgetManager.getAppWidgetInfo(mCursor.getInt(
...
public void testPendingWidget_notRestored_removed() throws Exception {
	...
	// The view does not exist
	assertFalse(mDevice
	            .findObject(new UiSelector().className(
					PendingAppWidgetHostView.class))
	            .exists());
	waitUntilLoaderIdle();
	// Item deleted from db
	mCursor = mResolver.query(LauncherSettings.Favorites.getContentUri(item.id),
...
public void testPendingWidget_notRestored_activeInstall() throws Exception {
	...
	PackageInstaller installer =
		mTargetContext.getPackageManager().getPackageInstaller();
...
/**
 * Adds {@param item} on the homescreen on the 0th screen at 0,0, and verifies
 * that the widget class is displayed on the homescreen.
 *
 ...
 */
private void setupAndVerifyContents(final LauncherAppWidgetInfo item,
	...
	item.id = LauncherSettings.Settings
	          .call(mResolver, LauncherSettings.Settings.METHOD_NEW_ITEM_ID)
	          .getLong(LauncherSettings.Settings.EXTRA_VALUE);
	item.screenId = screenId;
	item.onAddToDatabase(writer);
	writer.put(LauncherSettings.Favorites._ID, item.id);
	mResolver.insert(LauncherSettings.Favorites.CONTENT_URI,
	...
	UiSelector selector = new UiSelector()
	                      .packageName(mTargetContext.getPackageName())
 ...
 *
 * @param bindWidget if true the info is bound and a valid widgetId is
 *     assigned to
 *                   the LauncherAppWidgetInfo
 */
private LauncherAppWidgetInfo
createWidgetInfo(final LauncherAppWidgetProviderInfo info,
                 final boolean bindWidget) {
	LauncherAppWidgetInfo item =
		new LauncherAppWidgetInfo(LauncherAppWidgetInfo.NO_ID, info.provider);
...
/**
 * Returns a LauncherAppWidgetInfo with package name which is not present on
 * the device
 */
private LauncherAppWidgetInfo getInvalidWidgetInfo() {
	...
	LauncherAppWidgetInfo item = new LauncherAppWidgetInfo(
		10, new ComponentName(pkg, "com.test.widgetprovider"));
	item.spanX = 2;
	item.spanY = 2;
	item.minSpanX = 2;
	item.minSpanY = 2;
...
/**
 * Blocks the current thread until all the jobs in the main worker thread are
 * complete.
 */
