...
public void setup() {
	...
	mAdapter = new WidgetsListAdapter(mContext, mMockLayoutInflater,
	                                  mMockWidgetCache, mIconCache, null, null);
...
/**
 * Helper method to generate the sample widget model map that can be used for
 * the tests
...
private ArrayList<WidgetListRowEntry> generateSampleMap(final int num) {
	...
	for (AppWidgetProviderInfo widgetInfo :
	     widgetManager.getAllProviders(null)) {
		WidgetItem wi = new WidgetItem(
			LauncherAppWidgetProviderInfo.fromProviderInfo(mContext, widgetInfo),
			pm, mTestProfile);

		PackageItemInfo pInfo =
			new PackageItemInfo(wi.componentName.getPackageName());
