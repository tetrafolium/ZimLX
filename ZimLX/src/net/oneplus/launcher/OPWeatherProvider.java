...
public class OPWeatherProvider {
  ...
  public static final String WEATHER_LAUNCH_ACTIVITY =
  ...
  private final Uri WEATHER_CONTENT_URI =
      Uri.parse("content://com.oneplus.weather.ContentProvider/data");
  private ArrayList<IWeatherCallback> mCallbacks;
  private Context mContext;
  private WeatherObserver mObserver;

  private Handler mUiWorkerHandler =
  ...
  private enum WEATHER_COLUMNS {
    ...
    WEATHER_COLUMNS(final int i) { this.index = i; }
  }
  ...
  private enum WEATHER_TYPE {
    ...
    WEATHER_TYPE(final @IntRange(from = 1000, to = 9999) int i,
                 final WeatherIconManager.Icon icon) {
      weatherCode = i;
      this.icon = icon;
    }
    ...
    public String toString() { return String.valueOf(this.weatherCode); }
  }
  ...
  private class WeatherObserver extends ContentObserver {
    ...
    public WeatherObserver() { super(new Handler()); }
  ...
  public WeatherData getOfflineWeatherInformation() {
    ...
    if (defaultSharedPreferences.contains(KEY_TIMESTAMP)) {
      ...
      weatherData.timestamp =
          defaultSharedPreferences.getLong(KEY_TIMESTAMP, 0);
      weatherData.cityName =
          defaultSharedPreferences.getString(KEY_CITY_NAME, "");
      weatherData.weatherCode =
          defaultSharedPreferences.getInt(KEY_WEATHER_CODE, 9999);
      weatherData.weatherName = defaultSharedPreferences.getString(
          KEY_WEATHER_NAME, WEATHER_NAME_NONE);
      weatherData.temperature =
          defaultSharedPreferences.getInt(KEY_TEMPERATURE, -99);
      weatherData.temperatureHigh =
          defaultSharedPreferences.getInt(KEY_TEMPERATURE_HIGH, -99);
      weatherData.temperatureLow =
          defaultSharedPreferences.getInt(KEY_TEMPERATURE_LOW, -99);
      weatherData.temperatureUnit = defaultSharedPreferences.getString(
          KEY_TEMPERATURE_UNIT, TEMP_UNIT_CELSIUS);
  ...
  public void
  getCurrentWeatherInformation(final IWeatherCallback iWeatherCallback) {
  ...
  public void subscribeCallback(final
                                @NonNull IWeatherCallback iWeatherCallback) {
    Log.i(TAG, "subscribe new weather callback: " +
                   iWeatherCallback.getClass().getSimpleName());
    if (!isAppEnabled()) {
      Log.w(
          TAG,
          "the weather application is not installed, may not receive the updates");
    }
  ...
  public void unsubscribeCallback(final
                                  @NonNull IWeatherCallback iWeatherCallback) {
    if (mCallbacks.contains(iWeatherCallback)) {
      Log.i(TAG, "un-subscribe the weather callback: " +
                     iWeatherCallback.getClass().getSimpleName());
      mCallbacks.remove(iWeatherCallback);
      return;
    }
  ...
  public void registerContentObserver(final ContentResolver contentResolver) {
    if (isAppEnabled()) {
      ...
      Log.d(str, stringBuilder.toString());
      try {
        contentResolver.registerContentObserver(WEATHER_CONTENT_URI, true,
                                                mObserver);
        mUiWorkerHandler.post(this::queryWeatherInformation);
  ...
  private boolean isAppEnabled() {
    return PackageManagerHelper.isAppEnabled(mContext.getPackageManager(),
                                             WEATHER_PACKAGE_NAME, 0);
  }

  private void queryWeatherInformation() {
    if (isAppEnabled()) {
      processWeatherInformation(mContext.getContentResolver().query(
  ...
  private void processWeatherInformation(final Cursor cursor) {
    ...
    if (cursor == null) {
      ...
      if (cursor.getColumnCount() < WEATHER_COLUMNS.values().length) {
        Log.e(TAG,
              "the column count is not met the spec, contact OPWeather owner.");
        String stringBuilder2 =
            "expected columns: " + WEATHER_COLUMNS.values().length +
      ...
      try {
        ...
        Log.d(TAG, "[Raw Weather Data] timestamp: " + string2 + ", city: " +
                       string + ", code: " + string3 + ", name: " + string4 +
                       ", temp: " + string5 + ", high: " + string6 +
                       ", low: " + string7 + ", unit: " + string8);
        weatherData.timestamp =
            new SimpleDateFormat("yyyyMMddkkmm", Locale.getDefault())
                .parse(string2)
                .getTime() /
            1000;
        weatherData.cityName = string;
        weatherData.weatherCode =
            WEATHER_TYPE.getWeather(Integer.parseInt(string3)).weatherCode;
      ...
      Log.e(
          TAG,
          "cannot move the cursor point to the first row, is the cursor empty?");
      cursor.close();
    }
  }
  ...
  private void updateWeatherCallbacks(final WeatherData weatherData,
    ...
    Log.d(TAG, "push the weather information to: " +
                   iWeatherCallback.getClass().getSimpleName());
  ...
  private void writePreferences(final WeatherData weatherData) {
    PreferenceManager.getDefaultSharedPreferences(mContext)
        ...
        .putString(KEY_TEMPERATURE_UNIT, weatherData.temperatureUnit)
        .apply();
  }
}
