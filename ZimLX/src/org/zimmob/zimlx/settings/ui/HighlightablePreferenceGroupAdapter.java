...
public class HighlightablePreferenceGroupAdapter
  ...
  @VisibleForTesting final int mHighlightColor;
   ...
   * <p/>
   * Initial expanded child count will be ignored if: 1. fragment contains
   * request to highlight a particular row. 2. count value is invalid.
   */
  public static void
  adjustInitialExpandedChildCount(final SettingsActivity.BaseFragment host) {
    if (host == null) {
      return;
    }
  ...
  public HighlightablePreferenceGroupAdapter(
      final PreferenceGroup preferenceGroup, final String key,
    ...
    context.getTheme().resolveAttribute(android.R.attr.windowBackground,
                                        outValue, true);
    mInvisibleBackground = ColorUtils.setAlphaComponent(
        ContextCompat.getColor(context, outValue.resourceId), 0);
  ...
  void updateBackground(final PreferenceViewHolder holder, final int position) {
    ...
    if (position == mHighlightPosition) {
      // This position should be highlighted. If it's highlighted before - skip
      // animation.
  ...
  public void requestHighlight(final View root,
                               final RecyclerView recyclerView) {
    if (mHighlightRequested || recyclerView == null ||
        TextUtils.isEmpty(mHighlightKey)) {
      return;
    }
  ...
  public boolean isHighlightRequested() { return mHighlightRequested; }
  ...
  private void removeHighlightBackground(final View v, final boolean animate) {
    ...
    colorAnimation.addListener(new AnimatorListenerAdapter() {
      ...
      public void onAnimationEnd(final Animator animation) {
        ...
        // mNormalBackgroundRes so it is white and has ripple on touch.
        v.setBackgroundResource(mNormalBackgroundRes);
      }
    });
