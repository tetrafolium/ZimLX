...
public void showDialogWithHtmlTextView(final @StringRes int resTitleId,
                                       final String html) {
	showDialogWithHtmlTextView(resTitleId, html, true, null);
}

public void showDialogWithHtmlTextView(
	final @StringRes int resTitleId, final String text, final boolean isHtml,
	final DialogInterface.OnDismissListener dismissedListener) {
...
public void updateAppSummary(final Preference prefx) {
	...
	if ((pref = prefx) != null && pref.getSummary() == null) {
		...
		pref.setSummary(String.format(locale, "%s\nVersion v%s (build %d)",
		                              getPackageName(), getAppVersionName(),
		                              getAppVersionCode()));
	}
}
...
public void updateAppInfoSummary(final Preference prefx) {
	...
	if ((pref = prefx) != null && pref.getSummary() == null) {
		...
		String summary = String.format(
			locale, "\n<b>Package:</b> %s\n<b>Version:</b> v%s (build %s)",
			getPackageName(), getAppVersionName(), getAppVersionCode());
		summary += (tmp = bcstr("FLAVOR", "")).isEmpty()
		     ? ""
		     : ("\n<b>Flavor:</b> " + tmp.replace("flavor", ""));
		summary +=
			(tmp = bcstr("BUILD_TYPE", "")).isEmpty() ? "" : (" (" + tmp + ")");
...
public String getAppInstallationSource() {
	...
	try {
		src =
			context.getPackageManager().getInstallerPackageName(getPackageName());
	} catch (Exception ignored) {
	}
...
public int getResId(final ContextUtils.ResType resType, final String name) {
	return context.getResources().getIdentifier(
		name, resType.name().toLowerCase(), context.getPackageName());
}
