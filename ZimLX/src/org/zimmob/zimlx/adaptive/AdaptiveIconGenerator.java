...
// Average number of derived colors (based on averages with ~100 icons and
// performance testing)
...
public AdaptiveIconGenerator(final Context context,
...
private void loop() {
	if (Utilities.ATLEAST_OREO && shouldWrap) {
		...
		initTmpIfNeeded();
		scale = normalizer.getScale(extractee, bounds, tmp.getIconMask(),
		                            outShape, MIN_VISIBLE_ALPHA);
		...
		SparseIntArray rgbScoreHistogram =
			new SparseIntArray(NUMBER_OF_COLORS_GUESSTIMATION);
		...
		// Any icon with less than 10% transparent pixels (padding excluded) is
		// considered "full-bleed-ish"
		final int maxTransparent = (int)(round(size * .10) + addPixels);
		// Any icon with less than 27% transparent pixels (padding excluded)
		// doesn't need a color mix-in
		...
		// isBackgroundWhite = true = is adaptive
		...
		// Adjust color to reach suitable contrast depending on the relationship
		// between the colors
