...
/**
 * <p>This class can also be created via XML inflation using
 * <code>&lt;adaptive-icon></code> tag in addition to dynamic creation.
 *
 * <p>This drawable supports two drawable layers: foreground and background. The
 * layers are clipped when rendering using the mask defined in the device
 * configuration.
 *
 * <ul>
 * <li>Both foreground and background layers should be sized at 108 x 108
 * dp.</li> <li>The inner 72 x 72 dp  of the icon appears within the masked
 * viewport.</li> <li>The outer 18 dp on each of the 4 sides of the layers is
 * reserved for use by the system UI surfaces to create interesting visual
 * effects, such as parallax or pulsing.</li>
 * </ul>
 * <p>
 * Such motion effect is achieved by internally setting the bounds of the
 * foreground and background layer as following: <pre> Rect(getBounds().left -
 * getBounds().getWidth() * #getExtraInsetFraction(), getBounds().top -
 * getBounds().getHeight() * #getExtraInsetFraction(), getBounds().right +
 * getBounds().getWidth() * #getExtraInsetFraction(), getBounds().bottom +
 * getBounds().getHeight() * #getExtraInsetFraction())
 * </pre>
 */
public class AdaptiveIconCompat extends Drawable implements Drawable.Callback {
  ...
  /**
   * Mask path is defined inside device configuration in following dimension:
   * [100 x 100]
   */
   ...
   * transformations on the layers. <p> Each layers will reserve 25% of it's
   * width and height. <p> As a result, the view port of the layers is smaller
   * than their intrinsic width and height.
   */
  private static final float EXTRA_INSET_PERCENTAGE = 1 / 4f;
  private static final float DEFAULT_VIEW_PORT_SCALE =
      1f / (1 + 2 * EXTRA_INSET_PERCENTAGE);
  ...
  static {
    try {
      methodExtractThemeAttrs =
          TypedArray.class.getDeclaredMethod("extractThemeAttrs");
      methodCreateFromXmlInnerForDensity = Drawable.class.getDeclaredMethod(
          "createFromXmlInnerForDensity", Resources.class, XmlPullParser.class,
          AttributeSet.class, int.class, Theme.class);
  ...
  AdaptiveIconCompat(final @Nullable LayerState state,
  ...
  private <T> T invoke(final Method method, final Object obj,
                       final Object... params) {
    try {
  ...
  private Path createMaskPath() {
    try {
      return IconShapeManager.getInstanceNoCreate()
          .getIconShape()
          .getMaskPath();
    } catch (Exception e) {
  ...
  LayerState createConstantState(final @Nullable LayerState state,
                                 final @Nullable Resources res) {
   ...
   *
   * @param backgroundDrawable drawable that should be rendered in the
   *     background
   * @param foregroundDrawable drawable that should be rendered in the
   *     foreground
   */
  @RequiresApi(api = VERSION_CODES.O)
  ...
  @Override
  public void
  inflate(final @NonNull Resources r, final @NonNull XmlPullParser parser,
  ...
  static int resolveDensity(final @Nullable Resources r,
                            final int parentDensity) {
    final int densityDpi =
        r == null ? parentDensity : r.getDisplayMetrics().densityDpi;
   ...
   *
   * @see #getForeground() and #getBackground() for more info on how this value
   *     is used
   */
   ...
   * drawable is extended by {@link #getExtraInsetFraction()} *
   * getBounds().width on left/right sides and by
  ...
  /**
   * Set the child layer bounds bigger than the view port size by {@link
  ...
  private void updateLayerBoundsInternal(final Rect bounds) {
    ...
    for (int i = 0, count = LayerState.N_CHILDREN; i < count; i++) {
      ...
      outRect.set(cX - insetWidth, cY - insetHeight, cX + insetWidth,
                  cY + insetHeight);
  ...
  private void updateMaskBoundsInternal(final Rect b) {
    ...
    if (mMaskBitmap == null || mMaskBitmap.getWidth() != b.width() ||
      ...
      mMaskBitmap =
          Bitmap.createBitmap(b.width(), b.height(), Bitmap.Config.ALPHA_8);
      mLayersBitmap =
  ...
  public void draw(final Canvas canvas) {
    ...
    if (mLayersShader == null) {
      ...
      mLayersShader =
          new BitmapShader(mLayersBitmap, TileMode.CLAMP, TileMode.CLAMP);
   ...
   */
  private void
  inflateLayers(final @NonNull Resources r, final @NonNull XmlPullParser parser,
                final @NonNull AttributeSet attrs, final @Nullable Theme theme)
      throws XmlPullParserException, IOException {
    final LayerState state = mLayerState;
    ...
    while ((type = parser.next()) != XmlPullParser.END_DOCUMENT &&
      ...
      final TypedArray a = obtainAttributes(
          r, theme, attrs, new int[] {android.R.attr.drawable});
      ...
      if (layer.mDrawable == null && (layer.mThemeAttrs == null)) {
        ...
        layer.mDrawable = createFromXmlInnerForDensity(
            r, parser, attrs, mLayerState.mSrcDensityOverride, theme);
        layer.mDrawable.setCallback(this);
  ...
  private static Drawable
  createFromXmlInnerForDensity(final @NonNull Resources r,
                               final @NonNull XmlPullParser parser,
                               final @NonNull AttributeSet attrs,
                               final int density, final @Nullable Theme theme) {
    try {
      return (Drawable)methodCreateFromXmlInnerForDensity.invoke(
          null, r, parser, attrs, density, theme);
    } catch (IllegalAccessException | InvocationTargetException e) {
  ...
  private void updateLayerFromTypedArray(final @NonNull ChildDrawable layer,
                                         final @NonNull TypedArray a) {
    final LayerState state = mLayerState;
  ...
  private Drawable getDrawable(final TypedArray a, final int index) {
    ...
    if (value.resourceId != 0)
      return a.getResources().getDrawableForDensity(value.resourceId, 480,
                                                    null);
    return null;
  }
  ...
  public boolean canApplyTheme() {
    return (mLayerState != null && mLayerState.canApplyTheme()) ||
        super.canApplyTheme();
  }
  ...
  public int getChangingConfigurations() {
    return super.getChangingConfigurations() |
        mLayerState.getChangingConfigurations();
  }
  ...
  @Override
  public void setHotspotBounds(final int left, final int top, final int right,
                               final int bottom) {
    final ChildDrawable[] array = mLayerState.mChildren;
    for (int i = 0; i < LayerState.N_CHILDREN; i++) {
      final Drawable dr = array[i].mDrawable;
  ...
  protected static @NonNull TypedArray obtainAttributes(
      final @NonNull Resources res, final @Nullable Theme theme,
      final @NonNull AttributeSet set, final @NonNull int[] attrs) {
    if (theme == null) {
  ...
  static class ChildDrawable {
    ...
    ChildDrawable(final int density) { mDensity = density; }

    ChildDrawable(final @NonNull ChildDrawable orig,
                  final @NonNull AdaptiveIconCompat owner,
                  final @Nullable Resources res) {
  ...
  static class LayerState extends ConstantState {
    ...
    // The density to use when inflating/looking up the children drawables. A
    // value of 0 means use the system's density.
    ...
    LayerState(final @Nullable LayerState orig,
               final @NonNull AdaptiveIconCompat owner,
               final @Nullable Resources res) {
  ...
  public static Drawable wrap(final @NonNull Drawable icon) {
    if (Utilities.ATLEAST_OREO && icon instanceof AdaptiveIconDrawable) {
      ...
      return new AdaptiveIconCompat(adaptive.getBackground(),
                                    adaptive.getForeground());
    } else {
      return icon;
    }
  }
  ...
  public static void resetMask() { sMask = null; }
}
