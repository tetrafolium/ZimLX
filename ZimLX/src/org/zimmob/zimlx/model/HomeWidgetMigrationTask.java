...
protected boolean migrateWorkspace() throws Exception {
	...
	boolean allowOverlap =
		Utilities.getPrefs(mContext).getBoolean(ALLOW_OVERLAP_PREF, false);
	...
	if (allowOverlap || occupied.isRegionVacant(0, 0, mTrgX, 1)) {
		...
		if (!customWidgets.isEmpty()) {
			...
			int widgetId = CustomWidgetParser.getWidgetIdForCustomProvider(
				mContext, provider.provider);
			long itemId = LauncherSettings.Settings
...
public static void migrateIfNeeded(final Context context) {
	...
	try {
		if (!new HomeWidgetMigrationTask(
			    context, LauncherAppState.getIDP(context), validPackages, size)
		    .migrateWorkspace()) {
