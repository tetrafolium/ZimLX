...
public class PinchStateChangeTouchController extends AnimatorListenerAdapter
    implements TouchController, ScaleGestureDetector.OnScaleGestureListener {
  ...
  @Override
  public boolean
  onControllerInterceptTouchEvent(final MotionEvent motionEvent) {
  ...
  public boolean onControllerTouchEvent(final MotionEvent motionEvent) {
    return mPinchStarted && motionEvent.getPointerCount() <= 2 &&
        mDetector.onTouchEvent(motionEvent);
  }
  ...
  private LauncherState getTargetState(final LauncherState launcherState) {
    ...
    return launcherState == LauncherState.OPTIONS ? LauncherState.NORMAL
                                                  : launcherState;
  }
  ...
  private boolean reInitCurrentAnimation() {
    ...
    mInterpolator = mLauncher.isInState(LauncherState.OPTIONS)
  ...
  public boolean onScaleBegin(final ScaleGestureDetector r4) {
    ...
    if (mLauncher.isInState(LauncherState.NORMAL) ||
        mLauncher.isInState(LauncherState.OPTIONS)) {
  ...
  public boolean onScale(final ScaleGestureDetector scaleGestureDetector) {
    ...
    float f =
        LauncherState.OPTIONS.getWorkspaceScaleAndTranslation(mLauncher)[0];
    currentSpan = mInterpolator.getInterpolation(
        (Math.max(f, Math.min(mFromState.getWorkspaceScaleAndTranslation(
                                  mLauncher)[0] +
                                  currentSpan / width,
                              1)) -
         f) /
  ...
  public void onScaleEnd(final ScaleGestureDetector scaleGestureDetector) {
    ...
    boolean z =
        (mLauncher.isInState(LauncherState.OPTIONS) && f2 >= FLING_VELOCITY) ||
        (!mLauncher.isInState(LauncherState.OPTIONS) && f2 <= -0.003f);
    if (z) {
      i = 4;
      launcherState =
          Float.compare(Math.signum(f2), Math.signum(mProgressMultiplier)) == 0
              ? mToState
              : mFromState;
    } else {
      i = 5;
    ...
    if (launcherState2 == this.mToState) {
      if (progressFraction >= 1) {
        ...
        f = Utilities.boundToRange(
            ((16 * f2) * mProgressMultiplier) + progressFraction, 0, 1);
        j = SwipeDetector.calculateDuration(f2,
                                            1 - Math.max(progressFraction, 0));
        f3 = 1;
      }
      ...
      f = Utilities.boundToRange(
          ((16 * f2) * mProgressMultiplier) + progressFraction, 0, 1);
      j = SwipeDetector.calculateDuration(f2,
                                          Math.min(progressFraction, 1) - 0);
    }
    mCurrentAnimation.setEndAction(
        () -> onPinchInteractionCompleted(launcherState2, i));
    ValueAnimator animationPlayer = mCurrentAnimation.getAnimationPlayer();
    animationPlayer.setFloatValues(f, f3);
    animationPlayer.setDuration(j).setInterpolator(
        Interpolators.scrollInterpolatorForVelocity(f2));
  ...
  private void onPinchInteractionCompleted(final LauncherState launcherState,
                                           final int i) {
    clearState();
    boolean i2 = true;
    ...
    if (i2) {
      if (launcherState != mFromState) {
        mLauncher.getUserEventDispatcher().logStateChangeAction(
            i, getDirectionForLog(), 6, mFromState.containerType,
            mToState.containerType, mLauncher.getWorkspace().getCurrentPage());
      }
      mLauncher.getStateManager().goToState(launcherState, false);
    }
  }

  private void clearState() { mCurrentAnimation = null; }
}
