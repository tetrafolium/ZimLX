...
public class DynamicClock extends BroadcastReceiver {
    ...
    try {
      ...
      if (metaData != null) {
        int levelPerTickIcon = metaData.getInt(
            "com.google.android.apps.nexuslauncher.LEVEL_PER_TICK_ICON_ROUND",
            0);
        if (levelPerTickIcon != 0) {
          ...
          if (layers.mMinuteIndex < 0 ||
              layers.mMinuteIndex >= numberOfLayers) {
            layers.mMinuteIndex = -1;
          }
          if (layers.mSecondIndex < 0 ||
              layers.mSecondIndex >= numberOfLayers) {
  ...
  private void updateMainThread() {
    new MainThreadExecutor().execute(new Runnable() {
      @Override
      public void run() {
        updateWrapper(getClockLayers(
            mContext, LauncherAppState.getIDP(mContext).fillResIconDpi,
  ...
  public AutoUpdateClock drawIcon(final Bitmap bitmap) {
    final AutoUpdateClock updater =
        new AutoUpdateClock(bitmap, mLayers.clone());
