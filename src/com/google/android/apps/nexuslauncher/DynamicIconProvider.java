...
public DynamicIconProvider(final Context context) {
		...
		public void onReceive(final Context context, final Intent intent) {
			...
			for (UserHandle user :
				...
				List<ShortcutInfoCompat> shortcuts =
					DeepShortcutManager.getInstance(context).queryForPinnedShortcuts(
	...
	mContext.registerReceiver(mDateChangeReceiver, intentFilter, null,
...
private int getDayResId(final Bundle bundle, final Resources resources) {
	if (bundle != null) {
		int dateArrayId =
			bundle.getInt(GOOGLE_CALENDAR + ".dynamic_icons_nexus_round", 0);
		if (dateArrayId != 0) {
			try {
...
@Override
public Drawable getIcon(final LauncherActivityInfo launcherActivityInfo,
                        final int iconDpi, final boolean flattenDrawable) {
	...
	if (isCalendar(packageName)) {
		try {
			Bundle metaData =
				mPackageManager
				.getActivityInfo(launcherActivityInfo.getComponentName(),
			...
			if (dayResId != 0) {
				drawable = AdaptiveIconCompat.wrapNullable(
					resourcesForApplication.getDrawableForDensity(dayResId, iconDpi));
			}
		} catch (NameNotFoundException ignored) {
		}
	} else if (!flattenDrawable && Utilities.ATLEAST_OREO &&
	           DynamicClock.DESK_CLOCK.equals(
