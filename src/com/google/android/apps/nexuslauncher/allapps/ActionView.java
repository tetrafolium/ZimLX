...
public class ActionView extends BubbleTextView implements OnLongClickListener {
  ...
  private class MyDragPreviewProvider extends DragPreviewProvider {
    ...
    public float getScaleAndPosition(final Bitmap bitmap, final int[] iArr) {
      ...
      float iconSize = ((float)ActionView.this.getIconSize()) /
                       ((float)launcher.getDeviceProfile().iconSizePx);
    ...
    public Bitmap createDragBitmap() {
      Rect drawableBounds =
          DragPreviewProvider.getDrawableBounds(ActionView.this.getIcon());
      float width = drawableBounds.width() > 0
                        ? ((float)Launcher.getLauncher(this.mView.getContext())
                               .getDeviceProfile()
                               .iconSizePx) /
                              ((float)drawableBounds.width())
                        : 1.0f;
      int width1 = ((int)(((float)drawableBounds.width()) * width)) +
                   this.blurSizeOutline;
      int height = ((int)(((float)drawableBounds.height()) * width)) +
                   this.blurSizeOutline;
      return BitmapRenderer.createHardwareBitmap(
          width1, height, out -> { drawDragView(out, width); });
    }
  }

  public ActionView(final @NonNull Context context) { this(context, null); }
  ...
  public ActionView(final @NonNull Context context,
    ...
    setCompoundDrawablePadding(getResources().getDimensionPixelSize(
        R.dimen.action_view_compound_drawable_padding));

    BaseDraggingActivity activity =
        ZimUtilsKt.getBaseDraggingActivityOrNull(context);
  ...
  private void logClickEvent() {
    // actionsController.get(getContext()).getLogger().logClick(this.mAction.id,
    // this.mPos);
  }
  ...
  protected void onTextChanged(final CharSequence charSequence, final int i,
  ...
  public void onDraw(final Canvas canvas) {
    float measuredWidth =
        ((float)(getMeasuredWidth() - this.mMeasuredUnspecifiedWidth)) / 2.0f;
  ...
  public void getIconBounds(final Rect rect) {
    int measuredWidth =
        (getMeasuredWidth() - this.mMeasuredUnspecifiedWidth) / 2;
    ...
    rect.set(paddingLeft, measuredWidth, iconSize,
             getIconSize() + measuredWidth);
  }
  ...
  public boolean onLongClick(final View view) {
    ...
    if ((!launcher.isInState(LauncherState.ALL_APPS) &&
         !launcher.isInState(LauncherState.OVERVIEW)) ||
    ...
    dragOptions.intrinsicIconScaleFactor =
        ((float)deviceProfile.allAppsIconSizePx) /
        ((float)deviceProfile.iconSizePx);
    ItemInfo itemInfo = (ItemInfo)view.getTag();
    Point point = new Point();
    point.x = this.mLastTouchPos.x;
    point.y = this.mLastTouchPos.y;
    DragView dragView = launcher.getWorkspace().beginDragShared(
        view, launcher.getAppsView(), itemInfo,
        new MyDragPreviewProvider(view, point), dragOptions);
    Rect rect = new Rect();
    getIconBounds(rect);
