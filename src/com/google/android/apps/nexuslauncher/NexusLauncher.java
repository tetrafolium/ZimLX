...
public class NexusLauncher {
  ...
  class NexusLauncherCallbacks
    ...
    private Set<SmartspaceView> mSmartspaceViews =
        Collections.newSetFromMap(new WeakHashMap<>());
    private final FeedReconnector mFeedReconnector = new FeedReconnector();

    // private final Runnable mUpdatePredictionsIfResumed =
    // this::updatePredictionsIfResumed;

    private ItemInfoUpdateReceiver getUpdateReceiver() {
      if (mItemInfoUpdateReceiver == null) {
        mItemInfoUpdateReceiver =
            new ItemInfoUpdateReceiver(mLauncher, mCallbacks);
      }
    ...
    public boolean hasCustomContentToLeft() { return false; }
    ...
    public void onCreate(final Bundle bundle) {
      ...
      mClient = new LauncherClient(
          mLauncher, mOverlay,
          new StaticInteger(
              (prefs.getBoolean(SettingsActivity.ENABLE_MINUS_ONE_PREF, true)
                   ? 1
                   : 0) |
              2 | 4 | 8));
    ...
    public void onDestroy() {
      ...
      if (!launcherClient.mDestroyed) {
        launcherClient.mActivity.unregisterReceiver(
            launcherClient.googleInstallListener);
      }
      ...
      Utilities.getPrefs(mLauncher).unregisterOnSharedPreferenceChangeListener(
          this);
    ...
    public boolean startSearch(final String s, final boolean b,
                               final Bundle bundle) {
    ...
    onSharedPreferenceChanged(final SharedPreferences sharedPreferences,
      ...
      case SettingsActivity.ENABLE_MINUS_ONE_PREF:
        LauncherClient launcherClient = mClient;
        StaticInteger i =
            new StaticInteger((sharedPreferences.getBoolean(
                                   SettingsActivity.ENABLE_MINUS_ONE_PREF, true)
                                   ? 1
                                   : 0) |
                              2 | 4 | 8);
    ...
    onExtractedColorsChanged(final WallpaperColorInfo wallpaperColorInfo) {
      int alpha = mLauncher.getResources().getInteger(
      ...
      mUiInformation.putInt(
          "background_secondary_color_hint",
    ...
    private void applyFeedTheme(final boolean redraw) {
      String prefValue = Utilities.getPrefs(mLauncher).getString(
          SettingsActivity.FEED_THEME_PREF, null);
      int feedTheme;
      try {
      ...
      boolean isDark =
          auto ? Themes.getAttrBoolean(mLauncher, R.attr.isMainColorDark)
               : preferDark;
    ...
    /*private void updatePredictionsIfResumed() {
        if (mLauncher.hasBeenResumed()) {
            ...
            //        FeatureFlags.REFLECTION_FORCE_OVERVIEW_MODE ?
    Client.OVERVIEW.id : Client.HOME.id); handler.post(() -> {
                mLauncher.getUserEventDispatcher().updatePredictions();
    ...
    class FeedReconnector implements Runnable {
      ...
      void stop() { mHandler.removeCallbacks(this); }

      @Override
      public void run() {
        if (Utilities.getPrefs(mLauncher).getBoolean(
                SettingsActivity.ENABLE_MINUS_ONE_PREF, true) &&
            !mClient.mDestroyed && mClient.mLayoutParams != null &&
            !mOverlay.mAttached && mFeedConnectionTries++ < MAX_RETRIES) {
  ...
  public static int primaryColor(final WallpaperColorInfo wallpaperColorInfo,
                                 final Context context, final int alpha) {
    return compositeAllApps(
        ColorUtils.setAlphaComponent(wallpaperColorInfo.getMainColor(), alpha),
        context);
  }

  public static int secondaryColor(final WallpaperColorInfo wallpaperColorInfo,
                                   final Context context, final int alpha) {
    return compositeAllApps(ColorUtils.setAlphaComponent(
                                wallpaperColorInfo.getSecondaryColor(), alpha),
                            context);
  }
