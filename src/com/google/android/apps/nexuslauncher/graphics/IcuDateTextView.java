...
public class IcuDateTextView extends DoubleShadowTextView {
  ...
  public IcuDateTextView(final Context context) { this(context, null); }
  ...
  public void reloadDateFormat(final boolean forcedChange) {
    ...
    if (Utilities.ATLEAST_NOUGAT) {
      mDateFormat = getDateFormat(getContext(), forcedChange, mDateFormat,
                                  getId() == R.id.time_above);
      format = mDateFormat.format(System.currentTimeMillis());
    } else {
      format = DateUtils.formatDateTime(
  ...
  getDateFormat(final Context context, final boolean forcedChange,
                final DateFormat oldFormat, final boolean isTimeAbove) {
    if (oldFormat == null || forcedChange) {
      (oldFormat = DateFormat.getInstanceForSkeleton(
           context.getString(R.string.icu_abbrev_wday_month_day_no_year),
           Locale.getDefault()))
    ...
    if ((showTime && !timeAbove) || isTimeAbove) {
      String format = context.getString(show24h ? R.string.icu_abbrev_time
                                                : R.string.icu_abbrev_time_12h);
      if (showDate && !isTimeAbove)
        format += context.getString(R.string.icu_abbrev_date);
      (oldFormat =
           DateFormat.getInstanceForSkeleton(format, Locale.getDefault()))
          .setContext(DisplayContext.CAPITALIZATION_FOR_STANDALONE);
    }
    return oldFormat;
  }
