...
public class SmartspaceView extends FrameLayout
    implements ISmartspace, ValueAnimator.AnimatorUpdateListener,
               View.OnClickListener, View.OnLongClickListener, Runnable,
               ZimSmartspaceController.Listener {
  ...
  public SmartspaceView(final Context context, final AttributeSet set) {
    ...
    mCalendarClickListener = v -> {
      ...
      try {
        Launcher.getLauncher(getContext())
            .startActivitySafely(v, addFlags, null);
    ...
    dH = ColorStateList.valueOf(
        Themes.getAttrColor(getContext(), R.attr.workspaceTextColor));
    ds = dp.cY();
    mSmartspaceBackgroundRes = R.drawable.bg_smartspace;
    dB = new TextPaint();
    dB.setTextSize((float)getResources().getDimensionPixelSize(
        R.dimen.smartspace_title_size));
    ...
    mTitleMinSize =
        res.getDimensionPixelSize(R.dimen.smartspace_title_min_size);
    mHorizontalPadding =
        res.getDimensionPixelSize(R.dimen.smartspace_horizontal_padding);
    mSeparatorWidth =
        res.getDimensionPixelSize(R.dimen.smartspace_title_sep_width);
    mWeatherIconSize =
        res.getDimensionPixelSize(R.dimen.smartspace_title_weather_icon_size);
  ...
  protected void onMeasure(final int widthMeasureSpec,
    ...
    if (!mDoubleLine && mClockView != null && mTitleWeatherText != null &&
        mTitleWeatherText.getVisibility() == View.VISIBLE) {
      int textSize = mTitleSize;
      String title = mClockView.getText().toString() +
                     mTitleWeatherText.getText().toString();
      mTextPaint.set(mClockView.getPaint());
      while (true) {
        mTextPaint.setTextSize(textSize);
        mTextPaint.getTextBounds(title, 0, title.length(), mTextBounds);
        int padding =
            getPaddingRight() + getPaddingLeft() + mClockView.getPaddingLeft() +
            mClockView.getPaddingRight() + mTitleWeatherText.getPaddingLeft() +
            mTitleWeatherText.getPaddingRight();
        if (padding + mSeparatorWidth + mWeatherIconSize +
                mTextBounds.width() <=
            size) {
          break;
        }
  ...
  public final void setTitleSize(final int size) {
    ...
    if (mTitleWeatherText != null &&
        ((int)mTitleWeatherText.getTextSize()) != size) {
  ...
  public void onDataUpdated(final @Nullable WeatherData weather,
    ...
    if (mController.getRequiresSetup()) {
      if (getParent() instanceof SmartspacePreview) {
        ...
        lines.add(
            new Line(getContext().getString(R.string.smartspace_setup_text)));
  ...
  private void loadDoubleLine(final @Nullable WeatherData weather,
                              final @NotNull CardData card) {
  ...
  @SuppressWarnings("ConstantConditions")
  private void loadSingleLine(final @Nullable WeatherData weather,
                              final @Nullable CardData card) {
    setOnClickListener(null);
    setBackgroundResource(0);
    bindWeather(weather, mTitleWeatherContent, mTitleWeatherText,
                mTitleWeatherIcon);
  ...
  private void bindWeather(final @Nullable WeatherData weather,
                           final View container, final TextView title,
  ...
  private String cn() {
    ...
    return dp.cC(TextUtils
                     .ellipsize(dp.cB(b), dB,
                                getWidth() - getPaddingLeft() -
                                    getPaddingRight() -
                                    getResources().getDimensionPixelSize(
                                        R.dimen.smartspace_horizontal_padding) -
                                    dB.measureText(dp.cA(b)),
                                TextUtils.TruncateAt.END)
                     .toString());
  }

  private OnLongClickListener co() { return this; }

  private void cs() {
    final int indexOfChild = indexOfChild(mSmartspaceContent);
    removeView(mSmartspaceContent);
    final LayoutInflater from = LayoutInflater.from(getContext());
    addView(from.inflate(mDoubleLine ? R.layout.smartspace_twolines
  ...
  public boolean onLongClick(final View view) {
    ...
    if (textView != null) {
      ...
      tmp =
          (((float)view.getHeight()) - (fontMetrics.bottom - fontMetrics.top)) /
          2.0f;
    }
  ...
  public void onResume() { backportClockVisibility(true); }
  ...
  @Override
  public void setPadding(final int n, final int n2, final int n3,
                         final int n4) {
  ...
  final class h implements OnClickListener {
    ...
    h(final SmartspaceView dz) { dZ = dz; }
