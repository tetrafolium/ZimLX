...
public class CustomBottomSheet extends WidgetsBottomSheet {
  ...
  public void populateAndShow(final ItemInfo itemInfo) {
    ...
    mInfoProvider =
        CustomInfoProvider.Companion.forItem(getContext(), mItemInfo);

    TextView title = findViewById(R.id.title);
    title.setText(itemInfo.title);
    ((PrefsFragment)mFragmentManager.findFragmentById(R.id.sheet_prefs))
        .loadForApp(itemInfo, this::setForceOpen, this::unsetForceOpen,
                    this::reopen);
    ...
    if (itemInfo instanceof ItemInfoWithIcon || mInfoProvider.supportsIcon()) {
      ...
      if (itemInfo instanceof ShortcutInfo &&
          ((ShortcutInfo)itemInfo).customIcon != null) {
        ...
        if (folderInfo.container == ItemInfo.NO_ID) {
          // TODO: Allow editing title for drawer folder & sync with group
          // backend
      ...
      if (mInfoProvider != null) {
        ...
        icon.setOnClickListener(v -> {
          ItemInfo editItem;
          if (mItemInfo instanceof FolderInfo &&
              ((FolderInfo)mItemInfo).isCoverMode()) {
  ...
  private void setForceOpen() { mForceOpen = true; }

  private void unsetForceOpen() { mForceOpen = false; }
  ...
  public static class PrefsFragment extends PreferenceFragment
    ...
    private final static String PREF_HIDE_FROM_PREDICTIONS =
        "pref_app_prediction_hide";
    ...
    public void loadForApp(final ItemInfo info, final Runnable setForceOpen,
                           final Runnable unsetForceOpen,
      ...
      mSwipeUpPref = (LauncherGesturePreference)screen.findPreference(
          "pref_swipe_up_gesture");
      mTabsPref =
          (MultiSelectTabPreference)screen.findPreference("pref_show_in_tabs");
      ...
      if (mPrefHidePredictions != null) {
        mPrefHidePredictions.setChecked(
            CustomAppPredictor.isHiddenApp(context, mKey));
        mPrefHidePredictions.setOnPreferenceChangeListener(this);
      }

      if (prefs.getShowDebugInfo() && mKey != null &&
          mKey.componentName != null) {
        Preference componentPref =
            getPreferenceScreen().findPreference("componentName");
        Preference versionPref =
            getPreferenceScreen().findPreference("versionName");
        ...
        versionPref.setSummary(
            new PackageManagerHelper(context).getPackageVersion(
                mKey.componentName.getPackageName()));
      } else {
        getPreferenceScreen().removePreference(
            getPreferenceScreen().findPreference("debug"));
      }

      mPrefHidePredictions =
          (SwitchPreference)getPreferenceScreen().findPreference(
              PREF_HIDE_FROM_PREDICTIONS);
    ...
    private void onSelectHandler(final GestureHandler handler) {
      ...
      if (handler.getConfigIntent() != null) {
        setForceOpen.run();
        startActivityForResult(handler.getConfigIntent(),
                               PrefsFragment.requestCode);
      } else {
        updatePref();
      }
    }
    ...
    public void onDetach() {
      ...
      if (mProvider != null && selectedHandler != null) {
        ...
        CustomInfoProvider provider =
            CustomInfoProvider.Companion.forItem(getActivity(), itemInfo);
    ...
    public boolean onPreferenceClick(final Preference preference) {
      ...
      case "versionName":
        ...
        ClipData clip = ClipData.newPlainText(
            getString(R.string.debug_component_name), preference.getSummary());
        clipboard.setPrimaryClip(clip);
        Toast
            .makeText(getActivity(), R.string.debug_component_name_copied,
                      Toast.LENGTH_SHORT)
            .show();
        break;
      default:
  ...
  public static void show(final Launcher launcher, final ItemInfo itemInfo) {
    CustomBottomSheet cbs =
        (CustomBottomSheet)launcher.getLayoutInflater().inflate(
            R.layout.app_edit_bottom_sheet, launcher.getDragLayer(), false);
