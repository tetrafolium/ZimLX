...
public DateWidgetView(final Context context,
...
private void init() {
	...
	if (locale != null &&
		...
		mDateText2.setPadding(
			0, 0, 0,
			((int)(Math.abs(paint.getFontMetrics().ascent) - ((float)height))) /
			2);
	}
}
...
protected void onMeasure(final int widthMeasureSpec,
	...
	if (launcher != null) {
		...
		int size =
			MeasureSpec.getSize(widthMeasureSpec) / deviceProfile.inv.numColumns;
		marginEnd = (size - deviceProfile.iconSizePx) / 2;
		mWidth = (deviceProfile.inv.numColumns -
		          Math.max(1, (int)Math.ceil(
					   (double)(getResources().getDimension(
		...
		marginEnd = getResources().getDimensionPixelSize(
