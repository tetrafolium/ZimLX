...
public class CustomAppPredictor extends UserEventDispatcher
  ...
  private static final String HIDDEN_PREDICTIONS_SET_PREF =
  ...
  protected final static String[] PLACE_HOLDERS =
  ...
  public List<ComponentKeyMapper> getPredictions() {
    ...
    if (isPredictorEnabled()) {
      ...
      Collections.sort(
          predictionList,
          (o1, o2) -> Integer.compare(getLaunchCount(o2), getLaunchCount(o1)));
      ...
      for (int i = 0; i < PLACE_HOLDERS.length && list.size() < MAX_PREDICTIONS;
           i++) {
        ...
        if (intent != null) {
          ...
          if (componentInfo != null) {
            ComponentKey key =
                new ComponentKey(componentInfo, Process.myUserHandle());
            if (!predictionList.contains(key.toString()) &&
                !isHiddenApp(mContext, key)) {
  ...
  public void logAppLaunch(final View v, final Intent intent,
    ...
    if (isPredictorEnabled() && recursiveIsDrawer(v)) {
      ComponentName componentInfo = intent.getComponent();
      if (componentInfo != null &&
          mAppFilter.shouldShowApp(componentInfo, user)) {
        clearNonExistingComponents();
        ...
        if (predictionSet.contains(prediction)) {
          edit.putInt(PREDICTION_PREFIX + prediction,
                      getLaunchCount(prediction) + BOOST_ON_OPEN);
        } else if (predictionSet.size() < MAX_PREDICTIONS ||
                   decayHasSpotFree(predictionSet, edit)) {
  ...
  private boolean decayHasSpotFree(final Set<String> toDecay,
  ...
  protected boolean isPredictorEnabled() {
    return Utilities.getPrefs(mContext).getBoolean(
        SettingsActivity.SHOW_PREDICTIONS_PREF, true);
  }
  ...
  private void clearNonExistingComponents() {
    ...
    for (String prediction : originalSet) {
      ...
      try {
        ...
        edit.remove(PREDICTION_PREFIX + prediction);
        Intent intent =
            mPackageManager.getLaunchIntentForPackage(cn.getPackageName());
        if (intent != null) {
          ComponentName componentInfo = intent.getComponent();
          if (componentInfo != null) {
            ComponentKey key =
  ...
  static void setComponentNameState(final Context context,
  ...
  protected static boolean isHiddenApp(final Context context,
                                       final ComponentKey key) {
  ...
  private static Set<String> getHiddenApps(final Context context) {
    return new HashSet<>(
        Utilities.getZimPrefs(context).getHiddenPredictionAppSet());
  }

  private static void setHiddenApps(final Context context,
  ...
  public static class UiManager {
    ...
    public boolean isEnabled() { return mPredictor.isPredictorEnabled(); }
    ...
    public interface Listener { void onPredictionsUpdated(); }
  }
}
