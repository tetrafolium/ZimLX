...
public ConfigBuilder(final AbstractQsbLayout qsbLayout,
                     final boolean isAllApps) {
	mBundle = new Bundle();
	mNano = new c_search();
...
public static Intent getSearchIntent(final Rect sourceBounds,
                                     final View gIcon, final View micIcon) {
...
private b_search bZ(final AppInfo appInfo, final int n) {
	...
	b.ek = new Intent(
		...
		uri.buildUpon()
		.appendQueryParameter("predictionRank", Integer.toString(n))
		.build())
	       .toUri(0);
	return b;
}
...
private RemoteViews searchIconTemplate() {
	final RemoteViews remoteViews = new RemoteViews(
		mActivity.getPackageName(), R.layout.apps_search_icon_template);
	...
	final int paddingBottom =
		mBubbleTextView.getHeight() - iconSize - paddingTop;
	remoteViews.setViewPadding(android.R.id.icon, horizontalPadding, paddingTop,
	                           horizontalPadding, paddingBottom);
	final int minPadding = Math.min(
		(int)(iconSize * 0.12f),
		Math.min(horizontalPadding, Math.min(paddingTop, paddingBottom)));
	remoteViews.setViewPadding(
		R.id.click_feedback_wrapper, horizontalPadding - minPadding,
		paddingTop - minPadding, horizontalPadding - minPadding,
		paddingBottom - minPadding);
	remoteViews.setTextViewTextSize(
		android.R.id.title, 0,
		mActivity.getDeviceProfile().allAppsIconTextSizePx);
	remoteViews.setViewPadding(android.R.id.title,
	                           mBubbleTextView.getPaddingLeft(),
	                           mBubbleTextView.getCompoundDrawablePadding() +
	                           mBubbleTextView.getIconSize(),
	                           mBubbleTextView.getPaddingRight(), 0);
...
private RemoteViews searchQsbTemplate() {
	final RemoteViews remoteViews = new RemoteViews(
		mActivity.getPackageName(), R.layout.apps_search_qsb_template);

	final int effectiveHeight = mQsbLayout.getHeight() -
	                            mQsbLayout.getPaddingTop() -
	                            mQsbLayout.getPaddingBottom() + 20;
	final Bitmap mShadowBitmap = mQsbLayout.mShadowBitmap;
	if (mShadowBitmap != null) {
		final int internalWidth =
			(mShadowBitmap.getWidth() - effectiveHeight) / 2;
		final int verticalPadding =
			(mQsbLayout.getHeight() - mShadowBitmap.getHeight()) / 2;
		remoteViews.setViewPadding(
			R.id.qsb_background_container,
			mQsbLayout.getPaddingLeft() - internalWidth, verticalPadding,
			mQsbLayout.getPaddingRight() - internalWidth, verticalPadding);
		final Bitmap bitmap =
			Bitmap.createBitmap(mShadowBitmap, 0, 0, mShadowBitmap.getWidth() / 2,
			                    mShadowBitmap.getHeight());
		final Bitmap bitmap2 = Bitmap.createBitmap(
			mShadowBitmap, (mShadowBitmap.getWidth() - 20) / 2, 0, 20,
			mShadowBitmap.getHeight());
	...
	int horizontalPadding =
	    ...
	    : gIcon.getLeft();
	remoteViews.setViewPadding(R.id.qsb_icon_container, horizontalPadding, 0,
	                           horizontalPadding, 0);
...
private void cd() {
	...
	if (viewBounds.eh > 0 && viewBounds.ee > 0) {
		Bitmap bitmap = BitmapRenderer.createHardwareBitmap(
			viewBounds.eh, viewBounds.ee, out->a(topShift, out));
		mBundle.putParcelable(mNano.eu, bitmap);
	} else {
		String stringBuilder =
			"Invalid preview bitmap size. width: " + viewBounds.eh +
			"hight: " + viewBounds.ee + " top shift: " + topShift;
...
private void a(final Canvas canvas, final View view) {
	final int[] array = {0, 0};
	mActivity.getDragLayer().mapCoordInSelfToDescendant(mActivity.getAppsView(),
	                                                    array);
...
private void ce() {
	...
	int allAppsCols = Math.min(mActivity.getDeviceProfile().inv.numColumns,
	                           appsView.getChildCount());
	int childCount = appsView.getChildCount();
	BubbleTextView[] bubbleTextViewArr = new BubbleTextView[allAppsCols];
	int i4 = -1;
	for (int i = 0; i < childCount; i++) {
		RecyclerView.ViewHolder childViewHolder =
			appsView.getChildViewHolder(appsView.getChildAt(i));
		if (childViewHolder.itemView instanceof BubbleTextView) {
			...
			if (spanGroupIndex >= 0) {
				...
				int index = ((GridLayoutManager.LayoutParams)
				             childViewHolder.itemView.getLayoutParams())
				            .getSpanIndex();
	...
	co = appsView.getChildViewHolder(bubbleTextViewArr[0]).getItemViewType() ==
	     4;
	...
	bW();
	List<ComponentKeyMapper> predictedApps = mActivity.getAppsView()
	                                         .getFloatingHeaderView()
	                                         .getPredictionRowView()
	                                         .getPredictedAppComponents();
	List<b_search> bSearches = new ArrayList<>();
	final int count = Math.min(predictedApps.size(), allAppsCols);
	for (int i = 0; i < count; i++) {
		b_search bSearch = bZ(mActivity.getAppsView().getAppsStore().getApp(
					      predictedApps.get(i).getKey()),
		                      i);
		if (bSearch != null) {
...
private void cf() {
	...
	mBubbleTextView = (BubbleTextView)mActivity.getLayoutInflater().inflate(
		R.layout.all_apps_icon, getAppsView(), false);
	final ViewGroup.LayoutParams layoutParams =
		mBubbleTextView.getLayoutParams();
	...
	mBubbleTextView.measure(View.MeasureSpec.makeMeasureSpec(
	...
	mBundle.putParcelable(
		...
		PendingIntent.getBroadcast(
			...
			new Intent().setComponent(
				new ComponentName(mActivity, LongClickReceiver.class)),
