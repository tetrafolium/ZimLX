...
public class AllAppsQsbLayout
    extends AbstractQsbLayout implements SearchUiManager, o {
  ...
  public AllAppsQsbLayout(final Context context) { this(context, null); }
  ...
  public AllAppsQsbLayout(final Context context,
    ...
    this.Dt =
        getResources().getDimensionPixelSize(R.dimen.qsb_margin_top_adjusting);
    this.Dy = getResources().getDimensionPixelSize(
        R.dimen.all_apps_search_vertical_offset);
  ...
  private void applyTheme() {
    ...
    boolean themeBlack =
        Companion.isBlack(Companion.getInstance(mContext).getCurrentFlags());
    boolean themeDark =
        Companion.isDark(Companion.getInstance(mContext).getCurrentFlags()) ||
    ...
    switch (theme) {
    case 0: // light theme
      mBackgroundColor = mContext.getResources().getColor(
          R.color.qsb_background_drawer_default);
      break;

    case 4: // dark theme
      mBackgroundColor =
          mContext.getResources().getColor(R.color.qsb_background_drawer_dark);
      break;

    case 12: // black theme
      mBackgroundColor = mContext.getResources().getColor(
          R.color.qsb_background_drawer_dark_bar);
      break;
  ...
  public void setInsets(final Rect rect) {
    ...
    if (mActivity.getDeviceProfile().isVerticalBarLayout()) {
      ...
      if (!prefs.getDockHide()) {
        ...
        delta -= mActivity.getResources().getDimensionPixelSize(
  ...
  protected final int aA(final int i) {
    if (this.mActivity.getDeviceProfile().isVerticalBarLayout()) {
      return (i - this.mAppsView.getActiveRecyclerView().getPaddingLeft()) -
          this.mAppsView.getActiveRecyclerView().getPaddingRight();
    }
  ...
  public final void
  initialize(final AllAppsContainerView allAppsContainerView) {
  ...
  public final void startSearch(final String str, final int i) {
    ...
    if (shouldUseFallbackSearch(provider)) {
      ...
      if (!Objects.requireNonNull(mActivity.getGoogleNow())
  ...
  private void ensureFallbackView() {
    ...
    mFallback =
        (FallbackAppsSearchView)this.mActivity.getLayoutInflater().inflate(
            R.layout.all_apps_google_search_fallback, this, false);
    ...
    mFallback.DI.initialize(new SearchThread(mFallback.getContext()), mFallback,
                            Launcher.getLauncher(mFallback.getContext()),
                            mFallback);
  ...
  public void draw(final Canvas canvas) {
    if (this.mShadowAlpha > 0) {
      if (this.Dv == null) {
        this.Dv = c(getResources().getDimension(
                        R.dimen.hotseat_qsb_scroll_shadow_blur_radius),
                    getResources().getDimension(
                        R.dimen.hotseat_qsb_scroll_key_shadow_offset),
                    0, true);
      }
