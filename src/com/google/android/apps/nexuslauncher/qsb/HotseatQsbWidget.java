...
public class HotseatQsbWidget extends AbstractQsbLayout
    implements o, ZimPreferences.OnPreferenceChangeListener {
  ...
  static /* synthetic */ void a(final HotseatQsbWidget hotseatQsbWidget) {
    if (hotseatQsbWidget.mIsGoogleColored !=
        hotseatQsbWidget.isGoogleColored()) {
      hotseatQsbWidget.mIsGoogleColored = !hotseatQsbWidget.mIsGoogleColored;
      hotseatQsbWidget.dM();
    }
  }

  public HotseatQsbWidget(final Context context) { this(context, null); }
  ...
  protected void onAttachedToWindow() {
    Utilities.getZimPrefs(getContext())
        .addOnPreferenceChangeListener(this, KEY_DOCK_COLORED_GOOGLE,
                                       KEY_DOCK_SEARCHBAR);
  ...
  public final int dL() { return 0; }

  private void setColors() {
    View.inflate(new ContextThemeWrapper(getContext(),
                                         mIsGoogleColored
                                             ? R.style.HotseatQsbTheme_Colored
                                             : R.style.HotseatQsbTheme),
                 R.layout.qsb_hotseat_content, this);
    /*int colorRes;
    if (isDarkBar()) {
        colorRes = R.color.qsb_background_hotseat_dark;
    } else {
        colorRes = mIsGoogleColored ? R.color.qsb_background_hotseat_white :
  ...
  public boolean isGoogleColored() {
    ...
    return wallpaperInfo != null &&
        wallpaperInfo.getComponent().flattenToString().equals(
            getContext().getString(R.string.default_live_wallpaper));
  }
      ...
      controller.getSearchProvider().startSearch(intent -> {
        ...
        getContext().startActivity(
            ...
            ActivityOptionsCompat
                .makeClipRevealAnimation(this, 0, 0, getWidth(), getHeight())
                .toBundle());
        return null;
      });
    }
  ...
  private void startGoogleSearch() {
    ...
    if (!forceFallbackSearch() &&
        mActivity.getGoogleNow().startSearch(f.build(), f.getExtras())) {
      SharedPreferences devicePrefs = Utilities.getDevicePrefs(getContext());
      devicePrefs.edit()
          .putInt("key_hotseat_qsb_tap_count",
                  devicePrefs.getInt("key_hotseat_qsb_tap_count", 0) + 1)
          .apply();
      mActivity.playQsbAnimation();
    } else {
      getContext().sendOrderedBroadcast(
          getSearchIntent(), null, new BroadcastReceiver() {
            @Override
  ...
  public final boolean dI() { return false; }
  ...
  public void setInsets(final Rect rect) {
    ...
    setVisibility(mActivity.getDeviceProfile().isVerticalBarLayout()
                      ? View.GONE
                      : View.VISIBLE);
  }
  ...
  protected final Intent createSettingsBroadcast() {
    ...
    if (provider.isBroadcast()) {
      ...
      List queryBroadcastReceivers =
          getContext().getPackageManager().queryBroadcastReceivers(intent, 0);
  ...
  public final void l(final String str) { startSearch(str, 0); }
  ...
  private Intent getSearchIntent() {
    ...
    return ConfigBuilder.getSearchIntent(rect, findViewById(R.id.g_icon),
                                         mMicIconView);
  }
  ...
  static int getBottomMargin(final Launcher launcher) {
    ...
    int minBottom = launcher.getDeviceProfile().getInsets().bottom +
                    launcher.getResources().getDimensionPixelSize(
                        R.dimen.hotseat_qsb_bottom_margin);
    ...
    float f = ((hotseatIconsTop - hotseatLayoutPadding.bottom) +
               (profile.iconSizePx * 0.92f)) /
              2.0f;
    float f2 = ((float)rect.bottom) * 0.67f;
    int bottomMargin =
        Math.round(f2 + (((((((float)hotseatTop) - f2) - f) -
                           resources.getDimension(R.dimen.qsb_widget_height)) -
                          ((float)profile.verticalDragHandleSizePx)) /
                         2.0f));
