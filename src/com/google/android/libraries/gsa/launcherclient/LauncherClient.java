...
public class LauncherClient {
  ...
  public class OverlayCallback
      extends ILauncherOverlayCallback.Stub implements Callback {
  ...
  public LauncherClient(final Activity activity,
                        final IScrollCallback scrollCallback,
                        final StaticInteger flags) {
    mActivity = activity;
    mScrollCallback = scrollCallback;
    mBaseService = new BaseClientService(activity, Context.BIND_AUTO_CREATE |
  ...
  private void reconnect() {
    if (!mDestroyed &&
        (!mLauncherService.connect() || !mBaseService.connect())) {
  ...
  private boolean isConnected() { return mOverlay != null; }
  ...
  static Intent getIntent(final Context context, final boolean proxy) {
    FeedBridge.BridgeInfo bridgeInfo =
        proxy ? FeedBridge.Companion.getInstance(context).resolveBridge()
              : null;
    String pkg = context.getPackageName();
    return new Intent("com.android.launcher3.WINDOW_OVERLAY")
        .setPackage(bridgeInfo != null
                        ? bridgeInfo.getPackageName()
  ...
  private static void loadApiVersion(final Context context) {
    ResolveInfo resolveService = context.getPackageManager().resolveService(
        getIntent(context, false), PackageManager.GET_META_DATA);
    apiVersion =
        resolveService == null || resolveService.serviceInfo.metaData == null
            ? 1
            : resolveService.serviceInfo.metaData.getInt("service.api.version",
                                                         1);
  }
}
