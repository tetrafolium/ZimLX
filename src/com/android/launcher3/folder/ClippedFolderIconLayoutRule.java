...
public class ClippedFolderIconLayoutRule {
  ...
  public void init(final int availableSpace, final float intrinsicIconSize,
                   final boolean rtl) {
  ...
  public PreviewItemDrawingParams
  computePreviewItemDrawingParams(final int index, final int curNumItems,
    ...
    if (index == EXIT_INDEX) {
      ...
      mTmpPoint[0] = mTmpPoint[1] =
          mAvailableSpace / 2 - (mIconSize * totalScale) / 2;
    } else {
   ...
   */
  private void getGridPosition(final int row, final int col,
                               final float[] result) {
  ...
  private void getPosition(final int index, final int curNumItems,
                           final float[] result) {
    // The case of two items is homomorphic to the case of one.
    curNumItems = Math.max(curNumItems, 2);

    // We model the preview as a circle of items starting in the appropriate
    // piece of the upper left quadrant (to achieve horizontal and vertical
    // symmetry).
    ...
    // We want the items to appear in reading order. For the case of 1, 2 and 3
    // items, this is natural for the circular model. With 4 items, however, we
    // need to swap the 3rd and 4th indices to achieve reading order.
    ...
    // We bump the radius up between 0 and MAX_RADIUS_DILATION % as the number
    // of items increase
    float radius =
        mRadius *
        (1 + MAX_RADIUS_DILATION * (curNumItems - MIN_NUM_ITEMS_IN_PREVIEW) /
                 (MAX_NUM_ITEMS_IN_PREVIEW - MIN_NUM_ITEMS_IN_PREVIEW));
    double theta = theta0 + index * (2 * Math.PI / curNumItems) * direction;

    float halfIconSize = (mIconSize * scaleForItem(curNumItems)) / 2;

    // Map the location along the circle, and offset the coordinates to
    // represent the center of the icon, and to be based from the top / left of
    // the preview area. The y component is inverted to match the coordinate
    // system.
    result[0] = mAvailableSpace / 2 + (float)(radius * Math.cos(theta) / 2) -
                halfIconSize;
    result[1] = mAvailableSpace / 2 + (float)(-radius * Math.sin(theta) / 2) -
                halfIconSize;
  }
