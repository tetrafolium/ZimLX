...
public class PreviewItemManager {
  ...
  // are stored as member variables for shared usage and to avoid computation on
  // each frame
  ...
  private ArrayList<PreviewItemDrawingParams> mFirstPageParams =
      new ArrayList<>();
  // These hold the current page preview items. It is empty if the current page
  // is the first page.
  private ArrayList<PreviewItemDrawingParams> mCurrentPageParams =
      new ArrayList<>();
  ...
  public PreviewItemManager(final FolderIcon icon) { mIcon = icon; }
   ...
   */
  public FolderPreviewItemAnim
  createFirstItemAnimation(final boolean reverse,
                           final Runnable onCompleteRunnable) {
    return reverse
        ? new FolderPreviewItemAnim(this, mFirstPageParams.get(0), 0, 2, -1, -1,
                                    FINAL_ITEM_ANIMATION_DURATION,
                                    onCompleteRunnable)
  ...
  private void computePreviewDrawingParams(final int drawableSize,
                                           final int totalSize) {
    if (mIntrinsicIconSize != drawableSize || mTotalWidth != totalSize ||
      ...
      mIcon.mBackground.setup(mIcon.mLauncher, mIcon, mTotalWidth,
                              mIcon.getPaddingTop());
  ...
  computePreviewItemDrawingParams(final int index, final int curNumItems,
                                  final PreviewItemDrawingParams params) {
    // We use an index of -1 to represent an icon on the workspace for the
    // destroy and create animations
    if (index == -1) {
  ...
  private PreviewItemDrawingParams
  getFinalIconParams(final PreviewItemDrawingParams params) {
  ...
  public void drawParams(final Canvas canvas,
                         final ArrayList<PreviewItemDrawingParams> params,
                         final float transX) {
  ...
  public void draw(final Canvas canvas) {
    ...
    if (mShouldSlideInFirstPage) {
      ...
      firstPageItemsTransX =
          -ITEM_SLIDE_IN_OUT_DISTANCE_PX + mCurrentPageItemsTransX;
    }
  ...
  public void onParamsChanged() { mIcon.invalidate(); }

  private void drawPreviewItem(final Canvas canvas,
                               final PreviewItemDrawingParams params) {
    canvas.save();
    ...
    if (d != null) {
      ...
      canvas.scale(mIntrinsicIconSize / bounds.width(),
                   mIntrinsicIconSize / bounds.height());
      d.draw(canvas);
      canvas.restore();
    }
    canvas.restore();
  ...
  public void hidePreviewItem(final int index, final boolean hidden) {
    ...
    // enter/exit animation purposes and they were added to the front of the
    // list. To index the params properly, we need to skip these params.
    index =
        index + Math.max(mFirstPageParams.size() - MAX_NUM_ITEMS_IN_PREVIEW, 0);

    PreviewItemDrawingParams params =
        index < mFirstPageParams.size() ? mFirstPageParams.get(index) : null;
    if (params != null) {
      params.hidden = hidden;
    }
  }

  void buildParamsForPage(final int page,
                          final ArrayList<PreviewItemDrawingParams> params,
                          final boolean animate) {
    List<BubbleTextView> items = mIcon.getPreviewItemsOnPage(page);
    int prevNumItems = params.size();

    // We adjust the size of the list to match the number of items in the
    // preview.
    ...
    int numItemsInFirstPagePreview =
        page == 0 ? items.size() : MAX_NUM_ITEMS_IN_PREVIEW;
    for (int i = 0; i < params.size(); i++) {
      PreviewItemDrawingParams p = params.get(i);
      p.drawable = items.get(i).getCompoundDrawables()[1];

      if (p.drawable != null && !mIcon.mFolder.isOpen()) {
        // Set the callback to FolderIcon as it is responsible to drawing the
        // icon. The callback will be released when the folder is opened.
      ...
      if (!animate) {
        ...
        FolderPreviewItemAnim anim = new FolderPreviewItemAnim(
            this, p, i, prevNumItems, i, numItemsInFirstPagePreview,
  ...
  void onFolderClose(final int currentPage) {
    // If we are not closing on the first page, we animate the current page
    // preview items out, and animate the first page preview items in.
    ...
    if (mShouldSlideInFirstPage) {
      ...
      ValueAnimator slideAnimator =
          ValueAnimator.ofFloat(0, ITEM_SLIDE_IN_OUT_DISTANCE_PX);
  ...
  float getIntrinsicIconSize() { return mIntrinsicIconSize; }
   ...
   */
  public void onDrop(final List<BubbleTextView> oldParams,
                     final List<BubbleTextView> newParams,
                     final ShortcutInfo dropped) {
    int numItems = newParams.size();
    final ArrayList<PreviewItemDrawingParams> params = mFirstPageParams;
    buildParamsForPage(0, params, false);

    // New preview items for items that are moving in (except for the dropped
    // item).
    ...
    for (int i = 0; i < moveIn.size(); ++i) {
      ...
      updateTransitionParam(p, moveIn.get(i), ENTER_INDEX,
                            newParams.indexOf(moveIn.get(i)), numItems);
    }
    ...
    for (int i = 0; i < moveOut.size(); ++i) {
      ...
      PreviewItemDrawingParams p =
          computePreviewItemDrawingParams(oldIndex, numItems, null);
      updateTransitionParam(p, item, oldIndex, EXIT_INDEX, numItems);
      params.add(
          0, p); // We want these items first so that they are on drawn last.
    }
  ...
  private void updateTransitionParam(final PreviewItemDrawingParams p,
                                     final BubbleTextView btv,
                                     final int prevIndex, final int newIndex,
                                     final int numItems) {
    ...
    FolderPreviewItemAnim anim =
        new FolderPreviewItemAnim(this, p, prevIndex, numItems, newIndex,
                                  numItems, DROP_IN_ANIMATION_DURATION, null);
