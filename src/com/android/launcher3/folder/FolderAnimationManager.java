 ...
 * and show the Folder in its place before starting the animation.
 */
public class FolderAnimationManager {
  ...
  public FolderAnimationManager(final Folder folder, final boolean isOpening) {
    ...
    mFolderInterpolator =
        AnimationUtils.loadInterpolator(mContext, R.anim.folder_interpolator);
    mLargeFolderPreviewItemOpenInterpolator = AnimationUtils.loadInterpolator(
        mContext, R.anim.large_folder_preview_item_open_interpolator);
    mLargeFolderPreviewItemCloseInterpolator = AnimationUtils.loadInterpolator(
        mContext, R.anim.large_folder_preview_item_close_interpolator);
  }
  ...
  public AnimatorSet getAnimator() {
    final DragLayer.LayoutParams lp =
        (DragLayer.LayoutParams)mFolder.getLayoutParams();
    ...
    float scaleRelativeToDragLayer =
        mLauncher.getDragLayer().getDescendantRectRelativeToSelf(mFolderIcon,
                                                                 folderIconPos);
    ...
    // We want to create a small X offset for the preview items, so that they
    // follow their expected path to their final locations. ie. an icon should
    // not move right, if it's final location is to its left. This value is
    // arbitrarily defined.
    int previewItemOffsetX = (int)(previewSize / 2);
    if (Utilities.isRtl(mContext.getResources())) {
      previewItemOffsetX =
          (int)(lp.width * initialScale - initialSize - previewItemOffsetX);
    }

    final int paddingOffsetX =
        (int)((mFolder.getPaddingLeft() + mContent.getPaddingLeft()) *
              initialScale);
    final int paddingOffsetY =
        (int)((mFolder.getPaddingTop() + mContent.getPaddingTop()) *
              initialScale);

    int initialX = folderIconPos.left + mPreviewBackground.getOffsetX() -
                   paddingOffsetX - previewItemOffsetX;
    int initialY =
        folderIconPos.top + mPreviewBackground.getOffsetY() - paddingOffsetY;
    final float xDistance = initialX - lp.x;
    final float yDistance = initialY - lp.y;

    // Set up the Folder background.
    final int finalColor =
        Themes.getAttrColor(mContext, android.R.attr.colorPrimary);
    final int initialColor = ColorUtils.setAlphaComponent(
    ...
    Rect startRect =
        new Rect(Math.round(totalOffsetX / initialScale),
    ...
    play(a, FolderShape.sInstance.createRevealAnimator(
                mFolder, startRect, endRect, finalRadius, !mIsOpening));

    RoundedRectRevealOutlineProvider outlineProvider =
        new RoundedRectRevealOutlineProvider(initialRadius, finalRadius,
                                             startRect, endRect) {
          @Override
    ...
    // preview item animators may use a different interpolator.
    ...
    addPreviewItemAnimators(
        a, initialScale / scaleRelativeToDragLayer,
        // Background can have a scaled radius in drag and drop mode, so we need
        // to add the difference to keep the preview items centered.
   ...
   */
  private void addPreviewItemAnimators(final AnimatorSet animatorSet,
                                       final float folderScale,
                                       final int previewItemOffsetX,
                                       final int previewItemOffsetY) {
    ClippedFolderIconLayoutRule rule = mFolderIcon.getLayoutRule();
    boolean isOnFirstPage = mFolder.mContent.getCurrentPage() == 0;
    final List<BubbleTextView> itemsInPreview =
        isOnFirstPage ? mFolderIcon.getPreviewItems()
                      : mFolderIcon.getPreviewItemsOnPage(
                            mFolder.mContent.getCurrentPage());
    final int numItemsInPreview = itemsInPreview.size();
    final int numItemsInFirstPagePreview =
        isOnFirstPage ? numItemsInPreview : MAX_NUM_ITEMS_IN_PREVIEW;

    TimeInterpolator previewItemInterpolator = getPreviewItemInterpolator();

    ShortcutAndWidgetContainer cwc =
        mContent.getPageAt(0).getShortcutsAndWidgets();
    for (int i = 0; i < numItemsInPreview; ++i) {
      final BubbleTextView btv = itemsInPreview.get(i);
      CellLayout.LayoutParams btvLp =
          (CellLayout.LayoutParams)btv.getLayoutParams();
      ...
      // preview
      rule.computePreviewItemDrawingParams(i, numItemsInFirstPagePreview,
                                           mTmpParams);
      // The PreviewLayoutRule assumes that the icon size takes up the entire
      // width so we offset by the actual size.
      int iconOffsetX =
          (int)((btvLp.width - btv.getIconSize()) * iconScale) / 2;

      final int previewPosX =
          (int)((mTmpParams.transX - iconOffsetX + previewItemOffsetX) /
                folderScale);
      final int previewPosY =
          (int)((mTmpParams.transY + previewItemOffsetY) / folderScale);

      final float xDistance = previewPosX - btvLp.x;
      final float yDistance = previewPosY - btvLp.y;

      Animator translationX =
          getAnimator(btv, View.TRANSLATION_X, xDistance, 0f);
      translationX.setInterpolator(previewItemInterpolator);
      play(animatorSet, translationX);

      Animator translationY =
          getAnimator(btv, View.TRANSLATION_Y, yDistance, 0f);
      translationY.setInterpolator(previewItemInterpolator);
      play(animatorSet, translationY);

      Animator scaleAnimator =
          getAnimator(btv, SCALE_PROPERTY, initialScale, finalScale);
      scaleAnimator.setInterpolator(previewItemInterpolator);
      play(animatorSet, scaleAnimator);

      if (mFolderIcon.isCustomIcon) {
        Animator iconScaleAnimator =
            getAnimator(mFolderIcon, ICON_SCALE_PROPERTY, 1f, 1.3f);
      ...
      if (mFolder.getItemCount() > MAX_NUM_ITEMS_IN_PREVIEW) {
        // These delays allows the preview items to move as part of the Folder's
        // motion, and its only necessary for large folders because of differing
        // interpolators.
  ...
  private void play(final AnimatorSet as, final Animator a,
                    final long startDelay, final int duration) {
  ...
  private TimeInterpolator getPreviewItemInterpolator() {
    if (mFolder.getItemCount() > MAX_NUM_ITEMS_IN_PREVIEW) {
      // With larger folders, we want the preview items to reach their final
      // positions faster (when opening) and later (when closing) so that they
      // appear aligned with the rest of the folder items when they are both
      // visible.
  ...
  private Animator getAnimator(final View view, final Property property,
                               final float v1, final float v2) {
    return mIsOpening ? ObjectAnimator.ofFloat(view, property, v1, v2)
                      : ObjectAnimator.ofFloat(view, property, v2, v1);
  }

  private Animator getAnimator(final GradientDrawable drawable,
                               final String property, final int v1,
                               final int v2) {
    return mIsOpening ? ObjectAnimator.ofArgb(drawable, property, v1, v2)
