...
public class FolderPagedView extends PagedView<PageIndicatorDots> {
  ...
  @ViewDebug.ExportedProperty(category = "launcher") private int mGridCountX;
  @ViewDebug.ExportedProperty(category = "launcher") private int mGridCountY;
  ...
  /**
   * Calculates the grid size such that {@param count} items can fit in the
   * grid. The grid size is calculated such that countY <= countX and countX =
   * ceil(sqrt(count)) while maintaining the restrictions of {@link #mMaxCountX}
   * &amp; {@link #mMaxCountY}.
   */
  public static void calculateGridSize(final int count, final int countX,
                                       final int countY, final int maxCountX,
                                       final int maxCountY,
    ...
    while (!done) {
      ...
      if (gridCountX * gridCountY < count) {
        ...
        if (gridCountY == 0)
          gridCountY++;
      } else if ((gridCountY - 1) * gridCountX >= count &&
  ...
  public void setupContentDimensions(final int count) {
    ...
    calculateGridSize(count, mGridCountX, mGridCountY, mMaxCountX, mMaxCountY,
                      mMaxItemsPerPage, sTmpArray);
   ...
   * position related attributes. It assumes that {@param item} is already
   * attached to the view.
   */
  public void addViewForRank(final View view, final ShortcutInfo item,
                             final int rank) {
  ...
  public View createNewView(final ShortcutInfo item) {
    ...
    final BubbleTextView textView =
        (BubbleTextView)mInflater.inflate(layout, null, false);
  ...
  public CellLayout getCurrentCellLayout() { return getPageAt(getNextPage()); }

  private CellLayout createAndAddNewPage() {
    DeviceProfile grid = Launcher.getLauncher(getContext()).getDeviceProfile();
    CellLayout page =
        (CellLayout)mInflater.inflate(R.layout.folder_page, this, false);
    if (mFolder.isInAppDrawer()) {
      page.setCellDimensions(grid.allAppsFolderCellWidthPx,
                             grid.allAppsFolderCellHeightPx);
    } else {
   ...
   * It essentially removes all views from all the pages and then adds them
   * again in appropriate page.
   *
   * @param list the ordered list of children.
   * @param itemCount if greater than the total children count, empty spaces are
   *     left
  ...
  private void arrangeChildren(final ArrayList<View> list, final int itemCount,
    ...
    for (int i = 0; i < itemCount; i++) {
      ...
      if (v != null) {
        ...
        if (info.cellX != newX || info.cellY != newY || info.rank != rank) {
          ...
          info.rank = rank;
          if (saveChanges) {
            mFolder.mLauncher.getModelWriter().addOrMoveItemInDatabase(
                info, mFolder.mInfo.id, 0, info.cellX, info.cellY);
          }
        }
        lp.cellX = info.cellX;
        lp.cellY = info.cellY;
    ...
    // Set the gravity as LEFT or RIGHT instead of START, as START depends on
    // the actual text.
    mFolder.mFolderName.setGravity(getPageCount() > 1
                                       ? (mIsRtl ? Gravity.RIGHT : Gravity.LEFT)
                                       : Gravity.CENTER_HORIZONTAL);
  }

  public int getDesiredWidth() {
    return getPageCount() > 0 ? (getPageAt(0).getDesiredWidth() +
                                 getPaddingLeft() + getPaddingRight())
                              : 0;
  }

  public int getDesiredHeight() {
    return getPageCount() > 0 ? (getPageAt(0).getDesiredHeight() +
  ...
  public int findNearestArea(final int pixelX, final int pixelY) {
    ...
    return Math.min(mAllocatedContentSize - 1, pageIndex * mMaxItemsPerPage +
                                                   sTmpArray[1] * mGridCountX +
                                                   sTmpArray[0]);
  }
  ...
  public View getFirstItem() {
    ...
    ShortcutAndWidgetContainer currContainer =
        getCurrentCellLayout().getShortcutsAndWidgets();
    if (mGridCountX > 0) {
  ...
  public View getLastItem() {
    ...
    if (mGridCountX > 0) {
      return currContainer.getChildAt(lastRank % mGridCountX,
                                      lastRank / mGridCountX);
    } else {
  ...
  public String getAccessibilityDescription() {
    return getContext().getString(R.string.folder_opened, mGridCountX,
                                  mGridCountY);
  }
  ...
  public void completePendingPageChanges() {
    if (!mPendingAnimations.isEmpty()) {
      ArrayMap<View, Runnable> pendingViews =
          new ArrayMap<>(mPendingAnimations);
  ...
  public int getAllocatedContentSize() { return mAllocatedContentSize; }

  /**
   * Reorders the items such that the {@param empty} spot moves to {@param
   * target}
   */
  public void realTimeReorder(final int empty, final int target) {
    ...
    if (target == empty) {
      ...
      if (pageE < pageToAnimate) {
        ...
        // Animate the 2nd item in the current page, as the first item was
        // already moved to the last page.
        startPos = 0;
      } else {
        moveStart = moveEnd = -1;
        startPos = pagePosE;
    ...
    for (int i = startPos; i != endPos; i += direction) {
      ...
      if (page.animateChildToPosition(v, i % mGridCountX, i / mGridCountX,
                                      REORDER_ANIMATION_DURATION, delay, true,
                                      true)) {
        delay += delayAmount;
  ...
  public int itemsPerPage() { return mMaxItemsPerPage; }
}
