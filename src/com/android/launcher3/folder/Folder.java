...
public class Folder extends AbstractFloatingView
    implements DragSource, View.OnLongClickListener, DropTarget, FolderListener,
               TextView.OnEditorActionListener, View.OnFocusChangeListener,
               DragListener, ExtendedEditText.OnBackKeyListener {
  ...
  public static final int RESCROLL_DELAY =
  ...
  @ViewDebug.ExportedProperty(
      ...
      mapping =
      {
        ...
        , @ViewDebug.IntToString(from = STATE_SMALL, to = "STATE_SMALL"),
            @ViewDebug.IntToString(from = STATE_ANIMATING,
                                   to = "STATE_ANIMATING"),
  ...
  @ViewDebug.ExportedProperty(category = "launcher") private boolean mDestroyed;
   ...
   *     values.
   */
  public Folder(final Context context, final AttributeSet attrs) {
    super(context, attrs);
    ...
    // We need this view to be focusable in touch mode so that when text editing
    // of the folder name is complete, we have something to focus on, thus
    // hiding the cursor and giving reliable behavior when clicking the text
    // field (since it will always gain focus on click).
  ...
  protected void onFinishInflate() {
    ...
    if (!Utilities.ATLEAST_MARSHMALLOW) {
      // We disable action mode in older OSes where floating selection menu is
      // not yet available.
    ...
    // We find out how tall footer wants to be (it is set to wrap_content), so
    // that we can allocate the appropriate amount of space for it.
  ...
  public void setClipPath(final Path path) {
    mClipPath = path;
    invalidate();
  }
  ...
  public boolean startDrag(final View v, final DragOptions options) {
    Object tag = v.getTag();
  ...
  public void onDragStart(final DropTarget.DragObject dragObject,
    ...
    if (dragObject.dragInfo instanceof ShortcutInfo) {
      ...
      // We do not want to get events for the item being removed, as they will
      // get handled when the drop completes
  ...
  public boolean isEditingName() { return mIsEditingName; }
  ...
  public boolean onBackKey() {
    // Convert to a string here to ensure that no other state associated with
    // the text field gets saved.
    String newTitle = mFolderName.getText().toString();
    mInfo.setTitle(newTitle);
    mLauncher.getModelWriter().updateItemInDatabase(mInfo);

    mFolderName.setHint(sDefaultFolderName.contentEquals(newTitle) ? sHintText
                                                                   : null);
    ...
    // selects all the text and brings up the soft keyboard if necessary.
  ...
  public FolderIcon getFolderIcon() { return mFolderIcon; }

  public void setDragController(final DragController dragController) {
    mDragController = dragController;
  }

  public void setFolderIcon(final FolderIcon icon) { mFolderIcon = icon; }

  @Override
  protected void onAttachedToWindow() {
    // requestFocus() causes the focus onto the folder itself, which doesn't
    // cause visual effect but the next arrow key can start the keyboard focus
    // inside of the folder, not the folder itself.
    requestFocus();
    super.onAttachedToWindow();
    if (mFolderIcon != null && mFolderIcon.isCustomIcon &&
        Utilities.getZimPrefs(getContext()).getFolderBgColored()) {
      // int color =
      // ColorExtractor.findDominantColorByHue(Utilities.drawableToBitmap(mFolderIcon.customIcon));
      setBackgroundTintList(
          ColorStateList.valueOf(mFolderIcon.getFolderName().getBadgeColor()));
  ...
  public View focusSearch(final int direction) {
    // When the folder is focused, further focus search should be within the
    // folder contents.
   ...
   */
  public FolderInfo getInfo() { return mInfo; }
  ...
  void bind(final FolderInfo info) {
    ...
    // In case any children didn't come across during loading, clean up the
    // folder accordingly
    mFolderIcon.post(() -> {
      if (getItemCount() <= 1) {
  ...
  static Folder fromXml(final Launcher launcher) {
    return (Folder)launcher.getLayoutInflater().inflate(
        R.layout.user_folder_icon_normalized, null);
  }
  ...
  public void animateOpen() {
    ...
    if (getParent() == null) {
      ...
      if (FeatureFlags.IS_DOGFOOD_BUILD) {
        Log.e(TAG, "Opening folder (" + this +
                       ") which already has a parent:" + getParent());
      }
    }
    ...
    // onDropCompleted(). This leads to an inconsistent state if you drag out of
    // the folder and drag back in without dropping. One resulting issue is that
    // replaceFolderWithFinalItem() can be called twice.
    ...
    if (mContent.getPageCount() > 1 &&
      ...
      int footerWidth = mContent.getDesiredWidth() - mFooter.getPaddingLeft() -
                        mFooter.getPaddingRight();

      float textWidth =
          mFolderName.getPaint().measureText(mFolderName.getText().toString());
      float translation = (footerWidth - textWidth) / 2;
      mFolderName.setTranslationX(mContent.mIsRtl ? -translation : translation);
      mPageIndicator.prepareEntryAnimation();

      // Do not update the flag if we are in drag mode. The flag will be
      // updated, when we actually drop the icon.
    ...
    // Make sure the folder picks up the last drag move even if the finger
    // doesn't move.
  ...
  public void beginExternalDrag() {
    ...
    // Since this folder opened by another controller, it might not get onDrop
    // or onDropComplete. Perform cleanup once drag-n-drop ends.
    mDragController.addDragListener(this);
  }
  ...
  protected void handleClose(final boolean animate) {
    ...
    // disappeared and no longer occludes the workspace items
  ...
  protected Pair<View, String> getAccessibilityTarget() {
    return Pair.create(
        mContent, mIsOpen ? mContent.getAccessibilityDescription()
                          : getContext().getString(R.string.folder_closed));
  }

  private void closeComplete(final boolean wasAnimated) {
  ...
  private int getTargetRank(final DragObject d, final float[] recycle) {
    recycle = d.getVisualCenter(recycle);
    return mContent.findNearestArea((int)recycle[0] - getPaddingLeft(),
  ...
  public void onDragOver(final DragObject d) {
    ...
    if (currentPage > 0 &&
        (mContent.mIsRtl ? isOutsideRightEdge : isOutsideLeftEdge)) {
  ...
  private void showScrollHint(final int direction, final DragObject d) {
  ...
  OnAlarmListener mOnExitAlarmListener = new OnAlarmListener() {
    public void onAlarm(final Alarm alarm) { completeDragExit(); }
  };
   ...
   * onDragEnter. So we need to complete all transient states based on timers.
   */
  @Override
  ...
  public boolean isDropEnabled() { return mState != STATE_ANIMATING; }
  ...
  private void centerAboutIcon() {
    ...
    if (mLauncher.getStateManager().getState().overviewUi) {
      mLauncher.getDragLayer().getDescendantRectRelativeToSelf(
          mLauncher.getOverviewPanel(), sTempRect);
    } else {
    ...
    int distFromEdgeOfScreen =
        mLauncher.getWorkspace().getPaddingLeft() + getPaddingLeft();

    if (grid.isPhone &&
        (grid.availableWidthPx - width) < 4 * distFromEdgeOfScreen) {
      // Center the folder if it is very close to being centered anyway, by
      // virtue of filling the majority of the viewport. ie. remove it from the
      // uncanny valley of centeredness.
    ...
    if (height >= sTempRect.height()) {
      ...
      // Folder height is less than page height, so bound it to the absolute
      // open folder bounds if necessary
      Rect folderBounds = grid.getAbsoluteOpenFolderBounds();
      left = Math.max(folderBounds.left,
                      Math.min(left, folderBounds.right - width));
      top = Math.max(folderBounds.top,
                     Math.min(top, folderBounds.bottom - height));
    }
    ...
    mFolderIconPivotX =
        (int)(mFolderIcon.getMeasuredWidth() * (1.0f * folderPivotX / width));
    mFolderIconPivotY =
        (int)(mFolderIcon.getMeasuredHeight() * (1.0f * folderPivotY / height));
  ...
  public float getPivotXForIconAnimation() { return mFolderIconPivotX; }
  public float getPivotYForIconAnimation() { return mFolderIconPivotY; }
  ...
  private int getFolderHeight(final int contentAreaHeight) {
  ...
  protected void onMeasure(final int widthMeasureSpec,
    ...
    if (mContent.getChildCount() > 0) {
      int cellIconGap = (mContent.getPageAt(0).getCellWidth() -
                        ...
                        2;
      mFooter.setPadding(
          mContent.getPaddingLeft() + cellIconGap, mFooter.getPaddingTop(),
          mContent.getPaddingRight() + cellIconGap, mFooter.getPaddingBottom());
    }
    mFooter.measure(
        contentAreaWidthSpec,
   ...
   */
  public void rearrangeChildren() { rearrangeChildren(-1); }
   ...
   */
  public void rearrangeChildren(final int itemCount) {
    ArrayList<View> views = getItemsInReadingOrder();
    mContent.arrangeChildren(views, Math.max(itemCount, views.size()));
    mItemsInvalidated = true;
  }

  public int getItemCount() { return mContent.getItemCount(); }
    ...
    Runnable onCompleteRunnable = new Runnable() {
      ...
      public void run() {
        ...
        if (itemCount <= 1) {
          ...
          if (itemCount == 1) {
            // Move the item from the folder to the workspace, in the position
            // of the folder
            CellLayout cellLayout =
                mLauncher.getCellLayout(mInfo.container, mInfo.screenId);
            ShortcutInfo finalItem = mInfo.contents.remove(0);
            newIcon = mLauncher.createShortcut(cellLayout, finalItem);
            mLauncher.getModelWriter().addOrMoveItemInDatabase(
                finalItem, mInfo.container, mInfo.screenId, mInfo.cellX,
                mInfo.cellY);
          }
  ...
  public void updateTextViewFocus() {
    ...
    if (firstChild != null && lastChild != null) {
      ...
      // current folder page, and hitting SHIFT+TAB from that item wraps back to
      // the folder name.
      mFolderName.setNextFocusForwardId(firstChild.getId());
      // When clicking off the folder when editing the name, this Folder gains
      // focus. When pressing an arrow key from that state, give the focus to
      // the first item.
      ...
      // When pressing shift+tab in the above state, give the focus to the last
      // item.
  ...
  public void onDrop(final DragObject d, final DragOptions options) {
    // If the icon was dropped while the page was being scrolled, we need to
    // compute the target location again such that the icon is placed of the
    // final page.
    ...
    PendingAddShortcutInfo pasi = d.dragInfo instanceof PendingAddShortcutInfo
                                      ...
                                      : null;
    ShortcutInfo pasiSi =
        pasi != null ? pasi.activityInfo.createShortcutInfo() : null;
    if (pasi != null && pasiSi == null) {
      // There is no ShortcutInfo, so we have to go through a configuration
      // activity.
      pasi.container = mInfo.id;
      pasi.rank = mEmptyCellRank;

      mLauncher.addPendingItem(pasi, pasi.container, pasi.screenId, null,
                               pasi.spanX, pasi.spanY);
      ...
      if (mIsExternalDrag) {
        ...
        // Call this before creating the view, so that ShortcutInfo is updated
        // appropriately.
        mLauncher.getModelWriter().addOrMoveItemInDatabase(si, mInfo.id, 0,
                                                           si.cellX, si.cellY);
      ...
      if (d.dragView.hasDrawn()) {
        // Temporarily reset the scale such that the animation target gets
  ...
  // added. In one case we need to create the illusion that the item isn't added
  // back to the folder yet, to to correspond to the animation of the icon back
  // into the folder. This is
  public void hideItem(final ShortcutInfo info) {
    View v = getViewForInfo(info);
    v.setVisibility(INVISIBLE);
  }
  public void showItem(final ShortcutInfo info) {
    View v = getViewForInfo(info);
  ...
  public void onAdd(final ShortcutInfo item, final int rank) {
    ...
    mLauncher.getModelWriter().addOrMoveItemInDatabase(item, mInfo.id, 0,
  ...
  public void iterateOverItems(final ItemOperator op) {
    mContent.iterateOverItems(op);
  }

  public List<BubbleTextView> getItemsOnPage(final int page) {
    ...
    int endIndex =
        Math.min(startIndex + numItemsOnCurrentPage, allItems.size());

    List<BubbleTextView> itemsOnCurrentPage =
        new ArrayList<>(numItemsOnCurrentPage);
  ...
  private class OnScrollHintListener implements OnAlarmListener {
    ...
    OnScrollHintListener(final DragObject object) { mDragObject = object; }
    ...
    public void onAlarm(final Alarm alarm) {
      ...
      mScrollPauseAlarm.setOnAlarmListener(
          new OnScrollFinishedListener(mDragObject));
  ...
  private class OnScrollFinishedListener implements OnAlarmListener {
    ...
    OnScrollFinishedListener(final DragObject object) { mDragObject = object; }
  ...
  private class SuppressInfoChanges implements AutoCloseable {
    ...
    SuppressInfoChanges() { mInfo.removeListener(Folder.this); }
  ...
  public boolean isInAppDrawer() { return mInfo.container == ItemInfo.NO_ID; }

  public static String getDefaultFolderName() { return sDefaultFolderName; }
}
