 ...
 */
public abstract class BaseItemDragListener extends InternalStateHandler
  ...
  private static final String MIME_TYPE_PREFIX =
      "com.android.launcher3.drag_and_drop/";
  public static final String EXTRA_PIN_ITEM_DRAG_LISTENER =
      "pin_item_drag_listener";
  ...
  public BaseItemDragListener(final Rect previewRect,
  ...
  public String getMimeType() { return MIME_TYPE_PREFIX + mId; }
  ...
  protected boolean onDragStart(final DragEvent event) {
    ...
    // We use drag event position as the screenPos for the preview image. Since
    // mPreviewRect already includes the view position relative to the drag
    // event on the source window, and the absolute position (position relative
    // to the screen) of drag event is same across windows, using drag position
    // here give a good estimate for relative position to source window.
    createDragHelper().startDrag(new Rect(mPreviewRect), mPreviewBitmapWidth,
                                 mPreviewViewWidth, downPos, this, options);
  ...
  public void onPreDragStart(final DragObject dragObject) {
    // The predrag starts when the workspace is not yet loaded. In some cases we
    // set the dragLayer alpha to 0 to have a nice fade-in animation. But that
    // will prevent the dragView from being visible. Instead just skip the
  ...
  @Override
  public void onPreDragEnd(final DragObject dragObject,
  ...
  protected void postCleanup() {
    clearReference();
    if (mLauncher != null) {
      // Remove any drag params from the launcher intent since the drag
