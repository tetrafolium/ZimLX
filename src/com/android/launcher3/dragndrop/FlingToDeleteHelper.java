...
public class FlingToDeleteHelper {
  ...
  public FlingToDeleteHelper(final Launcher launcher) {
    mLauncher = launcher;
  ...
  public void recordMotionEvent(final MotionEvent ev) {
  ...
  /**
   * Same as {@link #recordMotionEvent}. It creates a temporary {@link
   * MotionEvent} object using {@param event} for tracking velocity.
   */
  public void recordDragEvent(final long dragStartTime, final DragEvent event) {
    ...
    MotionEvent emulatedEvent =
        MotionEvent.obtain(dragStartTime, SystemClock.uptimeMillis(),
  ...
  public DropTarget getDropTarget() { return mDropTarget; }

  public Runnable getFlingAnimation(final DropTarget.DragObject dragObject) {
  ...
  private PointF isFlingingToDelete() {
    ...
    mVelocityTracker.computeCurrentVelocity(
        1000, config.getScaledMaximumFlingVelocity());
    ...
    if (mVelocityTracker.getYVelocity() < mFlingToDeleteThresholdVelocity) {
      ...
      theta = getAngleBetweenVectors(vel, upVec);
               ...
               mVelocityTracker.getXVelocity() <
                   mFlingToDeleteThresholdVelocity) {
      // Remove icon is on left side instead of top, so check if we are flinging
      // to the left.
  ...
  private float getAngleBetweenVectors(final PointF vec1, final PointF vec2) {
