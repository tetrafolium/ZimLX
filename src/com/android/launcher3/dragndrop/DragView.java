...
public class DragView extends View {
  ...
  ValueAnimator mAnim;
  // The intrinsic icon scale factor is the scale factor for a drag icon over
  // the workspace size.  This is ignored for non-icons.
   ...
   */
  public DragView(final Launcher launcher, final Bitmap bitmap,
                  final int registrationX, final int registrationY,
                  final float initialScale, final float scaleOnDrop,
    ...
    // Force a measure, because Workspace uses getMeasuredHeight() before the
    // layout pass
  ...
  public void setItemInfo(final ItemInfo info) {
    ...
    if (info instanceof FolderInfo &&
        ((FolderInfo)info).usingCustomIcon(mLauncher)) {
      return;
    }
    // Load the adaptive icon on a background thread and add the view in ui
    // thread.
      ...
      public void run() {
        ...
        if (dr instanceof AdaptiveIconDrawable &&
            !(info instanceof FolderInfo &&
              ((FolderInfo)info).hasCustomIcon(mLauncher))) {

          // if (dr instanceof AdaptiveIconDrawable) {
          int w = mBitmap.getWidth();
          int h = mBitmap.getHeight();
          int blurMargin = (int)mLauncher.getResources().getDimension(
                               R.dimen.blur_size_medium_outline) /
                           2;

          Rect bounds = new Rect(0, 0, w, h);
          bounds.inset(blurMargin, blurMargin);
          // Badge is applied after icon normalization so the bounds for badge
          // should not be scaled down due to icon normalization.
          ...
          Utilities.scaleRectAboutCenter(
              bounds, li.getNormalizer().getScale(dr, null, null, null));
          li.recycle();
          AdaptiveIconDrawable adaptiveIcon = (AdaptiveIconDrawable)dr;

          // Shrink very tiny bit so that the clip path is smaller than the
          // original bitmap that has anti aliased edges and shadows.
          Rect shrunkBounds = new Rect(bounds);
          Utilities.scaleRectAboutCenter(shrunkBounds, 0.98f);
          adaptiveIcon.setBounds(shrunkBounds);
          final Path mask = adaptiveIcon.getIconMask();

          mTranslateX = new SpringFloatValue(
              DragView.this, w * AdaptiveIconDrawable.getExtraInsetFraction());
          mTranslateY = new SpringFloatValue(
              DragView.this, h * AdaptiveIconDrawable.getExtraInsetFraction());

          bounds.inset((int)(-bounds.width() *
  ...
  private void updateColorFilter() {
    if (mCurrentFilter == null) {
      ...
      ColorMatrixColorFilter currentFilter =
          new ColorMatrixColorFilter(mCurrentFilter);
   ...
   * @param outObj this is set to the internal data associated with {@param
   *     info},
   *               eg {@link LauncherActivityInfo} or {@link
   * ShortcutInfoCompat}.
   */
  private Drawable getFullDrawable(final ItemInfo info,
                                   final LauncherAppState appState,
                                   final Object[] outObj) {
    if (info.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION) {
      LauncherActivityInfo activityInfo =
          LauncherAppsCompat.getInstance(mLauncher).resolveActivity(
              info.getIntent(), info.user);
      ...
      List<ShortcutInfoCompat> si =
          sm.queryForFullDetails(key.componentName.getPackageName(),
                                 Arrays.asList(key.getId()), key.user);
      if (si.isEmpty()) {
        return null;
      } else {
        outObj[0] = si.get(0);
      ...
      FolderAdaptiveIcon icon = FolderAdaptiveIcon.createFolderAdaptiveIcon(
          mLauncher, info.id,
          new Point(mBitmap.getWidth(), mBitmap.getHeight()));
      if (icon == null) {
        return null;
      }
      outObj[0] = icon;
   ...
   *drawable that can later on be rendered on top of the layers for the badges.
   *For app icons, work profile badges can only be applied. For deep shortcuts,
   *when dragged from the pop up container, there's no badge. When dragged from
   *workspace or folder, it may contain app AND/OR work profile badge
   **/

  @TargetApi(Build.VERSION_CODES.O)
  private Drawable getBadge(final ItemInfo info,
                            final LauncherAppState appState, final Object obj) {
    ...
    if (info.itemType == LauncherSettings.Favorites.ITEM_TYPE_DEEP_SHORTCUT) {
      ...
      Bitmap badge =
          li.getShortcutInfoBadge(si, appState.getIconCache()).iconBitmap;
      li.recycle();
      float badgeSize =
          mLauncher.getResources().getDimension(R.dimen.profile_badge_size);
      float insetFraction = (iconSize - badgeSize) / iconSize;
      return new InsetDrawable(new FastBitmapDrawable(badge), insetFraction,
                               insetFraction, 0, 0);
    } else if (info.itemType == LauncherSettings.Favorites.ITEM_TYPE_FOLDER) {
      return ((FolderAdaptiveIcon)obj).getBadge();
    } else {
      return mLauncher.getPackageManager().getUserBadgedIcon(
          new FixedSizeEmptyDrawable(iconSize), info.user);
    }
  }
  ...
  public float getIntrinsicIconScaleFactor() { return mIntrinsicIconScale; }

  public int getDragRegionLeft() { return mDragRegion.left; }

  public int getDragRegionTop() { return mDragRegion.top; }

  public int getDragRegionWidth() { return mDragRegion.width(); }

  public int getDragRegionHeight() { return mDragRegion.height(); }

  public void setDragVisualizeOffset(final Point p) {
    mDragVisualizeOffset = p;
  }

  public Point getDragVisualizeOffset() { return mDragVisualizeOffset; }

  public void setDragRegion(final Rect r) { mDragRegion = r; }

  public Rect getDragRegion() { return mDragRegion; }

  public Bitmap getPreviewBitmap() { return mBitmap; }
  ...
  private void animateFilterTo(final float[] targetFilter) {
    float[] oldFilter =
        mCurrentFilter == null ? new ColorMatrix().getArray() : mCurrentFilter;
    mCurrentFilter = Arrays.copyOf(oldFilter, oldFilter.length);

    if (mFilterAnimator != null) {
      mFilterAnimator.cancel();
    }
    mFilterAnimator = ValueAnimator.ofObject(
        new FloatArrayEvaluator(mCurrentFilter), oldFilter, targetFilter);
  ...
  public boolean hasDrawn() { return mHasDrawn; }
  ...
  public void show(final int touchX, final int touchY) {
    ...
    setLayoutParams(lp);
    move(touchX, touchY);
    // Post the animation to skip other expensive work happening on the first
    // frame
    post(new Runnable() {
      public void run() { mAnim.start(); }
    });
  }
  ...
  public void animateTo(final int toTouchX, final int toTouchY,
                        final Runnable onCompleteRunnable, final int duration) {
    mTempLoc[0] = toTouchX - mRegistrationX;
    mTempLoc[1] = toTouchY - mRegistrationY;
    mDragLayer.animateViewIntoPosition(
        this, mTempLoc, 1f, mScaleOnDrop, mScaleOnDrop,
  ...
  public int getBlurSizeOutline() { return mBlurSizeOutline; }

  public float getInitialScale() { return mInitialScale; }
  ...
  private static class SpringFloatValue {
    ...
    private static final FloatPropertyCompat<SpringFloatValue> VALUE =
        new FloatPropertyCompat<SpringFloatValue>("value") {
          ...
          @Override
          public void setValue(final SpringFloatValue object,
                               final float value) {
            object.mValue = value;
    ...
    public SpringFloatValue(final View view, final float range) {
      ...
      mDelta =
          view.getResources().getDisplayMetrics().density * PARALLAX_MAX_IN_DP;
    }

    public void animateToPos(final float value) {
      mSpring.animateToFinalPosition(
          Utilities.boundToRange(value, -mDelta, mDelta));
    }
  }
