...
public final class Utilities {
  ...
  // public static boolean HIDDEN_APIS_ALLOWED = !ATLEAST_P ||
  // HiddenApiCompat.tryAccess();

  public static final int SINGLE_FRAME_MS = 16;

  /**
   * Indicates if the device has a debug build. Should only be used to store
   * additional info or add extra logging and not for changing the app behavior.
   */
  public static final boolean IS_DEBUG_DEVICE =
      Build.TYPE.toLowerCase().contains("debug");
  // An intent extra to indicate the horizontal scroll of the wallpaper.
  public static final String EXTRA_WALLPAPER_OFFSET =
      "com.android.launcher3.WALLPAPER_OFFSET";
  ...
  private static final Pattern sTrimPattern = Pattern.compile(
      "^[\\s|\\p{javaSpaceChar}]*(.*)[\\s|\\p{javaSpaceChar}]*$");
   ...
   */
  public static float
  getDescendantCoordRelativeToAncestor(final View descendant,
                                       final View ancestor, final int[] coord,
  ...
  /**
   * Inverse of {@link #getDescendantCoordRelativeToAncestor(View, View, int[],
   * boolean)}.
   */
  public static void mapCoordInSelfToDescendant(final View descendant,
                                                final View root,
                                                final int[] coord) {
    sMatrix.reset();
    View v = descendant;
   ...
   * factor. This method is called while processing touch-move events to
   * determine if the event is still within the view.
   */
  public static boolean pointInView(final View v, final float localX,
                                    final float localY, final float slop) {
    return localX >= -slop && localY >= -slop &&
        localX < (v.getWidth() + slop) && localY < (v.getHeight() + slop);
  }

  public static int[] getCenterDeltaInScreenSpace(final View v0,
                                                  final View v1) {
  ...
  public static float shrinkRect(final Rect r, final float scaleX,
                                 final float scaleY) {
  ...
  /*
   * Finds a system apk which had a broadcast receiver listening to a particular
   * action.
   * @param action intent action used to find the apk
   * @return a pair of apk package name and the resources.
   */
  static Pair<String, Resources> findSystemApk(final String action,
  ...
  public static boolean isRtl(final Resources res) {
    return res.getConfiguration().getLayoutDirection() ==
        View.LAYOUT_DIRECTION_RTL;
  }

  /**
   * Returns true if the intent is a valid launch intent for a launcher activity
   * of an app. This is used to identify shortcuts which are different from the
   * ones exposed by the applications' manifest file.
   *
  ...
  public static String createDbSelectionQuery(final String columnName,
   ...
   * If value is less than lowerBound, return lowerBound; else if value is
   * greater than upperBound, return upperBound; else return value unchanged.
   */
  public static int boundToRange(final int value, final int lowerBound,
   ...
   */
  public static float boundToRange(final float value, final float lowerBound,
   ...
   * @see #boundToRange(int, int, int).
   */
  public static long boundToRange(final long value, final long lowerBound,
   ...
   */
  public static CharSequence wrapForTts(final CharSequence msg,
                                        final String ttsMsg) {
    SpannableString spanned = new SpannableString(msg);
    spanned.setSpan(new TtsSpan.TextBuilder(ttsMsg).build(), 0,
                    spanned.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
    return spanned;
  }
  ...
  public static SharedPreferences getDevicePrefs(final Context context) {
    return context.getSharedPreferences(LauncherFiles.DEVICE_PREFERENCES_KEY,
  ...
  public static boolean isWallpaperAllowed(final Context context) {
    if (ATLEAST_NOUGAT) {
      try {
        ...
        return (Boolean)wm.getClass()
            .getDeclaredMethod("isSetWallpaperAllowed")
            .invoke(wm);
      } catch (Exception e) {
      }
    }
  ...
  public static <T> T getOverrideObject(final Class<T> clazz,
    ...
    if (!TextUtils.isEmpty(className)) {
      try {
        ...
        return (T)cls.getDeclaredConstructor(Context.class)
            .newInstance(context);
  ...
  /**
   * Returns a HashSet with a single element. We use this instead of
   * Collections.singleton() because HashSet ensures all operations, such as
  ...
  /**
   * Cues a runnable to be executed after binding all launcher elements the next
   * time
   */
  ...
  public static void restartLauncher(final Context context,
    ...
    // System.exit(0) was called. We use an AlarmManager to call this intent in
    // 100ms
  ...
  public static void killLauncher() { System.exit(0); }
  ...
  public static void checkRestoreSuccess(final Context context) {
    ...
    if (prefs.getRestoreSuccess()) {
      ...
      context.startActivity(
          new Intent(context, RestoreBackupActivity.class)
  ...
  public static void requestStoragePermission(final Activity activity) {
    ActivityCompat.requestPermissions(
        ...
        ZimLauncher.REQUEST_PERMISSION_STORAGE_ACCESS);
  }
  ...
  public static int setFlag(final int flags, final int flag,
                            final boolean value) {
    if (value) {
      return flags | flag;
    } else {
   ...
   */
  public static void postAsyncCallback(final Handler handler,
   ...
   * @param threshold             the maxium dimension (either width or height)
   *     of the scaled bitmap
  ...
  getScaledDownBitmap(final Bitmap bitmap, final int threshold,
                      final boolean isNecessaryToKeepOrig) {
    if (bitmap == null)
      return null;
    ...
    if (width > height && width <= threshold) {
      // the bitmap is already smaller than our required dimension, no need to
      // resize it
      return bitmap;
    }
  ...
  private static Bitmap getResizedBitmap(final Bitmap bm, final int newWidth,
                                         final int newHeight,
                                         final boolean isNecessaryToKeepOrig) {
  ...
  /*public static Drawable getIconForTask(Context context, int userId, String
  packageName) { IconCache ic =
  LauncherAppState.getInstanceNoCreate().getIconCache(); LauncherAppsCompat lac
  = LauncherAppsCompat.getInstance(context); UserHandle user =
  UserHandle.of(userId); List<LauncherActivityInfo> al =
  lac.getActivityList(packageName, user); if (!al.isEmpty()) { Drawable
  fullResIcon = ic.getFullResIcon(al.get(0)); if (user ==
  Process.myUserHandle()) { return fullResIcon; } else { LauncherIcons li =
  LauncherIcons.obtain(context); BitmapInfo bitmapInfo =
  li.createBadgedIconBitmap(fullResIcon, user, 24); li.recycle();
  ...
  /*public static float getScrimProgress(Launcher launcher, LauncherState
  toState, float targetProgress) { if
  (Utilities.getZimPrefs(launcher).getDockGradientStyle()) return
  targetProgress; if (toState == LauncherState.OVERVIEW) { return
  OverviewState.getNormalVerticalProgress(launcher);
      }
   ...
   */
  public static float mapToRange(final float t, final float fromMin,
                                 final float fromMax, final float toMin,
                                 final float toMax,
  ...
  public static float mapRange(final float value, final float min,
                               final float max) {
    return min + (value * (max - min));
  }

  public static int parseResourceIdentifier(final Resources res,
  ...
  public static int resolveAttributeData(final Context context,
                                         final int attr) {
