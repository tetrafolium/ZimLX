...
public class LauncherStateManager {
  ...
  // We separate the state animations into "atomic" and "non-atomic" components.
  // The atomic components may be run atomically - that is, all at once, instead
  // of user-controlled. However, atomic components are not restricted to this
  // purpose; they can be user-controlled alongside non atomic components as
  // well.
  @IntDef(flag = true, value = {NON_ATOMIC_COMPONENT, ATOMIC_COMPONENT})
  ...
  public LauncherStateManager(final Launcher l) {
    mUiHandler = new Handler(Looper.getMainLooper());
    mLauncher = l;
  }

  public LauncherState getState() { return mState; }

  public LauncherState getToState() { return mToState; }
   ...
   */
  public void goToState(final LauncherState state) {
    goToState(state, !mLauncher.isForceInvisible() &&
                         mLauncher.isStarted() /* animated */);
  }

  /**
   * @see #goToState(LauncherState, boolean, Runnable)
   */
  public void goToState(final LauncherState state, final boolean animated) {
   ...
   *
   * @param animated false if the state should change immediately without any
   *     animation,
   *                 true otherwise
   * @paras onCompleteRunnable any action to perform at the end of the
   * transition, of null.
   */
  public void goToState(final LauncherState state, final boolean animated,
   ...
   */
  public void goToState(final LauncherState state, final long delay,
                        final Runnable onCompleteRunnable) {
  ...
  public void reapplyState() { reapplyState(false); }

  public void reapplyState(final boolean cancelCurrentAnimation) {
  ...
  private void goToState(final LauncherState state, final boolean animated,
                         final long delay, final Runnable onCompleteRunnable) {
    ...
    // Cancel the current animation. This will reset mState to
    // mCurrentStableState, so store it.
    ...
    // Since state NORMAL can be reached from multiple states, just assume that
    // the transition plays in reverse and use the same duration as previous
    // state.
    mConfig.duration = state == NORMAL ? fromState.transitionDuration
  ...
  /**
   * Prepares for a non-user controlled animation from fromState to toState.
   * Preparations include:
   * - Setting interpolators for various animations included in the state
   * transition.
   * - Setting some start values (e.g. scale) for views that are hidden but
   * about to be shown.
   */
  public void prepareForAtomicAnimation(final LauncherState fromState,
    ...
    if (fromState == NORMAL && toState.overviewUi) {
      ...
      // Start from a higher overview scale, but only if we're invisible so we
      // don't jump.
      ...
      builder.setInterpolator(ANIM_OVERVIEW_SCALE,
                              clampToProgress(ACCEL, 0, 0.9f));
      builder.setInterpolator(ANIM_OVERVIEW_FADE, DEACCEL_1_7);
      Workspace workspace = mLauncher.getWorkspace();

      // Start from a higher workspace scale, but only if we're invisible so we
      // don't jump.
      boolean isWorkspaceVisible = workspace.getVisibility() == VISIBLE;
      if (isWorkspaceVisible) {
        CellLayout currentChild =
            (CellLayout)workspace.getChildAt(workspace.getCurrentPage());
        isWorkspaceVisible =
            currentChild.getVisibility() == VISIBLE &&
  ...
  /**
   * Creates a {@link AnimatorPlaybackController} that can be used for a
   * controlled state transition. The UI is force-set to fromState before
   * creating the controller.
   *
   * @param fromState the initial state for the transition.
   * @param state     the final state for the transition.
   * @param duration  intended duration for normal playback. Use higher duration
   *     for better
   *                  accuracy.
   */
  public AnimatorPlaybackController
  createAnimationToNewWorkspace(final LauncherState fromState,
   ...
   */
  public AnimatorPlaybackController
  createAnimationToNewWorkspace(final LauncherState state,
                                final long duration) {
    return createAnimationToNewWorkspace(state, duration,
  ...
  public AnimatorPlaybackController
  createAnimationToNewWorkspace(final LauncherState state, final long duration,
                                final @AnimationComponents int animComponents) {
    return createAnimationToNewWorkspace(state, new AnimatorSetBuilder(),
                                         duration, null, animComponents);
  }

  public AnimatorPlaybackController createAnimationToNewWorkspace(
      final LauncherState state, final AnimatorSetBuilder builder,
      final long duration, final Runnable onCancelRunnable,
      final @AnimationComponents int animComponents) {
    mConfig.reset();
  ...
  createAnimationToNewWorkspaceInternal(final LauncherState state,
                                        final AnimatorSetBuilder builder,
                                        final Runnable onCompleteRunnable) {
  ...
  private void onStateTransitionStart(final LauncherState state) {
    if (mState != state) {
  ...
  private void onStateTransitionEnd(final LauncherState state) {
  ...
  public LauncherState getLastState() { return mLastStableState; }
  ...
  public void setRestState(final LauncherState restState) {
   ...
   */
  public void cancelAnimation() { mConfig.reset(); }

  public void setCurrentUserControlledAnimation(
      final AnimatorPlaybackController controller) {
   ...
   * starting another animation and may block some launcher interactions while
   * running.
   *
   * @param childAnimations Set of animations with the new target is
   *     controlling.
   */
  public void setCurrentAnimation(final AnimatorSet anim,
                                  final Animator... childAnimations) {
  ...
  private class StartAnimRunnable implements Runnable {
    ...
    public StartAnimRunnable(final AnimatorSet anim) { mAnim = anim; }
  ...
  public static class AnimationConfig extends AnimatorListenerAdapter {
    ...
    public PropertySetter getPropertySetter(final AnimatorSetBuilder builder) {
      if (mPropertySetter == null) {
        mPropertySetter = duration == 0
    ...
    public void onAnimationEnd(final Animator animation) {
      if (playbackController != null &&
          playbackController.getTarget() == animation) {
        playbackController = null;
      }
    ...
    public void setAnimation(final AnimatorSet animation,
