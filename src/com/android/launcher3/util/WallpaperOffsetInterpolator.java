...
public class WallpaperOffsetInterpolator extends BroadcastReceiver {
  ...
  public boolean isLockedToDefaultPage() { return mLockedToDefaultPage; }
   ...
   */
  private void wallpaperOffsetForScroll(final int scroll,
                                        final int numScrollingPages,
                                        final int[] out) {
    out[1] = 1;

    // To match the default wallpaper behavior in the system, we default to
    // either the left or right edge on initialization
    if (mLockedToDefaultPage || numScrollingPages <= 1) {
      out[0] = mIsRtl ? 1 : 0;
      return;
    }

    // Distribute the wallpaper parallax over a minimum of
    // MIN_PARALLAX_PAGE_SPAN workspace screens, not including the custom
    // screen, and empty screens (if > MIN_PARALLAX_PAGE_SPAN)
    ...
    // transition; this parameter offsets it to keep the wallpaper from
    // animating as well
    ...
    // The offset is now distributed 0..1 between the left and right pages that
    // we care about, so we just map that between the pages that we are using
    // for parallax
    int rtlOffset = 0;
    if (mIsRtl) {
  ...
  private int getNumScreensExcludingEmpty() {
    ...
    if (numScrollingPages >= MIN_PARALLAX_PAGE_SPAN &&
        mWorkspace.hasExtraEmptyScreen()) {
  ...
  public void syncWithScroll() {
    ...
    Message msg = Message.obtain(mHandler, MSG_UPDATE_OFFSET, sTempInt[0],
                                 sTempInt[1], mWindowToken);
  ...
  private void updateOffset() {
    ...
    if (mWallpaperIsLiveWallpaper) {
      ...
      numPagesForWallpaperParallax =
          Math.max(MIN_PARALLAX_PAGE_SPAN, mNumScreens);
    }
    Message
        .obtain(mHandler, MSG_SET_NUM_PARALLAX, numPagesForWallpaperParallax, 0,
                mWindowToken)
        .sendToTarget();
  }
  ...
  public void setWindowToken(final IBinder token) {
    ...
    if (mWindowToken == null && mRegistered) {
      ...
      mWorkspace.getContext().registerReceiver(
          this, new IntentFilter(Intent.ACTION_WALLPAPER_CHANGED));
  ...
  public void onReceive(final Context context, final Intent intent) {
    mWallpaperIsLiveWallpaper =
        WallpaperManager.getInstance(mWorkspace.getContext())
  ...
  private static class OffsetHandler extends Handler {
    ...
    public void handleMessage(final Message msg) {
      ...
      case MSG_APPLY_OFFSET: {
        ...
        if (mAnimating) {
          long durationSinceAnimation =
              SystemClock.uptimeMillis() - mAnimationStartTime;
        ...
        if (Float.compare(mCurrentOffset, oldOffset) != 0) {
          ...
          // Force the wallpaper offset steps to be set again, because another
          // app might have changed them
