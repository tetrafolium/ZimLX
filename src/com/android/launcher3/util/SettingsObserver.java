...
/**
 * Registers the content observer to call {@link #onSettingChanged(boolean)}
 * when any of the passed settings change. The value passed to
 * onSettingChanged() is based on the key setting.
 */
...
@Override
public void register(final String keySetting,
                     final String... dependentSettings) {
	mKeySetting = keySetting;
	mResolver.registerContentObserver(Settings.Secure.getUriFor(mKeySetting),
	                                  false, this);
	for (String setting : dependentSettings) {
		mResolver.registerContentObserver(Settings.Secure.getUriFor(setting),
		                                  false, this);
	}
	onChange(true);
}
...
public void register(final String keySetting,
                     final String... dependentSettings) {
	mKeySetting = keySetting;
	mResolver.registerContentObserver(Settings.System.getUriFor(mKeySetting),
	                                  false, this);
	for (String setting : dependentSettings) {
		mResolver.registerContentObserver(Settings.System.getUriFor(setting),
		                                  false, this);
	}
	onChange(true);
}
