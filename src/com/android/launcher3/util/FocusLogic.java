 ...
 * <p>
 * Note, this utility class calculates everything regards to icon index and its
 * (x,y) coordinates. Currently supports: <ul> <li> full matrix of cells that
 * are 1x1 <li> sparse matrix of cells that are 1x1 [ 1][  ][ 2][  ] [  ][  ][
 * 3][  ] [  ][ 4][  ][  ] [  ][ 5][ 6][ 7]
 * </ul>
 * *<p>
...
public class FocusLogic {
   ...
   */
  public static boolean shouldConsume(final int keyCode) {
  ...
  public static int handleKeyEvent(final int keyCode, final int[][] map,
                                   final int iconIdx, final int pageIndex,
                                   final int pageCount, final boolean isRtl) {
    ...
    case KeyEvent.KEYCODE_DPAD_LEFT:
      newIndex = handleDpadHorizontal(iconIdx, cntX, cntY, map,
                                      -1 /*increment*/, isRtl);
  ...
  /**
   * Returns a matrix of size (m x n) that has been initialized with {@link
   * #EMPTY}.
   *
  ...
  private static int[][] createFullMatrix(final int m, final int n) {
  ...
  /**
   * Returns a matrix of size same as the {@link CellLayout} dimension that is
   * initialized with the index of the child view.
   */
  // TODO: get rid of the dynamic matrix creation
  public static int[][] createSparseMatrix(final CellLayout layout) {
  ...
  /**
   * Creates a sparse matrix that merges the icon and hotseat view group using
   * the cell layout. The size of the returning matrix is [icon column count x
   * (icon + hotseat row count)] in portrait orientation. In landscape, [(icon +
   * hotseat) column count x (icon row count)]
   */
  // TODO: get rid of the dynamic matrix creation
  public static int[][] createSparseMatrixWithHotseat(
      final CellLayout iconLayout, final CellLayout hotseatLayout,
    ...
    for (int i = hotseatParent.getChildCount() - 1; i >= 0; i--) {
      if (isHotseatHorizontal) {
        int cx = ((CellLayout.LayoutParams)hotseatParent.getChildAt(i)
                      .getLayoutParams())
                     .cellX;
        matrix[cx][iconLayout.getCountY()] = iconParent.getChildCount() + i;
      } else {
        int cy = ((CellLayout.LayoutParams)hotseatParent.getChildAt(i)
                      .getLayoutParams())
                     .cellY;
   ...
   * column of current page. When left key is triggered on the leftmost column,
   * sparse matrix is created that combines previous page matrix and an extra
   * column on the right. Likewise, when right key is triggered on the rightmost
   * column, sparse matrix is created that combines this column on the 0th
   * column and the next page matrix.
   *
   ...
   */
  // TODO: get rid of the dynamic matrix creation
  public static int[][] createSparseMatrixWithPivotColumn(
      final CellLayout iconLayout, final int pivotX, final int pivotY) {

    ViewGroup iconParent = iconLayout.getShortcutsAndWidgets();

    int[][] matrix =
        createFullMatrix(iconLayout.getCountX() + 1, iconLayout.getCountY());
  ...
  /**
   * Calculates icon that has is closest to the horizontal axis in reference to
   * the cur icon. <p> Example of the check order for KEYCODE_DPAD_RIGHT: [  ][
   * ][13][14][15] [  ][ 6][ 8][10][12] [ X][ 1][ 2][ 3][ 4] [  ][ 5][ 7][
   * 9][11]
   */
  // TODO: add unit tests to verify all permutation.
  private static int handleDpadHorizontal(final int iconIdx, final int cntX,
    ...
    if (DEBUG) {
      Log.v(TAG, String.format(
                     "\thandleDpadHorizontal: \t[x, y]=[%d, %d] iconIndex=%d",
                     xPos, yPos, iconIdx));
    }
    ...
    for (int coeff = 1; coeff < cntY; coeff++) {
      ...
      for (; 0 <= x && x < cntX; x += increment) {
        ...
        newIconIndex =
            inspectMatrix(x, nextYPos1 + offset1, cntX, cntY, matrix);
        if (newIconIndex != NOOP) {
          return newIconIndex;
        }
        int offset2 =
            haveCrossedAllAppsColumn2 && x < cntX - 1 ? -increment : 0;
        newIconIndex =
            inspectMatrix(x, nextYPos2 + offset2, cntX, cntY, matrix);
        if (newIconIndex != NOOP) {
          return newIconIndex;
        }
      }
    ...
    // Rule3: if switching between pages, do a brute-force search to find an
    // item that was
    //        missed by rules 1 and 2 (such as when going from a bottom right
    //        icon to top left)
    if (iconIdx == PIVOT) {
      if (isRtl) {
  ...
  /**
   * Calculates icon that is closest to the vertical axis in reference to the
   * current icon. <p> Example of the check order for KEYCODE_DPAD_DOWN: [  ][
   * ][  ][ X][  ][  ][  ] [  ][  ][ 5][ 1][ 4][  ][  ] [  ][10][ 7][ 2][ 6][
   * 9][  ] [14][12][ 9][ 3][ 8][11][13]
   */
  // TODO: add unit tests to verify all permutation.
  private static int handleDpadVertical(final int iconIndex, final int cntX,
                                        final int cntY, final int[][] matrix,
                                        final int increment) {
    int newIconIndex = NOOP;
    if (matrix == null) {
    ...
    if (DEBUG) {
      Log.v(TAG, String.format(
                     "\thandleDpadVertical: \t[x, y]=[%d, %d] iconIndex=%d",
                     xPos, yPos, iconIndex));
    }

    // Rule1: check first in the dpad direction
    for (int y = yPos + increment; 0 <= y && y < cntY && 0 <= y;
         y += increment) {
    ...
    //              (xPos + 2*increment, y_(2-n))), (xPos - 2*increment,
    //              y_(2-n))
    int nextXPos1;
    int nextXPos2;
    boolean haveCrossedAllAppsColumn1 = false;
    ...
    for (int coeff = 1; coeff < cntX; coeff++) {
      ...
      for (; 0 <= y && y < cntY; y = y + increment) {
        ...
        newIconIndex =
            inspectMatrix(nextXPos1 + offset1, y, cntX, cntY, matrix);
        if (newIconIndex != NOOP) {
          return newIconIndex;
        }
        int offset2 =
            haveCrossedAllAppsColumn2 && y < cntY - 1 ? -increment : 0;
        newIconIndex =
            inspectMatrix(nextXPos2 + offset2, y, cntX, cntY, matrix);
        if (newIconIndex != NOOP) {
          return newIconIndex;
        }
      }
  ...
  private static int handleMoveHome() { return CURRENT_PAGE_FIRST_ITEM; }

  private static int handleMoveEnd() { return CURRENT_PAGE_LAST_ITEM; }

  private static int handlePageDown(final int pageIndex, final int pageCount) {
  ...
  private static int handlePageUp(final int pageIndex) {
    if (pageIndex > 0) {
  ...
  private static boolean isValid(final int xPos, final int yPos,
   ...
   */
  private static String getStringIndex(final int index) {
    switch (index) {
    case NOOP:
      return "NOOP";
  ...
  /**
   * @param edgeColumn the column of the new icon. either {@link
   *     #NEXT_PAGE_LEFT_COLUMN} or
   *                   {@link #NEXT_PAGE_RIGHT_COLUMN}
   * @return the view adjacent to {@param oldView} in the {@param nextPage} of
   *     the folder.
   */
  public static View
  getAdjacentChildInNextFolderPage(final ShortcutAndWidgetContainer nextPage,
                                   final View oldView, final int edgeColumn) {
    final int newRow =
        ((CellLayout.LayoutParams)oldView.getLayoutParams()).cellY;

    int column = (edgeColumn == NEXT_PAGE_LEFT_COLUMN) ^
                         nextPage.invertLayoutHorizontally()
                     ? 0
                     : (((CellLayout)nextPage.getParent()).getCountX() - 1);
