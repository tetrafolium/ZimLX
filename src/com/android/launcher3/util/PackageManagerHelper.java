 ...
 * workaround and doesn't guarantee that the app is on SD card.
 */
public boolean isAppOnSdcard(final String packageName,
...
public static boolean isAppEnabled(final PackageManager pm,
...
public static boolean isAppInstalled(final PackageManager pm,
                                     final String packageName,
                                     final int flags) {
	try {
		ApplicationInfo info = pm.getApplicationInfo(packageName, flags);
 ...
 */
public boolean isAppSuspended(final String packageName,
                              final UserHandle user) {
	ApplicationInfo info =
		mLauncherApps.getApplicationInfo(packageName, 0, user);
...
public Intent getAppLaunchIntent(final String pkg, final UserHandle user) {
	List<LauncherActivityInfo> activities =
		mLauncherApps.getActivityList(pkg, user);
...
public static boolean isAppSuspended(final ApplicationInfo info) {
	...
	// ApplicationInfo.FLAG_PRIVILEGED prior to N, so only check for suspended
	// flag on N or later.
...
/**
 * Returns true if {@param srcPackage} has the permission required to start
 * the activity from
 * {@param intent}. If {@param srcPackage} is null, then the activity should
 * not need any permissions
 */
public boolean hasPermissionForActivity(final Intent intent,
	...
	// On M and above also check AppOpsManager for compatibility mode
	// permissions.
	if (TextUtils.isEmpty(
		    AppOpsManager.permissionToOp(target.activityInfo.permission))) {
		// There is no app-op for this permission, which could have been disabled.
		return true;
	}

	// There is no direct way to check if the app-op is allowed for a particular
	// app. Since app-op is only enabled for apps running in compatibility mode,
	// simply block such apps.

	try {
		return mPm.getApplicationInfo(srcPackage, 0).targetSdkVersion >=
 ...
 */
public static Intent getMarketSearchIntent(final Context context,
                                           final String query) {
	try {
		Intent intent =
 ...
 */
public void startDetailsActivityForInfo(final ItemInfo info,
	...
	if (componentName != null) {
		try {
			mLauncherApps.showAppDetailsForProfile(componentName, info.user,
			                                       sourceBounds, opts);
		} catch (SecurityException | ActivityNotFoundException e) {
			Toast
			.makeText(mContext, R.string.activity_not_found, Toast.LENGTH_SHORT)
			.show();
