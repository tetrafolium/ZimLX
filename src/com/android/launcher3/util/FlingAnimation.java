...
public class FlingAnimation implements AnimatorUpdateListener, Runnable {
  ...
  public FlingAnimation(final DragObject d, final PointF vel,
                        final ButtonDropTarget dropTarget,
  ...
  public void run() {
    ...
    float xOffset =
        ((scale - 1f) * mDragObject.dragView.getMeasuredWidth()) / 2f;
    float yOffset =
        ((scale - 1f) * mDragObject.dragView.getMeasuredHeight()) / 2f;
    mFrom.left += xOffset;
    mFrom.right -= xOffset;
    mFrom.top += yOffset;
    mFrom.bottom -= yOffset;
    mDuration = Math.abs(mUY) > Math.abs(mUX) ? initFlingUpDuration()
                                              : initFlingLeftDuration();
    ...
    // NOTE: Because it takes time for the first frame of animation to actually
    // be called and we expect the animation to be a continuation of the fling,
    // we have to account for the time that has elapsed since the fling
    // finished.  And since we don't have a startDelay, we will always get call
    // to update when we call start() (which we want to ignore).
    final TimeInterpolator tInterpolator = new TimeInterpolator() {
      ...
      public float getInterpolation(final float t) {
        if (mCount < 0) {
          ...
          mOffset = Math.min(
              0.5f,
              (float)(AnimationUtils.currentAnimationTimeMillis() - startTime) /
                  duration);
          mCount++;
        }
   ...
   *   - Apply a constant force in the y direction to causing the fling to
   * decelerate.
   *   - The animation runs for the time taken by the object to go out of the
   * screen.
   *   - Calculate a constant acceleration in x direction such that the object
   * reaches
  ...
  protected int initFlingUpDuration() {
    ...
    if (d >= 0) {
      // sY can be reached under the MAX_ACCELERATION. Use MAX_ACCELERATION for
      // y direction.
      mAY = MAX_ACCELERATION;
    } else {
      // sY is not reachable, decrease the acceleration so that sY is almost
      // reached.
      d = 0;
      mAY = mUY * mUY / (2 * -sY);
    }
  ...
  /**
   * The fling animation is based on the following system
   *   - Apply a constant force in the x direction to causing the fling to
   * decelerate.
   *   - The animation runs for the time taken by the object to go out of the
   * screen.
   *   - Calculate a constant acceleration in y direction such that the object
   * reaches
   *     {@link #mIconRect} in the given time.
   */
  protected int initFlingLeftDuration() {
    ...
    if (d >= 0) {
      // sX can be reached under the MAX_ACCELERATION. Use MAX_ACCELERATION for
      // x direction.
      mAX = MAX_ACCELERATION;
    } else {
      // sX is not reachable, decrease the acceleration so that sX is almost
      // reached.
      d = 0;
      mAX = mUX * mUX / (2 * -sX);
    }
