...
public class ComponentKey {
  ...
  public ComponentKey(final ComponentName componentName,
  ...
  /**
   * Creates a new component key from an encoded component key string in the
   * form of [flattenedComponentString#userId].  If the userId is not present,
   * then it defaults to the current user.
   */
  public ComponentKey(final Context context, final String componentKeyStr) {
    ...
    if (userDelimiterIndex != -1) {
      ...
      Long componentUser =
          Long.valueOf(componentKeyStr.substring(userDelimiterIndex + 1));
      componentName = ComponentName.unflattenFromString(componentStr);
      user = UserManagerCompat.getInstance(context).getUserForSerialNumber(
          componentUser.longValue());
    } else {
