...
public class Hotseat
    extends FrameLayout implements LogContainerProvider, Insettable {
  ...
  public Hotseat(final Context context) { this(context, null); }
  ...
  public Hotseat(final Context context, final AttributeSet attrs,
    ...
    if (Utilities.getZimPrefs(context).getTransparentDock() ||
      ...
      mBackgroundColor = ColorUtils.setAlphaComponent(
          Utilities.resolveAttributeData(context, R.attr.allAppsContainerColor),
          0);
      mBackground = BlurWallpaperProvider.Companion.isEnabled(
                        BlurWallpaperProvider.BLUR_ALLAPPS)
                        ? mLauncher.getBlurWallpaperProvider().createDrawable()
                        : new ColorDrawable(mBackgroundColor);
  ...
  int getOrderInHotseat(final int x, final int y) {
    ...
    int yOrder = mHasVerticalHotseat ? x * mContent.getCountY()
                                     : y * mContent.getCountX();
  ...
  int getCellXFromOrder(final int rank) {
    int size =
        mHasVerticalHotseat ? mContent.getCountY() : mContent.getCountX();
  ...
  int getCellYFromOrder(final int rank) {
    int size =
        mHasVerticalHotseat ? mContent.getCountY() : mContent.getCountX();
    return mHasVerticalHotseat ? (mContent.getCountY() - ((rank % size) + 1))
                               : rank / size;
  }
  ...
  void resetLayout(final boolean hasVerticalHotseat) {
    ...
    if (!FeatureFlags.NO_ALL_APPS_ICON &&
        !Utilities.getZimPrefs(mLauncher.getApplicationContext())
             .getHideDockButton()) {
      // Add the Apps button
      ...
      int scaleDownPx = getResources().getDimensionPixelSize(
          R.dimen.all_apps_button_scale_down);
      Rect bounds = d.getBounds();
      d.setBounds(bounds.left, bounds.top + scaleDownPx / 2,
                  bounds.right - scaleDownPx, bounds.bottom - scaleDownPx / 2);
      allAppsButton.setCompoundDrawables(null, d, null, null);

      allAppsButton.setContentDescription(
          context.getString(R.string.all_apps_button_label));
      if (mLauncher != null) {
      ...
      // orientation of the hotseat in order regardless of which orientation
      // they were added
      int x = getCellXFromOrder(allAppsButtonRank);
      int y = getCellYFromOrder(allAppsButtonRank);
      CellLayout.LayoutParams lp = new CellLayout.LayoutParams(x, y, 1, 1);
      lp.canReorder = false;
      mContent.addViewToCellLayout(allAppsButton, -1, allAppsButton.getId(), lp,
                                   true);
    }
  }
  ...
  public boolean onInterceptTouchEvent(final MotionEvent ev) {
    ...
    // workspace is in the normal state or an accessible drag is in progress.
  ...
  public void setInsets(final Rect insets) {
    ...
    getLayout().setPadding(padding.left, padding.top, padding.right,
                           padding.bottom);
    setLayoutParams(lp);
  ...
  /*public void updateColor(ExtractedColors extractedColors, boolean animate) {
      ...
      if (!mHasVerticalHotseat) {
          ...
          if (!animate) {
              ...
              mBackgroundColorAnimator = ValueAnimator.ofInt(mBackgroundColor,
  color); mBackgroundColorAnimator.setEvaluator(new ArgbEvaluator());
              mBackgroundColorAnimator.addUpdateListener(animation ->
  ((ColorDrawable) mBackground).setColor((Integer)
  animation.getAnimatedValue())); mBackgroundColorAnimator.addListener(new
