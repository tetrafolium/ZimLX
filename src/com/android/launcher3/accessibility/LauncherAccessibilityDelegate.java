...
public class LauncherAccessibilityDelegate
    extends AccessibilityDelegate implements DragListener {
  ...
  public static final int SHORTCUTS_AND_NOTIFICATIONS =
      R.id.action_shortcuts_and_notifications;

  public enum DragType { ICON, FOLDER, WIDGET }
  ...
  protected final SparseArray<AccessibilityAction> mActions =
      new SparseArray<>();
  ...
  public LauncherAccessibilityDelegate(final Launcher launcher) {
    ...
    mActions.put(REMOVE, new AccessibilityAction(
                             REMOVE, launcher.getText(
                                         R.string.remove_drop_target_label)));
    mActions.put(
        UNINSTALL,
        new AccessibilityAction(
            UNINSTALL, launcher.getText(R.string.uninstall_drop_target_label)));
    mActions.put(
        RECONFIGURE,
        new AccessibilityAction(RECONFIGURE,
                                launcher.getText(R.string.gadget_setup_text)));
    mActions.put(ADD_TO_WORKSPACE,
                 new AccessibilityAction(
                     ADD_TO_WORKSPACE,
                     launcher.getText(R.string.action_add_to_workspace)));
    mActions.put(MOVE, new AccessibilityAction(
                           MOVE, launcher.getText(R.string.action_move)));
    mActions.put(MOVE_TO_WORKSPACE,
                 new AccessibilityAction(
                     MOVE_TO_WORKSPACE,
                     launcher.getText(R.string.action_move_to_workspace)));
    mActions.put(RESIZE, new AccessibilityAction(
                             RESIZE, launcher.getText(R.string.action_resize)));
    mActions.put(
        DEEP_SHORTCUTS,
        new AccessibilityAction(
            DEEP_SHORTCUTS, launcher.getText(R.string.action_deep_shortcut)));
    mActions.put(
  ...
  public void addAccessibilityAction(final int action, final int actionLabel) {
    mActions.put(action, new AccessibilityAction(
                             action, mLauncher.getText(actionLabel)));
  }

  @Override
  public void
  onInitializeAccessibilityNodeInfo(final View host,
  ...
  public void addSupportedActions(final View host,
    ...
    // If the request came from keyboard, do not add custom shortcuts as that is
    // already exposed as a direct shortcut
    if (!fromKeyboard && DeepShortcutManager.supportsShortcuts(item)) {
      info.addAction(
          mActions.get(NotificationListener.getInstanceIfConnected() != null
                           ? SHORTCUTS_AND_NOTIFICATIONS
                           : DEEP_SHORTCUTS));
    }

    for (ButtonDropTarget target :
         mLauncher.getDropTargetBar().getDropTargets()) {
  ...
  public boolean performAction(final View host, final ItemInfo item,
    ...
    if (action == MOVE) {
        ...
        public void run() {
          if (item instanceof AppInfo) {
            ...
            mLauncher.getModelWriter().addItemToDatabase(
                info, Favorites.CONTAINER_DESKTOP, screenId, coordinates[0],
                coordinates[1]);
            ...
            mLauncher.addPendingItem(info, Favorites.CONTAINER_DESKTOP,
                                     screenId, coordinates, info.spanX,
                                     info.spanY);
          }
      ...
      final int[] coordinates = new int[2];
      final long screenId = findSpaceOnWorkspace(item, coordinates);
      mLauncher.getModelWriter().moveItemInDatabase(
          info, LauncherSettings.Favorites.CONTAINER_DESKTOP, screenId,
          coordinates[0], coordinates[1]);

      // Bind the item in next frame so that if a new workspace page was
      // created, it will get laid out.
      ...
      for (ButtonDropTarget dropTarget :
           mLauncher.getDropTargetBar().getDropTargets()) {
  ...
  getSupportedResizeActions(final View host, final LauncherAppWidgetInfo info) {
    ArrayList<Integer> actions = new ArrayList<>();

    AppWidgetProviderInfo providerInfo =
        ((LauncherAppWidgetHostView)host).getAppWidgetInfo();
    if (providerInfo == null) {
      return actions;
    }

    CellLayout layout = (CellLayout)host.getParent().getParent();
    if ((providerInfo.resizeMode & AppWidgetProviderInfo.RESIZE_HORIZONTAL) !=
        0) {
      if (layout.isRegionVacant(info.cellX + info.spanX, info.cellY, 1,
                                info.spanY) ||
    ...
    if ((providerInfo.resizeMode & AppWidgetProviderInfo.RESIZE_VERTICAL) !=
        0) {
      if (layout.isRegionVacant(info.cellX, info.cellY + info.spanY, info.spanX,
                                1) ||
  ...
  @Thunk
  void performResizeAction(final int action, final View host,
                           final LauncherAppWidgetInfo info) {
    CellLayout.LayoutParams lp =
        (CellLayout.LayoutParams)host.getLayoutParams();
    CellLayout layout = (CellLayout)host.getParent().getParent();
    ...
    if (action == R.string.action_increase_width) {
      if (((host.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL) &&
           layout.isRegionVacant(info.cellX - 1, info.cellY, 1, info.spanY)) ||
          !layout.isRegionVacant(info.cellX + info.spanX, info.cellY, 1,
                                 info.spanY)) {
        lp.cellX--;
        info.cellX--;
      }
      ...
      if (!layout.isRegionVacant(info.cellX, info.cellY + info.spanY,
                                 info.spanX, 1)) {
        lp.cellY--;
        info.cellY--;
      }
    ...
    AppWidgetResizeFrame.getWidgetSizeRanges(mLauncher, info.spanX, info.spanY,
                                             sizeRange);
    ((LauncherAppWidgetHostView)host)
        .updateAppWidgetSize(null, sizeRange.left, sizeRange.top,
                             sizeRange.right, sizeRange.bottom);
    host.requestLayout();
    mLauncher.getModelWriter().updateItemInDatabase(info);
    announceConfirmation(
        mLauncher.getString(R.string.widget_resized, info.spanX, info.spanY));
  }
  ...
  public boolean isInAccessibleDrag() { return mDragInfo != null; }

  public DragInfo getDragInfo() { return mDragInfo; }

  /**
   * @param clickedTarget the actual view that was clicked
   * @param dropLocation relative to {@param clickedTarget}. If provided, its
   *     center is used
   * as the actual drop location otherwise the views center is used.
   */
  public void handleAccessibleDrop(final View clickedTarget,
                                   final Rect dropLocation,
    ...
    mLauncher.getDragLayer().getDescendantCoordRelativeToSelf(clickedTarget,
                                                              loc);
  ...
  public void beginAccessibleDrag(final View item, final ItemInfo info) {
    ...
    mLauncher.getDragController().prepareAccessibleDrag(pos.centerX(),
                                                        pos.centerY());
   ...
   */
  protected long findSpaceOnWorkspace(final ItemInfo info,
                                      final int[] outCoordinates) {
    Workspace workspace = mLauncher.getWorkspace();
    ...
    boolean found =
        layout.findCellForSpan(outCoordinates, info.spanX, info.spanY);
    screenIndex = 0;
