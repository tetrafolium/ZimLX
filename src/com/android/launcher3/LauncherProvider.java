...
public class LauncherProvider extends ContentProvider {
  ...
  private static final String RESTRICTION_PACKAGE_NAME =
      "workspace.configuration.package.name";

  private final ChangeListenerWrapper mListenerWrapper =
      new ChangeListenerWrapper();
   ...
   */
  @Override
  public void dump(final FileDescriptor fd, final PrintWriter writer,
                   final String[] args) {
  ...
  public boolean onCreate() {
    ...
    // process and is the first component to get created.
   ...
   */
  public void setLauncherProviderChangeListener(
      final LauncherProviderChangeListener listener) {
  ...
  protected synchronized void createDbIfNotExists() {
    if (mOpenHelper == null) {
      ...
      if (RestoreDbTask.isPending(getContext())) {
        ...
        // Set is pending to false irrespective of the result, so that it
  ...
  public Cursor query(final Uri uri, final String[] projection,
    ...
    Cursor result =
        qb.query(db, projection, args.where, args.args, null, null, sortOrder);
  ...
  @Thunk
  static long dbInsertAndCheck(final DatabaseHelper helper,
  ...
  public Uri insert(final Uri uri, final ContentValues initialValues) {
    ...
    final long rowId =
        dbInsertAndCheck(mOpenHelper, db, args.table, null, initialValues);
    if (rowId < 0)
      return null;
    ...
    if (Utilities.ATLEAST_MARSHMALLOW) {
      ...
      // Deprecated behavior to support legacy devices which rely on provider
      // callbacks.
      LauncherAppState app = LauncherAppState.getInstanceNoCreate();
      if (app != null &&
          "true".equals(uri.getQueryParameter("isExternalAdd"))) {
        app.getModel().forceReload();
      }
  ...
  private boolean initializeExternalAdd(final ContentValues values) {
    ...
    try {
      ...
      valuesInserted.put(LauncherSettings.BaseLauncherColumns._ID,
                         stmp.executeInsert());
  ...
  public int bulkInsert(final Uri uri, final ContentValues[] values) {
    ...
    try (SQLiteTransaction t = new SQLiteTransaction(db)) {
      ...
      for (int i = 0; i < numValues; i++) {
        ...
        if (dbInsertAndCheck(mOpenHelper, db, args.table, null, values[i]) <
            0) {
          return 0;
        }
      }
  ...
  @Override
  public Bundle call(final String method, final String arg,
                     final Bundle extras) {
    ...
    case LauncherSettings.Settings.METHOD_WAS_EMPTY_DB_CREATED: {
      ...
      result.putBoolean(LauncherSettings.Settings.EXTRA_VALUE,
                        Utilities.getPrefs(getContext())
                            .getBoolean(EMPTY_DATABASE_CREATED, false));
      return result;
    }
    case LauncherSettings.Settings.METHOD_DELETE_EMPTY_FOLDERS: {
      Bundle result = new Bundle();
      result.putSerializable(LauncherSettings.Settings.EXTRA_VALUE,
                             deleteEmptyFolders());
      return result;
    }
    case LauncherSettings.Settings.METHOD_NEW_ITEM_ID: {
      Bundle result = new Bundle();
      result.putLong(LauncherSettings.Settings.EXTRA_VALUE,
      ...
      if (!folderIds.isEmpty()) {
        db.delete(Favorites.TABLE_NAME,
                  Utilities.createDbSelectionQuery(
                      LauncherSettings.Favorites._ID, folderIds),
                  null);
      }
      t.commit();
  ...
  protected void notifyListeners() {
    mListenerHandler.sendEmptyMessage(
        ChangeListenerWrapper.MSG_LAUNCHER_PROVIDER_CHANGED);
  }

  @Thunk
  static void addModifiedTime(final ContentValues values) {
    values.put(LauncherSettings.ChangeLogColumns.MODIFIED,
  ...
  synchronized private void loadDefaultFavoritesIfNecessary() {
    ...
    if (sp.getBoolean(EMPTY_DATABASE_CREATED, false)) {
      ...
      AutoInstallsLayout loader =
          createWorkspaceLoaderFromAppRestriction(widgetHost);
      if (loader == null) {
      ...
      if (loader == null) {
        ...
        if (partner != null && partner.hasDefaultLayout()) {
          ...
          int workspaceResId = partnerRes.getIdentifier(
              Partner.RES_DEFAULT_LAYOUT, "xml", partner.getPackageName());
          if (workspaceResId != 0) {
            loader =
                new DefaultLayoutParser(getContext(), widgetHost, mOpenHelper,
                                        partnerRes, workspaceResId);
          }
        }
      }
      ...
      // There might be some partially restored DB items, due to buggy restore
      // logic in previous versions of launcher.
      mOpenHelper.createEmptyDB(mOpenHelper.getWritableDatabase());
      // Populate favorites table with initial favorites
      if ((mOpenHelper.loadFavorites(mOpenHelper.getWritableDatabase(),
                                     loader) <= 0) &&
  ...
  /**
   * Creates workspace loader from an XML resource listed in the app
   * restrictions.
   *
   * @return the loader if the restrictions are set and the resource exists;
   *     null otherwise.
   */
  private AutoInstallsLayout
  createWorkspaceLoaderFromAppRestriction(final AppWidgetHost widgetHost) {
    ...
    if (packageName != null) {
      try {
        Resources targetResources =
            ctx.getPackageManager().getResourcesForApplication(packageName);
  ...
  getDefaultLayoutParser(final AppWidgetHost widgetHost) {
    ...
    return new DefaultLayoutParser(getContext(), widgetHost, mOpenHelper,
                                   getContext().getResources(), defaultLayout);
  }
  ...
  public static class DatabaseHelper
      extends NoLocaleSQLiteHelper implements LayoutParserCallback {
    ...
    DatabaseHelper(final Context context,
                   final Handler widgetHostResetHandler) {
      this(context, widgetHostResetHandler, LauncherFiles.LAUNCHER_DB);
      // Table creation sometimes fails silently, which leads to a crash loop.
      // This way, we will try to create a table every time after crash, so the
      // device would eventually be able to recover.
      if (!tableExists(Favorites.TABLE_NAME) ||
          !tableExists(WorkspaceScreens.TABLE_NAME)) {
        Log.e(
            TAG,
            "Tables are missing after onCreate has been called. Trying to recreate");
    ...
    protected void initIds() {
      // In the case where neither onCreate nor onUpgrade gets called, we read
      // the maxId from the DB here
      if (mMaxItemId == -1) {
    ...
    private boolean tableExists(final String tableName) {
      Cursor c = getReadableDatabase().query(
          true, "sqlite_master", new String[] {"tbl_name"}, "tbl_name = ?",
          new String[] {tableName}, null, null, null, null, null);
      try {
        return c.getCount() > 0;
      } finally {
        c.close();
    ...
    protected void onEmptyDbCreated() {
      ...
      Utilities.getPrefs(mContext)
          .edit()
          .putBoolean(EMPTY_DATABASE_CREATED, true)
          .commit();
    }
    ...
    private void addFavoritesTable(final SQLiteDatabase db,
                                   final boolean optional) {
      Favorites.addTableToDb(db, getDefaultUserSerial(), optional);
    }

    private void addWorkspacesTable(final SQLiteDatabase db,
                                    final boolean optional) {
    ...
    private void removeOrphanedItems(final SQLiteDatabase db) {
      // Delete items directly on the workspace who's screen id doesn't exist
      //  "DELETE FROM favorites WHERE screen NOT IN (SELECT _id FROM
      //  workspaceScreens)
      //   AND container = -100"
      String removeOrphanedDesktopItems =
          "DELETE FROM " + Favorites.TABLE_NAME + " WHERE " +
      ...
      // Delete items contained in folders which no longer exist (after above
      // statement)
      //  "DELETE FROM favorites  WHERE container <> -100 AND container <> -101
      //  AND container
    ...
    /**
     * One-time data updated before support of onDowngrade was added. This
     * update is backwards compatible and can safely be run multiple times.
     * Note: No new logic should be added here after release, as the new logic
     * might not get executed on an existing device.
    ...
    public void onUpgrade(final SQLiteDatabase db, final int oldVersion,
      ...
      switch (oldVersion) {
      // The version cannot be lower that 12, as Launcher3 never supported a
      // lower version of the DB.
      case 12: {
        // With the new shrink-wrapped and re-orderable workspaces, it makes
        // sense to persist workspace screens and their relative order.
        mMaxScreenId = 0;
        addWorkspacesTable(db, false);
      }
      case 13: {
      ...
      case 18: {
        // Due to a data loss bug, some users may have items associated with
        // screen ids which no longer exist. Since this can cause other
        // problems, and since the user will never see these items anyway, we
        // use database upgrade as an opportunity to clean things up.
      ...
      case 27:
        db.execSQL("ALTER TABLE " + Favorites.TABLE_NAME + " ADD COLUMN " +
                   Favorites.TITLE_ALIAS + " TEXT;");
        db.execSQL("ALTER TABLE " + Favorites.TABLE_NAME + " ADD COLUMN " +
                   Favorites.CUSTOM_ICON + " BLOB;");
      case 29:
        db.execSQL("ALTER TABLE " + Favorites.TABLE_NAME + " ADD COLUMN " +
                   Favorites.CUSTOM_ICON_ENTRY + " TEXT;");
      case 30:
        db.execSQL("ALTER TABLE " + Favorites.TABLE_NAME + " ADD COLUMN " +
                   Favorites.SWIPE_UP_ACTION + " TEXT;");
        return;
      }
    ...
    public void onDowngrade(final SQLiteDatabase db, final int oldVersion,
      ...
      try {
        DbDowngradeHelper
            .parse(mContext.getFileStreamPath(DOWNGRADE_SCHEMA_FILE))
            .onDowngrade(db, oldVersion, newVersion);
      } catch (Exception e) {
        Log.d(TAG,
              "Unable to downgrade from: " + oldVersion + " to " + newVersion +
                  ". Wiping databse.",
              e);
        createEmptyDB(db);
      }
     ...
     */
    public void createEmptyDB(final SQLiteDatabase db) {
      try (SQLiteTransaction t = new SQLiteTransaction(db)) {
    ...
    public void removeGhostWidgets(final SQLiteDatabase db) {
      // Get all existing widget ids.
      final AppWidgetHost host = newLauncherWidgetHost();
      final int[] allWidgets;
      try {
        // Although the method was defined in O, it has existed since the
        // beginning of time, so it might work on older platforms as well.
     ...
     * have a valid launcher activity target with {@link
     * Favorites#ITEM_TYPE_APPLICATION}.
     */
    @Thunk
    void convertShortcutsToLauncherActivities(final SQLiteDatabase db) {
      try (SQLiteTransaction t = new SQLiteTransaction(db);
           // Only consider the primary user as other users can't have a
           // shortcut.
           Cursor c = db.query(Favorites.TABLE_NAME,
                               ...
                               null, null, null, null);
           SQLiteStatement updateStmt = db.compileStatement(
     ...
     */
    public boolean recreateWorkspaceTable(final SQLiteDatabase db) {
      try (SQLiteTransaction t = new SQLiteTransaction(db)) {
        final ArrayList<Long> sortedIDs;

        try (Cursor c =
                 db.query(WorkspaceScreens.TABLE_NAME,
    ...
    @Thunk
    boolean updateFolderItemsRank(final SQLiteDatabase db,
                                  final boolean addRankColumn) {
      try (SQLiteTransaction t = new SQLiteTransaction(db)) {
        if (addRankColumn) {
    ...
    private boolean addProfileColumn(final SQLiteDatabase db) {
      return addIntegerColumn(db, Favorites.PROFILE_ID, getDefaultUserSerial());
    }

    private boolean addIntegerColumn(final SQLiteDatabase db,
                                     final String columnName,
                                     final long defaultValue) {
      try (SQLiteTransaction t = new SQLiteTransaction(db)) {
        db.execSQL("ALTER TABLE favorites ADD COLUMN " + columnName +
    ...
    // this doesn't extend until after the constructor is called, and we only
    // pass a reference to LauncherProvider to LauncherApp after that point
    @Override
    ...
    @Override
    public long insertAndCheck(final SQLiteDatabase db,
                               final ContentValues values) {
      return dbInsertAndCheck(this, db, Favorites.TABLE_NAME, null, values);
    }

    public void checkId(final String table, final ContentValues values) {
    ...
    private long initializeMaxItemId(final SQLiteDatabase db) {
      return getMaxId(db, Favorites.TABLE_NAME);
    }

    // Generates a new ID to use for an workspace screen in your database. This
    // method should be only called from the main UI thread. As an exception, we
    // do call it when we call the constructor from the worker thread; however,
    // this doesn't extend until after the constructor is called, and we only
    // pass a reference to LauncherProvider to LauncherApp after that point
    ...
    private long initializeMaxScreenId(final SQLiteDatabase db) {
      return getMaxId(db, WorkspaceScreens.TABLE_NAME);
    }

    @Thunk
    int loadFavorites(final SQLiteDatabase db,
                      final AutoInstallsLayout loader) {
      ...
      for (Long id : screenIds) {
        ...
        if (dbInsertAndCheck(this, db, WorkspaceScreens.TABLE_NAME, null,
                             values) < 0) {
   ...
   */
  @Thunk
  static long getMaxId(final SQLiteDatabase db, final String table) {
  ...
  static class SqlArguments {
    ...
    SqlArguments(final Uri url, final String where, final String[] args) {
      if (url.getPathSegments().size() == 1) {
        ...
        throw new UnsupportedOperationException("WHERE clause not supported: " +
                                                url);
      } else {
        this.table = url.getPathSegments().get(0);
