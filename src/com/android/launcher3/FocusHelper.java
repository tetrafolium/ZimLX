...
public boolean onKey(final View v, final int keyCode, final KeyEvent event) {
	if (keyCode == KeyEvent.KEYCODE_DPAD_LEFT ||
		...
		// Handle the key event just like a workspace icon would in these cases.
		// In this case, it will basically act as if there is a single icon in the
		// top left (so you could think of the fullscreen page as a focusable
		// fullscreen widget).
		return FocusHelper.handleIconKeyEvent(v, keyCode, event);
	}
	return false;
 ...
 */
public static class PagedFolderKeyEventListener
...
public boolean onKey(final View v, final int keyCode, final KeyEvent e) {
	...
	if (!(v.getParent() instanceof ShortcutAndWidgetContainer)) {
		if (FeatureFlags.IS_DOGFOOD_BUILD) {
			throw new IllegalStateException(
				      "Parent of the focused item is not supported.");
		} else {
			return false;
		}
	}
	...
	final ShortcutAndWidgetContainer itemContainer =
		(ShortcutAndWidgetContainer)v.getParent();
	...
	// Process focus.
	int newIconIndex = FocusLogic.handleKeyEvent(
		keyCode, matrix, iconIndex, pageIndex, pageCount, isLayoutRtl);
	...
	case FocusLogic.NEXT_PAGE_RIGHT_COLUMN:
		...
		if (newParent != null) {
			pagedView.snapToPage(pageIndex + 1);
			child = FocusLogic.getAdjacentChildInNextFolderPage(newParent, v,
			                                                    newIconIndex);
		}
		break;
 ...
 * <p>Currently we don't special case for the phone UI in different
 * orientations, even though the hotseat is on the side in landscape mode.
 * This is to ensure that accessibility consistency is maintained across
 * rotations.
 */
static boolean handleHotseatButtonKeyEvent(final View v, final int keyCode,
                                           final KeyEvent e) {
	boolean consume = FocusLogic.shouldConsume(keyCode);
	...
	final ShortcutAndWidgetContainer hotseatParent =
		(ShortcutAndWidgetContainer)v.getParent();
	...
	int iconRank =
		((CellLayout.LayoutParams)hotseatLayout.getShortcutsAndWidgets()
		 .getChildAt(iconIndex)
		 .getLayoutParams())
		.cellX;

	final CellLayout iconLayout = (CellLayout)workspace.getChildAt(pageIndex);
	if (iconLayout == null) {
		// This check is to guard against cases where key strokes rushes in when
		// workspace child creation/deletion is still in flux. (e.g., during drop
		// or fling animation.)
		return consume;
	}
	...
	if (keyCode == KeyEvent.KEYCODE_DPAD_UP && !profile.isVerticalBarLayout()) {
		matrix = FocusLogic.createSparseMatrixWithHotseat(iconLayout,
		                                                  hotseatLayout, profile);
		...
		// For other KEYCODE_DPAD_LEFT and KEYCODE_DPAD_RIGHT navigation, do not
		// use the matrix extended with hotseat.
	...
	// Process the focus.
	int newIconIndex =
		FocusLogic.handleKeyEvent(keyCode, matrix, iconIndex, pageIndex,
 ...
 */
static boolean handleIconKeyEvent(final View v, final int keyCode,
                                  final KeyEvent e) {
	boolean consume = FocusLogic.shouldConsume(keyCode);
	if (e.getAction() == KeyEvent.ACTION_UP || !consume) {
		return consume;
	...
	ShortcutAndWidgetContainer hotseatParent =
		hotseatLayout.getShortcutsAndWidgets();
	int[][] matrix;

	// KEYCODE_DPAD_DOWN in portrait (KEYCODE_DPAD_RIGHT in landscape) is the
	// only key allowed to take a user to the hotseat. For other dpad
	// navigation, do not use the matrix extended with the hotseat.
	if (keyCode == KeyEvent.KEYCODE_DPAD_DOWN &&
	    !profile.isVerticalBarLayout()) {
		matrix = FocusLogic.createSparseMatrixWithHotseat(iconLayout,
		                                                  hotseatLayout, profile);
	...
	case FocusLogic.NEXT_PAGE_RIGHT_COLUMN:
		...
		if (parent != null) {
			...
			matrix = FocusLogic.createSparseMatrixWithPivotColumn(
				iconLayout, iconLayout.getCountX(), row);
			newIconIndex = FocusLogic.handleKeyEvent(
				keyCode, matrix, FocusLogic.PIVOT, newPageIndex, pageCount,
			...
			if (newIconIndex == FocusLogic.NEXT_PAGE_FIRST_ITEM) {
				...
				newIcon = handlePreviousPageLastItem(workspace, hotseatLayout,
				                                     pageIndex, isRtl);
			} else {
				newIcon = parent.getChildAt(newIconIndex);
			}
	...
	case FocusLogic.NEXT_PAGE_FIRST_ITEM:
		newIcon =
			handleNextPageFirstItem(workspace, hotseatLayout, pageIndex, isRtl);
		break;
	case FocusLogic.NEXT_PAGE_LEFT_COLUMN:
	case FocusLogic.PREVIOUS_PAGE_LEFT_COLUMN:
		newPageIndex = pageIndex + 1;
		...
		if (parent != null) {
			iconLayout = (CellLayout)parent.getParent();
			matrix =
				FocusLogic.createSparseMatrixWithPivotColumn(iconLayout, -1, row);
			newIconIndex = FocusLogic.handleKeyEvent(
				keyCode, matrix, FocusLogic.PIVOT, newPageIndex, pageCount,
				Utilities.isRtl(v.getResources()));
			if (newIconIndex == FocusLogic.NEXT_PAGE_FIRST_ITEM) {
	...
	case FocusLogic.CURRENT_PAGE_LAST_ITEM:
		newIcon =
			getFirstFocusableIconInReverseReadingOrder(workspaceLayout, isRtl);
		if (newIcon == null) {
			// Check the hotseat if no focusable item was found on the workspace.
			newIcon =
				getFirstFocusableIconInReverseReadingOrder(hotseatLayout, isRtl);
		}
		break;
	default:
		...
		if (0 <= newIconIndex && newIconIndex < parent.getChildCount()) {
			newIcon = parent.getChildAt(newIconIndex);
		           ...
		           newIconIndex <
		           parent.getChildCount() + hotseatParent.getChildCount()) {
			newIcon =
				hotseatParent.getChildAt(newIconIndex - parent.getChildCount());
		}
		break;
	}
	if (newIcon != null) {
 ...
 */
@Thunk
static ShortcutAndWidgetContainer
getCellLayoutChildrenForIndex(final ViewGroup container, final int i) {
...
private static View handlePreviousPageLastItem(final Workspace workspace,
                                               final CellLayout hotseatLayout,
                                               final int pageIndex,
                                               final boolean isRtl) {
	if (pageIndex - 1 < 0) {
		return null;
	}
	CellLayout workspaceLayout =
		(CellLayout)workspace.getChildAt(pageIndex - 1);
	View newIcon =
		getFirstFocusableIconInReverseReadingOrder(workspaceLayout, isRtl);
	if (newIcon == null) {
		// Check the hotseat if no focusable item was found on the workspace.
...
private static View handleNextPageFirstItem(final Workspace workspace,
                                            final CellLayout hotseatLayout,
                                            final int pageIndex,
                                            final boolean isRtl) {
...
private static View
getFirstFocusableIconInReadingOrder(final CellLayout cellLayout,
                                    final boolean isRtl) {
	View icon;
	int countX = cellLayout.getCountX();
	for (int y = 0; y < cellLayout.getCountY(); y++) {
		int increment = isRtl ? -1 : 1;
		for (int x = isRtl ? countX - 1 : 0; 0 <= x && x < countX;
		     x += increment) {
			if ((icon = cellLayout.getChildAt(x, y)) != null &&
			    icon.isFocusable()) {
				return icon;
			}
		}
...
private static View
getFirstFocusableIconInReverseReadingOrder(final CellLayout cellLayout,
                                           final boolean isRtl) {
	View icon;
	int countX = cellLayout.getCountX();
	for (int y = cellLayout.getCountY() - 1; y >= 0; y--) {
		int increment = isRtl ? 1 : -1;
		for (int x = isRtl ? 0 : countX - 1; 0 <= x && x < countX;
		     x += increment) {
			if ((icon = cellLayout.getChildAt(x, y)) != null &&
			    icon.isFocusable()) {
				return icon;
