...
/**
 * Contains parameters necessary to draw a badge for an icon (e.g. the size of
 * the badge).
 *
...
public BadgeRenderer(final @NotNull Context context, final int iconSizePx) {
	...
	mLargeIconDrawer =
		new IconDrawer(res.getDimensionPixelSize(R.dimen.badge_small_padding));
	mSmallIconDrawer =
	...
	showNotificationCount =
		Utilities.getZimPrefs(context).getNotificationCount();
}
 ...
 *
 * @param badgeInfo      Contains data to draw on the badge. Could be null if
 *     we are animating out.
 * @param iconBounds     The bounds of the icon being badged.
 * @param badgeScale     The progress of the animation, from 0 to 1.
 * @param spaceForOffset How much space is available to offset the badge up
 *     and to the right.
 */
public void draw(final Canvas canvas, final @Nullable BadgeInfo badgeInfo,
                 final Rect iconBounds, final float badgeScale,
                 final Point spaceForOffset) {
	mTextPaint.setColor(Color.WHITE);
	showNotificationCount =
		Utilities.getZimPrefs(mContext).getNotificationCount();
	...
	Shader icon =
		badgeInfo == null
	    ? null
	    : badgeInfo.getNotificationIconForBadge(mContext, backgroundColor,
		                                    mSize, iconDrawer.mPadding);
	...
	int width =
		showNotificationCount ? mSize : mSize + mCharSize * (numChars - 1);
	...
	backgroundColor =
		Utilities.getZimPrefs(mContext).getNotificationBackground();
	if (backgroundWithShadow == null) {
		backgroundWithShadow = new ShadowGenerator.Builder(backgroundColor)
		                       .setupBlurForSize(mSize)
		                       .createPill(width, mSize);
	...
	boolean isText = showNotificationCount && badgeInfo != null &&
	                 badgeInfo.getNotificationCount() != 0;
	...
	int backgroundWithShadowSize =
		backgroundWithShadow.getHeight(); // Same as width.
...
public IconDrawer(final int padding) {
	mPadding = padding;
	mCircleClipBitmap =
		Bitmap.createBitmap(mSize, mSize, Bitmap.Config.ALPHA_8);
