...
public class BadgeInfo {
  ...
  /**
   * The keys of the notifications that this badge represents. These keys can
   * later be used to retrieve {@link NotificationInfo}'s.
   */
  ...
  /**
   * This will only be initialized if the badge should display the notification
   * icon.
   */
  private NotificationInfo mNotificationInfo;

  /**
   * When retrieving the notification icon, we draw it into this shader, which
   * can be clipped as necessary when drawn in a badge.
   */
   ...
   */
  public boolean
  addOrUpdateNotificationKey(final NotificationKeyData notificationKey) {
   ...
   */
  public boolean
  removeNotificationKey(final NotificationKeyData notificationKey) {
  ...
  public int getNotificationCount() { return Math.min(mTotalCount, MAX_COUNT); }

  public void
  setNotificationToShow(final @Nullable NotificationInfo notificationInfo) {
    mNotificationInfo = notificationInfo;
    mNotificationIcon = null;
  }

  public boolean hasNotificationToShow() { return mNotificationInfo != null; }

  /**
   * Returns a shader to set on a Paint that will draw the notification icon in
   * a badge. <p> The shader is cached until {@link
   * #setNotificationToShow(NotificationInfo)} is called.
   */
  public @Nullable Shader getNotificationIconForBadge(final Context context,
    ...
    if (mNotificationIcon == null) {
      Drawable icon =
          mNotificationInfo.getIconForBackground(context, badgeColor)
              .getConstantState()
              .newDrawable();
  ...
  /**
   * Whether newBadge represents the same PackageUserKey as this badge, and
   * icons with this badge should be invalidated. So, for instance, if a badge
   * has 3 notifications and one of those notifications is updated, this method
   * should return false because the badge still says "3" and the contents of
   * those notifications are only retrieved upon long-click. This method always
   * returns true when adding or removing notifications, or if the badge has a
