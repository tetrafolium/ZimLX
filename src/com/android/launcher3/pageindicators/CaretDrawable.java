...
public class CaretDrawable extends Drawable {
  ...
  public CaretDrawable(final Context context) {
    ...
    final int strokeWidth =
        res.getDimensionPixelSize(R.dimen.all_apps_caret_stroke_width);
    final int shadowSpread =
        res.getDimensionPixelSize(R.dimen.all_apps_caret_shadow_spread);
    ...
    mShadowPaint.setAlpha(
        Themes.getAlpha(context, android.R.attr.spotShadowAlpha));
  ...
  public void draw(final Canvas canvas) {
    ...
    // When the bounds are square, this will result in a caret with a right
    // angle
    ...
    mPath.lineTo(left + (width / 2),
                 top + caretHeight * getNormalizedCaretProgress());
    mPath.lineTo(left + width,
                 top + caretHeight * (1 - getNormalizedCaretProgress()));
   ...
   * @return The progress
   */
  public float getCaretProgress() { return mCaretProgress; }

  /**
   * Sets the caret progress
   *
   * @param progress The progress ({@value #PROGRESS_CARET_POINTING_UP} for
   *     pointing up,
   *                 {@value #PROGRESS_CARET_POINTING_DOWN} for pointing down,
   * {@value #PROGRESS_CARET_NEUTRAL} for neutral)
   */
  ...
  public void setHidden(final boolean hidden) { mHidden = hidden; }
