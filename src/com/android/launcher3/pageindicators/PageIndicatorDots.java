...
public class PageIndicatorDots extends View implements PageIndicator {
   ...
   *   0.77 => Active dot has left position 0 and is collapsing towards position
   * 1 1.0  => Active dot is at position 1
   */
  ...
  public PageIndicatorDots(final Context context) { this(context, null); }
  ...
  private void animateToPosition(final float position) {
    ...
    if (mAnimator == null &&
        Float.compare(mCurrentPosition, mFinalPosition) != 0) {
      float positionForThisAnim = mCurrentPosition > mFinalPosition
                                      ? mCurrentPosition - SHIFT_PER_ANIMATION
                                      : mCurrentPosition + SHIFT_PER_ANIMATION;
      mAnimator =
          ObjectAnimator.ofFloat(this, CURRENT_POSITION, positionForThisAnim);
  ...
  public void playEntryAnimation() {
    ...
    Interpolator interpolator =
        new OvershootInterpolator(ENTER_ANIMATION_OVERSHOOT_TENSION);
    AnimatorSet animSet = new AnimatorSet();
    for (int i = 0; i < count; i++) {
      ValueAnimator anim =
          ValueAnimator.ofFloat(0, 1).setDuration(ENTER_ANIMATION_DURATION);
      final int index = i;
      anim.addUpdateListener(new AnimatorUpdateListener() {
        @Override
        public void onAnimationUpdate(final ValueAnimator animation) {
          mEntryAnimationRadiusFactors[index] =
              (Float)animation.getAnimatedValue();
          invalidate();
        }
      });
      anim.setInterpolator(interpolator);
      anim.setStartDelay(ENTER_ANIMATION_START_DELAY +
                         ENTER_ANIMATION_STAGGERED_DELAY * i);
      animSet.play(anim);
    }
  ...
  protected void onDraw(final Canvas canvas) {
    ...
    if (mEntryAnimationRadiusFactors != null) {
      ...
      canvas.drawRoundRect(getActiveRect(), mDotRadius, mDotRadius,
                           mCirclePaint);
    }
  }
  ...
  private class MyOutlineProver extends ViewOutlineProvider {
    ...
    public void getOutline(final View view, final Outline outline) {
      if (mEntryAnimationRadiusFactors == null) {
        ...
        outline.setRoundRect((int)activeRect.left, (int)activeRect.top,
                             (int)activeRect.right, (int)activeRect.bottom,
                             mDotRadius);
      }
    }
  }
