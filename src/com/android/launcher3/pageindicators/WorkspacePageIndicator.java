...
/**
 * A PageIndicator that briefly shows a fraction of a line when moving between
 * pages
 *
 * The fraction is 1 / number of pages and the position is based on the progress
 * of the page scroll.
 */
public class WorkspacePageIndicator
    extends View implements Insettable, PageIndicator {
  ...
  // The alpha that the line is being animated to or already at (either 0 or
  // mActiveAlpha).
  private int mToAlpha;
  // A float value representing the number of pages, to allow for an animation
  // when it changes.
  ...
  private static final Property<WorkspacePageIndicator, Integer> PAINT_ALPHA =
      new Property<WorkspacePageIndicator, Integer>(Integer.class,
                                                    "paint_alpha") {
        @Override
  ...
  private static final Property<WorkspacePageIndicator, Float> NUM_PAGES =
      new Property<WorkspacePageIndicator, Float>(Float.class, "num_pages") {
        ...
        @Override
        public void set(final WorkspacePageIndicator obj,
                        final Float numPages) {
  ...
  public WorkspacePageIndicator(final Context context) { this(context, null); }
  ...
  public WorkspacePageIndicator(final Context context, final AttributeSet attrs,
    ...
    updateLineHeight();
    mLineHeight = res.getDimensionPixelSize(
        R.dimen.dynamic_grid_page_indicator_line_height);

    boolean darkText =
        WallpaperColorInfo.getInstance(context).supportsDarkText();
  ...
  public void updateLineHeight() {
    boolean show =
        Utilities.getZimPrefs(getContext()).getDockShowPageIndicator();
    mLineHeight = !show ? 0
  ...
  protected void onDraw(final Canvas canvas) {
    ...
    float progress =
        Utilities.boundToRange(((float)mCurrentScroll) / mTotalScroll, 0f, 1f);
    ...
    canvas.drawRoundRect(lineLeft, getHeight() / 2 - mLineHeight / 2, lineRight,
                         getHeight() / 2 + mLineHeight / 2, mLineHeight,
                         mLineHeight, mLinePaint);
  }
  ...
  private void animateLineToAlpha(final int alpha) {
    if (alpha == mToAlpha) {
      // Ignore the new animation if it is going to the same alpha as the
      // current animation.
      return;
    }
    mToAlpha = alpha;
  ...
  /**
   * Starts the given animator and stores it in the provided index in {@link
   * #mAnimators} until the animation ends.
   *
   * If an animator is already at the index (i.e. it is already playing), it is
   * canceled and replaced with the new animator.
   */
  private void setupAndRunAnimation(final ValueAnimator animator,
