 ...
 */
public class WidgetsFullSheet
	extends BaseWidgetSheet implements Insettable, ProviderChangedListener {
...
protected void onMeasure(final int widthMeasureSpec,
	...
	measureChildWithMargins(mContent, widthMeasureSpec, widthUsed,
...
private void open(final boolean animate) {
	if (animate) {
		...
		mOpenCloseAnimator.setDuration(DEFAULT_OPEN_DURATION)
...
public boolean onControllerInterceptTouchEvent(final MotionEvent ev) {
	...
	if (ev.getAction() == MotionEvent.ACTION_DOWN) {
		...
		if (scroller.getThumbOffsetY() >= 0 &&
			...
			mNoIntercept =
				!mRecyclerView.shouldContainerScroll(ev, mLauncher.getDragLayer());
		}
	}
	return super.onControllerInterceptTouchEvent(ev);
}

public static WidgetsFullSheet show(final Launcher launcher,
                                    final boolean animate) {
	WidgetsFullSheet sheet =
		(WidgetsFullSheet)launcher.getLayoutInflater().inflate(
