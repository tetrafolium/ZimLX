...
public PendingAppWidgetHostView(final Context context,
                                final LauncherAppWidgetInfo info,
                                final IconCache cache,
                                final boolean disabledForSafeMode) {
	...
	mPaint.setTextSize(TypedValue.applyDimension(
				   TypedValue.COMPLEX_UNIT_PX, mLauncher.getDeviceProfile().iconTextSizePx,
	...
	if (info.pendingItemInfo == null) {
		info.pendingItemInfo =
			new PackageItemInfo(info.providerName.getPackageName());
...
@Override
public void updateAppWidgetSize(final Bundle newOptions, final int minWidth,
                                final int minHeight, final int maxWidth,
                                final int maxHeight) {
	// No-op
}
...
protected View getDefaultView() {
	if (mDefaultView == null) {
		mDefaultView =
			mInflater.inflate(R.layout.appwidget_not_ready, this, false);
	...
	if (info.iconBitmap != null) {
		...
		if (mDisabledForSafeMode) {
			...
			mSettingIconDrawable =
				getResources().getDrawable(R.drawable.ic_setting).mutate();
...
private void updateSettingColor(final int dominantColor) {
	...
	hsv[2] = 1;
	mSettingIconDrawable.setColorFilter(Color.HSVToColor(hsv),
...
public void onClick(final View v) {
	// AppWidgetHostView blocks all click events on the root view. Instead
	// handle click events on the content and pass it along.
 ...
 * probably because the launcher doesn't have appropriate permissions. Note
 * that we would still have an allocated id as that does not require any
 * permissions and can be done during view inflation. 2) UI is not ready: the
 * id is valid and the bound. But the widget has a configure activity which
 * needs to be called once.
 */
public boolean isReadyForClickSetup() {
	return !mInfo.hasRestoreFlag(
...
private void updateDrawableBounds() {
	...
	int minPadding = getResources().getDimensionPixelSize(
		R.dimen.pending_widget_min_padding);

	int availableWidth =
		getWidth() - paddingLeft - paddingRight - 2 * minPadding;
	int availableHeight =
	...
	if (mSettingIconDrawable == null) {
		...
		mRect.offsetTo((getWidth() - mRect.width()) / 2,
		               (getHeight() - mRect.height()) / 2);
		mCenterDrawable.setBounds(mRect);
	} else {
		float iconSize = Math.max(0, Math.min(availableWidth, availableHeight));

		// Use twice the setting size factor, as the setting is drawn at a corner
		// and the icon is drawn in the center.
		...
		if (availableWidth > 0) {
			...
			mSetupTextLayout = new StaticLayout(
				getResources().getText(R.string.gadget_setup_text), mPaint,
			...
			float minHeightWithText = textHeight +
			                          actualIconSize * settingIconScaleFactor +
			                          grid.iconDrawablePaddingPx;

			if (minHeightWithText < availableHeight) {
				// We can draw the text as well
				iconTop = (getHeight() - textHeight - grid.iconDrawablePaddingPx -
				           actualIconSize) /
				          2;
		...
		if (mSetupTextLayout != null) {
			...
			mRect.left = paddingLeft + minPadding;
			mRect.top =
				mCenterDrawable.getBounds().bottom + grid.iconDrawablePaddingPx;
		}
	}
}
