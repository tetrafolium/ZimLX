...
public WidgetsDiffReporter(final IconCache iconCache,
                           final RecyclerView.Adapter listener) {
	mIconCache = iconCache;
	mListener = listener;
}

public void process(final ArrayList<WidgetListRowEntry> currentEntries,
                    final ArrayList<WidgetListRowEntry> newEntries,
	...
	if (currentEntries.isEmpty() || newEntries.isEmpty()) {
		...
		// On rotation, we open the widget tray with empty. Then try to fetch the
		// list again when the animation completes (which still gives empty). And
		// we get the final result when the bind actually completes.
	...
	do {
		...
		if (DEBUG) {
			Log.d(TAG, String.format(
				      "diff=%d orgRowEntry (%s) newRowEntry (%s)", diff,
				      orgRowEntry != null ? orgRowEntry.toString() : null,
...
/**
 * Compare package name using the same comparator as in {@link
 * WidgetsListAdapter}. Also handle null row pointers.
 */
private int
comparePackageName(final WidgetListRowEntry curRow,
                   final WidgetListRowEntry newRow,
                   final WidgetListRowEntryComparator comparator) {
...
private boolean isSamePackageItemInfo(final PackageItemInfo curInfo,
                                      final PackageItemInfo newInfo) {
