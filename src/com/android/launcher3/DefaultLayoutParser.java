...
/**
 * Implements the layout parser with rules for internal layouts and partner
 * layouts.
 */
public class DefaultLayoutParser extends AutoInstallsLayout {
  ...
  // TODO: Remove support for this broadcast, instead use widget options to send
  // bind time options
  private static final String ACTION_APPWIDGET_DEFAULT_WORKSPACE_CONFIGURE =
      "com.android.launcher.action.APPWIDGET_DEFAULT_WORKSPACE_CONFIGURE";

  public DefaultLayoutParser(final Context context,
                             final AppWidgetHost appWidgetHost,
                             final LayoutParserCallback callback,
                             final Resources sourceRes, final int layoutId) {
  ...
  @Thunk
  ArrayMap<String, TagParser> getFolderElementsMap(final Resources res) {
  ...
  public class AppShortcutWithUriParser extends AppShortcutParser {
    ...
    protected long invalidPackageOrClass(final XmlResourceParser parser) {
      ...
      ResolveInfo resolved = mPackageManager.resolveActivity(
          metaIntent, PackageManager.MATCH_DEFAULT_ONLY);
      final List<ResolveInfo> appList = mPackageManager.queryIntentActivities(
          metaIntent, PackageManager.MATCH_DEFAULT_ONLY);

      // Verify that the result is an app and not just the resolver dialog
      // asking which app to use.
      if (wouldLaunchResolverActivity(resolved, appList)) {
        // If only one of the results is a system app then choose that as the
        // default.
        final ResolveInfo systemApp = getSingleSystemActivity(appList);
        if (systemApp == null) {
          // There is no logical choice for this meta-favorite, so rather than
          // making a bad choice just add nothing.
      ...
      final Intent intent =
          mPackageManager.getLaunchIntentForPackage(info.packageName);
      if (intent == null) {
        return -1;
      }
    ...
    private ResolveInfo
    getSingleSystemActivity(final List<ResolveInfo> appList) {
    ...
    private boolean
    wouldLaunchResolverActivity(final ResolveInfo resolved,
      ...
      for (int i = 0; i < appList.size(); ++i) {
        ...
        if (tmp.activityInfo.name.equals(resolved.activityInfo.name) &&
            tmp.activityInfo.packageName.equals(
  ...
  public class UriShortcutParser extends ShortcutParser {
    ...
    public UriShortcutParser(final Resources iconRes) { super(iconRes); }
  ...
  class PartnerFolderParser implements TagParser {
    ...
    public long parseAndAdd(final XmlResourceParser parser)
      ...
      if (partner != null) {
        ...
        final int resId = partnerRes.getIdentifier(Partner.RES_FOLDER, "xml",
        ...
        if (resId != 0) {
          ...
          FolderParser folderParser =
              new FolderParser(getFolderElementsMap(partnerRes));
  ...
  /**
   * An extension of FolderParser which allows adding items from a different
   * xml.
   */
  @Thunk
  ...
  /**
   * AppWidget parser which enforces that the app is already installed when the
   * layout is parsed.
   */
  protected class AppWidgetParser extends PendingWidgetParser {
    ...
    protected long verifyAndInsert(final ComponentName cn,
      ...
      try {
        ...
        if (!extras.isEmpty()) {
          Intent intent =
              new Intent(ACTION_APPWIDGET_DEFAULT_WORKSPACE_CONFIGURE);
