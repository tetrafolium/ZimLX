...
/**
 * Contains logic relevant to populating a {@link PopupContainerWithArrow}. In
 * particular, this class determines which items appear in the container, and in
 * what order.
 */
public class PopupPopulator {
  ...
  /**
   * Sorts shortcuts in rank order, with manifest shortcuts coming before
   * dynamic shortcuts.
   */
  private static final Comparator<ShortcutInfoCompat> SHORTCUT_RANK_COMPARATOR =
      new Comparator<ShortcutInfoCompat>() {
        @Override
        public int compare(final ShortcutInfoCompat a,
  ...
  /**
   * Filters the shortcuts so that only MAX_SHORTCUTS or fewer shortcuts are
   * retained. We want the filter to include both static and dynamic shortcuts,
   * so we always include NUM_DYNAMIC dynamic shortcuts, if at least that many
   * are present.
   *
   * @param shortcutIdToRemoveFirst An id that should be filtered out first, if
   *     any.
   * @return a subset of shortcuts, in sorted order, with size <= MAX_SHORTCUTS.
   */
  public static List<ShortcutInfoCompat>
  sortAndFilterShortcuts(final List<ShortcutInfoCompat> shortcuts,
                         final @Nullable String shortcutIdToRemoveFirst) {
    // Remove up to one specific shortcut before sorting and doing somewhat
    // fancy filtering.
    ...
    // The list of shortcuts is now sorted with static shortcuts followed by
    // dynamic shortcuts. We want to preserve this order, but only keep
    // MAX_SHORTCUTS.
    ...
    for (int i = 0; i < size; i++) {
      ...
      // At this point, we have MAX_SHORTCUTS already, but they may all be
      // static. If there are dynamic shortcuts, remove static shortcuts to add
      // them.
    ...
    return () -> {
      if (!notificationKeys.isEmpty()) {
        List<StatusBarNotification> notifications =
            launcher.getPopupDataProvider().getStatusBarNotificationsForKeys(
                notificationKeys);
      ...
      String shortcutIdToDeDupe = notificationKeys.isEmpty()
                                      ? null
                                      : notificationKeys.get(0).shortcutId;
      shortcuts =
          PopupPopulator.sortAndFilterShortcuts(shortcuts, shortcutIdToDeDupe);
