...
public abstract class ArrowPopup extends AbstractFloatingView {
  ...
  public ArrowPopup(final Context context, final AttributeSet attrs,
    ...
    final int arrowWidth =
        resources.getDimensionPixelSize(R.dimen.popup_arrow_width);
    final int arrowHeight =
        resources.getDimensionPixelSize(R.dimen.popup_arrow_height);
  ...
  public ArrowPopup(final Context context) { this(context, null, 0); }
  ...
  public <T extends View> T inflateAndAdd(final int resId,
   ...
   *
   * @param viewsToFlip number of views from the top to to flip in case of
   *     reverse order
   */
  protected void reorderAndShow(final int viewsToFlip) {
    ...
    final int arrowCenterOffset = res.getDimensionPixelSize(
        isAlignedWithStart() ? R.dimen.popup_arrow_horizontal_center_start
                             : R.dimen.popup_arrow_horizontal_center_end);
    final int halfArrowWidth =
        res.getDimensionPixelSize(R.dimen.popup_arrow_width) / 2;
    mLauncher.getDragLayer().addView(mArrow);
    DragLayer.LayoutParams arrowLp =
        (DragLayer.LayoutParams)mArrow.getLayoutParams();
    if (mIsLeftAligned) {
      mArrow.setX(getX() + arrowCenterOffset - halfArrowWidth);
    } else {
      mArrow.setX(getX() + getMeasuredWidth() - arrowCenterOffset -
                  halfArrowWidth);
    }

    if (Gravity.isVertical(mGravity)) {
      // This is only true if there wasn't room for the container next to the
      // icon, so we centered it instead. In that case we don't want to
      // showDefaultOptions the arrow.
      mArrow.setVisibility(INVISIBLE);
    } else {
      ShapeDrawable arrowDrawable = new ShapeDrawable(
          TriangleShape.create(arrowLp.width, arrowLp.height, !mIsAboveIcon));
      Paint arrowPaint = arrowDrawable.getPaint();
      arrowPaint.setColor(
          Themes.getAttrColor(mLauncher, R.attr.popupColorPrimary));
      // The corner path effect won't be reflected in the shadow, but shouldn't
      // be noticeable.
      int radius = getResources().getDimensionPixelSize(
          R.dimen.popup_arrow_corner_radius);
  ...
  /**
   * Orients this container above or below the given icon, aligning with the
   * left or right. <p> These are the preferred orientations, in order (RTL
   * prefers right-aligned over left):
  ...
  protected void orientAboutObject() {
    ...
    int extraVerticalSpace =
        mArrow.getLayoutParams().height + mArrayOffset +
    ...
    boolean canBeRightAligned =
        rightAlignedX > dragLayer.getLeft() + insets.left;
    ...
    if (y + height > dragLayer.getBottom() - insets.bottom) {
      // The container is opening off the screen, so just center it in the drag
      // layer instead.
      mGravity = Gravity.CENTER_VERTICAL;
      // Put the container next to the icon, preferring the right side in ltr
      // (left in rtl).
    ...
    if (mIsAboveIcon) {
      ...
      lp.bottomMargin = mLauncher.getDragLayer().getHeight() - y -
                        getMeasuredHeight() - insets.top;
      arrowLp.bottomMargin =
          lp.bottomMargin - arrowLp.height - mArrayOffset - insets.bottom;
    } else {
      arrowLp.gravity = lp.gravity = Gravity.TOP;
      lp.topMargin = y + insets.top;
      arrowLp.topMargin =
          lp.topMargin - insets.top - arrowLp.height - mArrayOffset;
    }
  }
  ...
  private void animateOpen() {
    ...
    final long revealDuration =
        (long)res.getInteger(R.integer.config_popupOpenCloseDuration);
    final TimeInterpolator revealInterpolator =
        new AccelerateDecelerateInterpolator();

    // Rectangular reveal.
    final ValueAnimator revealAnim =
        createOpenCloseOutlineProvider().createRevealAnimator(this, false);
    ...
    mArrow.setScaleY(0);
    Animator arrowScale =
        ObjectAnimator.ofFloat(mArrow, LauncherAnimUtils.SCALE_PROPERTY, 1)
            .setDuration(
                res.getInteger(R.integer.config_popupArrowOpenDuration));
  ...
  protected void animateClose() {
    ...
    // Hide the arrow
    closeAnim.play(
        ObjectAnimator.ofFloat(mArrow, LauncherAnimUtils.SCALE_PROPERTY, 0));
    ...
    final ValueAnimator revealAnim =
        createOpenCloseOutlineProvider().createRevealAnimator(this, true);
    revealAnim.setInterpolator(revealInterpolator);
  ...
  private RoundedRectRevealOutlineProvider createOpenCloseOutlineProvider() {
    int arrowCenterX = getResources().getDimensionPixelSize(
        mIsLeftAligned ^ mIsRtl ? R.dimen.popup_arrow_horizontal_center_start
                                : R.dimen.popup_arrow_horizontal_center_end);
    if (!mIsLeftAligned) {
    ...
    return new RoundedRectRevealOutlineProvider(mOutlineRadius, mOutlineRadius,
                                                mStartRect, mEndRect);
  }
