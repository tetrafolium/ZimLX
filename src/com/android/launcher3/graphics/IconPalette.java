...
private IconPalette(final int color, final boolean desaturateBackground) {
	dominantColor = color;
	backgroundColor = desaturateBackground ? getMutedColor(dominantColor, 0.87f)
	                                   : dominantColor;
	ColorMatrix backgroundColorMatrix = new ColorMatrix();
	Themes.setColorScaleOnMatrix(backgroundColor, backgroundColorMatrix);
	backgroundColorMatrixFilter =
		new ColorMatrixColorFilter(backgroundColorMatrix);
	if (!desaturateBackground) {
		...
		Themes.setColorScaleOnMatrix(getMutedColor(dominantColor, 0.54f),
		                             backgroundColorMatrix);
		saturatedBackgroundColorMatrixFilter =
			new ColorMatrixColorFilter(backgroundColorMatrix);
	}
...
public static IconPalette
fromDominantColor(final int dominantColor,
                  final boolean desaturateBackground) {
 ...
 */
public static int getPreloadProgressColor(final Context context,
                                          final int dominantColor) {
	int result = dominantColor;

	// Make sure that the dominant color has enough saturation to be visible
	// properly.
 ...
 *
 * @return a color of the same hue with enough contrast against the
 *     background.
 *
 * This was copied from com.android.internal.util.NotificationColorUtil.
 */
public static int resolveContrastColor(final Context context, final int color,
...
/**
 * Resolves {@param color} to an actual color if it is {@link
...
/**
 * For debugging. This was copied from
 * com.android.internal.util.NotificationColorUtil.
 */
private static String contrastChange(final int colorOld, final int colorNew,
                                     final int bg) {
...
/**
 * Finds a text color with sufficient contrast over bg that has the same hue
 ...
 * @return a color with the same hue as {@param color}, potentially darkened
 *     to meet the
 *          contrast ratio.
 *
 * This was copied from com.android.internal.util.NotificationColorUtil.
 */
private static int findContrastColor(final int fg, final int bg,
...
public static int getMutedColor(final int color,
                                final float whiteScrimAlpha) {
	int whiteScrim =
		ColorUtils.setAlphaComponent(Color.WHITE, (int)(255 * whiteScrimAlpha));
	return ColorUtils.compositeColors(whiteScrim, color);
}

public static int getMutedColor(final Context context, final int color,
                                final float scrimAlpha) {
	int scrim =
		ColorUtils.setAlphaComponent(Color.WHITE, (int)(255 * scrimAlpha));
...
private static int
getLighterOrDarkerVersionOfColor(final int color, final float contrastRatio) {
	int whiteMinAlpha =
		ColorUtils.calculateMinimumAlpha(Color.WHITE, color, contrastRatio);
	int blackMinAlpha =
		ColorUtils.calculateMinimumAlpha(Color.BLACK, color, contrastRatio);
	int translucentWhiteOrBlack;
	if (whiteMinAlpha >= 0) {
		translucentWhiteOrBlack =
			ColorUtils.setAlphaComponent(Color.WHITE, whiteMinAlpha);
	} else if (blackMinAlpha >= 0) {
		translucentWhiteOrBlack =
			ColorUtils.setAlphaComponent(Color.BLACK, blackMinAlpha);
	} else {
