...
// Ratio of the diameter of an normalized circular icon to the actual icon
// size.
...
IconNormalizer(final Context context) {
	...
	mPaintMaskShapeOutline.setXfermode(
 ...
 * For this method to work, the shape path bounds should be in [0,1]x[0,1]
 * bounds.
 */
private boolean isShape(final Path maskPath) {
	// Condition1:
	// If width and height of the path not close to a square, then the icon
	// shape is not same as the mask shape.
	float iconRatio = ((float)mBounds.width()) / mBounds.height();
	if (Math.abs(iconRatio - 1) > BOUND_RATIO_MARGIN) {
		if (DEBUG) {
			Log.d(TAG,
			      "Not same as mask shape because width != height. " + iconRatio);
		}
		return false;
	}

	// Condition 2:
	// Actual icon (white) and the fitted shape (e.g., circle)(red) XOR
	// operation should generate transparent image, if the actual icon is
...
private boolean isTransparentBitmap() {
	...
	float percentageDiffPixels =
		((float)sum) / (mBounds.width() * mBounds.height());
...
/**
 * Returns the amount by which the {@param d} should be scaled (in both
 * dimensions) so that it matches the design guidelines for a launcher icon.
 *
 * We first calculate the convex hull of the visible portion of the icon.
 * This hull then compared with the bounding rectangle of the hull to find how
 * closely it resembles a circle and a square, by comparing the ratio of the
 * areas. Note that this is not an ideal solution but it gives satisfactory
 * result without affecting the performance.
 *
 * This closeness is used to determine the ratio of hull area to the full icon
 * size. Refer {@link #MAX_CIRCLE_AREA_FACTOR} and {@link
 * #MAX_SQUARE_AREA_FACTOR}
 *
 * @param outBounds optional rect to receive the fraction distance from each
 *     edge.
 */
public synchronized float getScale(final @NonNull Drawable d,
                                   final @Nullable RectF outBounds,
                                   final @Nullable Path path,
                                   final @Nullable boolean[] outMaskShape) {
	...
	// row find the first and the last non-transparent pixel. Set those values
	// to mLeftBorder and mRightBorder and use -1 if there are no visible pixel
	// in the row.
	...
	if (outBounds != null) {
		outBounds.set(((float)mBounds.left) / width,
		              ((float)mBounds.top) / height,
	...
	// Use sqrt of the final ratio as the images is scaled across both width and
	// height.
	float scale = areaScale > scaleRequired
	              ? (float)Math.sqrt(scaleRequired / areaScale)
	              : 1;
	if (Utilities.ATLEAST_OREO && d instanceof AdaptiveIconDrawable &&
	    mAdaptiveIconScale == SCALE_NOT_INITIALIZED) {
 ...
 * closely it resembles a circle and a square, by comparing the ratio of the
 * areas. Note that this is not an ideal solution but it gives satisfactory
 * result without affecting the performance. <p> This closeness is used to
 * determine the ratio of hull area to the full icon size. Refer {@link
 * #MAX_CIRCLE_AREA_FACTOR} and {@link #MAX_SQUARE_AREA_FACTOR}
 *
 * @param outBounds optional rect to receive the fraction distance from each
 *     edge.
 */
...
getScale(final @NonNull Drawable d, final @Nullable RectF outBounds,
...
/**
 * Modifies {@param xCoordinates} to represent a convex border. Fills in all
 * missing values (except on either ends) with appropriate values.
 *
 ...
 */
private static void convertToConvexArray(final float[] xCoordinates,
                                         final int direction, final int topY,
                                         final int bottomY) {
	...
	for (int i = topY + 1; i <= bottomY; i++) {
		...
		if (lastAngle == Float.MAX_VALUE) {
			...
			float currentAngle =
				(xCoordinates[i] - xCoordinates[last]) / (i - last);
			start = last;
			// If this position creates a concave angle, keep moving up until we
			// find a position which creates a convex angle.
...
/**
 * @return The diameter of the normalized circle that fits inside of the
 *     square (size x size).
 */
