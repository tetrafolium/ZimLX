...
public class ColorExtractor {
  ...
  public static int findDominantColorByHue(final Bitmap bitmap,
    ...
    // creating a histogram of weighted s*v scores, for up to 100*100 [s,v]
    // buckets. The highest-scoring RGB color wins.
    for (int i = 0; i < pixelCount; i++) {
      ...
      if (hue == bestHue) {
        ...
        if (newTotal > highScore) {
          highScore = newTotal;
          // All the colors in the winning bucket are very similar. Last in
          // wins.
          bestColor = rgb;
        }
      }
  ...
  public static boolean isSingleColor(final Drawable drawable,
                                      final int color) {
    if (drawable == null)
      return true;
