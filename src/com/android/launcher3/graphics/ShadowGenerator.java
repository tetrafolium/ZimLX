...
public class ShadowGenerator {
  ...
  public ShadowGenerator(final Context context) {
    ...
    mDefaultBlurMaskFilter =
        new BlurMaskFilter(mIconSize * BLUR_FACTOR, Blur.NORMAL);
  }

  public static ShadowGenerator getInstance(final Context context) {
    // TODO: This currently fails as the system default icon also needs a shadow
    // as it uses adaptive icon. Preconditions.assertNonUiThread();
    synchronized (LOCK) {
  ...
  public synchronized void recreateIcon(final Bitmap icon, final Canvas out) {
    recreateIcon(icon, mDefaultBlurMaskFilter, AMBIENT_SHADOW_ALPHA,
                 KEY_SHADOW_ALPHA, out);
  }

  public synchronized void recreateIcon(final Bitmap icon,
                                        final BlurMaskFilter blurMaskFilter,
                                        final int ambientAlpha,
                                        final int keyAlpha, final Canvas out) {
    ...
    out.drawBitmap(shadow, offset[0],
                   offset[1] + KEY_SHADOW_DISTANCE * mIconSize, mDrawPaint);
  ...
  public synchronized Bitmap recreateIcon(final Bitmap icon,
    ...
    // Draw key shadow
    mDrawPaint.setAlpha(keyAlpha);
    mCanvas.drawBitmap(shadow, offset[0],
                       offset[1] + KEY_SHADOW_DISTANCE * mIconSize, mDrawPaint);
  ...
  /**
   * Returns the minimum amount by which an icon with {@param bounds} should be
   * scaled so that the shadows do not get clipped.
   */
  public static float getScaleForBounds(final RectF bounds) {
    ...
    if (bounds.bottom < bottomSpace) {
      scale = Math.min(scale, (HALF_DISTANCE - bottomSpace) /
                                  (HALF_DISTANCE - bounds.bottom));
    }
    return scale;
  }
  ...
  public static class Builder {
    ...
    public Builder(final int color) { this.color = color; }
    ...
    public void drawShadow(final Canvas c) {
      ...
      // Key shadow
      p.setShadowLayer(
          shadowBlur, 0, keyShadowDistance,
