...
public class DragPreviewProvider {
  ...
  public DragPreviewProvider(final View view) { this(view, view.getContext()); }

  public DragPreviewProvider(final View view, final Context context) {
    mView = view;
    blurSizeOutline = context.getResources().getDimensionPixelSize(
  ...
  /**
   * Returns a new bitmap to show when the {@link #mView} is being dragged
   * around. Responsibility for the bitmap is transferred to the caller.
   */
  ...
  public final void generateDragOutline(final Bitmap preview) {
    ...
    new Handler(UiThreadHelper.getBackgroundLooper())
        .post(mOutlineGeneratorCallback);
  }
  ...
  public float getScaleAndPosition(final Bitmap preview, final int[] outPos) {
    ...
    if (mView instanceof LauncherAppWidgetHostView) {
      // App widgets are technically scaled, but are drawn at their expected
      // size -- so the app widget scale should not affect the scale of the
      // preview.
      scale /= ((LauncherAppWidgetHostView)mView).getScaleToFit();
    }

    outPos[0] =
        Math.round(outPos[0] - (preview.getWidth() -
                                scale * mView.getWidth() * mView.getScaleX()) /
                                   2);
  ...
  private class OutlineGeneratorCallback implements Runnable {
    ...
    public void run() {
      ...
      // We start by removing most of the alpha channel so as to ignore shadows,
      // and other types of partial transparency when defining the shape of the
      // object
      ...
      paint.setMaskFilter(
          new BlurMaskFilter(blurSizeOutline, BlurMaskFilter.Blur.OUTER));
      ...
      paint.setMaskFilter(
          new BlurMaskFilter(blurSizeOutline, BlurMaskFilter.Blur.NORMAL));
      ...
      canvas.drawRect(0, 0, -thickInnerBlurOffset[0],
                      thickInnerBlur.getHeight(), paint);
      canvas.drawRect(0, 0, thickInnerBlur.getWidth(), -thickInnerBlurOffset[1],
                      paint);
      ...
      canvas.drawBitmap(thickInnerBlur, thickInnerBlurOffset[0],
                        thickInnerBlurOffset[1], paint);
      canvas.drawBitmap(thickOuterBlur, outerBlurOffset[0], outerBlurOffset[1],
                        paint);

      // draw the bright outline
      canvas.drawBitmap(brightOutline, brightOutlineOffset[0],
                        brightOutlineOffset[1], paint);
