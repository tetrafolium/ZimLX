...
public class LauncherIcons implements AutoCloseable {
  ...
  private LauncherIcons(final Context context) {
    ...
    mCanvas.setDrawFilter(
        new PaintFlagsDrawFilter(DITHER_FLAG, FILTER_BITMAP_FLAG));
  }
  ...
  public BitmapInfo createIconBitmap(final ShortcutIconResource iconRes) {
    try {
      ...
      if (resources != null) {
        final int id =
            resources.getIdentifier(iconRes.resourceName, null, null);
        // do not stamp old legacy shortcuts as the app may have already
        // forgotten about it
  ...
  public BitmapInfo createIconBitmap(final Bitmap icon) {
    if (mIconBitmapSize == icon.getWidth() &&
        mIconBitmapSize == icon.getHeight()) {
      return BitmapInfo.fromBitmap(icon);
    }
    return BitmapInfo.fromBitmap(createIconBitmap(
        new BitmapDrawable(mContext.getResources(), icon), 1f));
  }

  /**
   * Returns a bitmap suitable for displaying as an icon at various launcher UIs
   * like all apps view or workspace. The icon is badged for {@param user}. The
   * bitmap is also visually normalized with other icons.
   */
  public BitmapInfo createBadgedIconBitmap(final Drawable icon,
                                           final UserHandle user,
                                           final int iconAppTargetSdk) {
  ...
  /**
   * Creates a normalized bitmap suitable for the all apps view. The bitmap is
   * also visually normalized with other icons and has enough spacing to add
   * shadow.
   */
  public Bitmap createScaledBitmapWithoutShadow(final Drawable icon,
                                                final int iconAppTargetSdk) {
  ...
  public void setWrapperBackgroundColor(final int color) {
    mWrapperBackgroundColor =
        (Color.alpha(color) < 255) ? DEFAULT_WRAPPER_BACKGROUND : color;
  }

  private Drawable normalizeAndWrapToAdaptiveIcon(final Drawable mIcon,
                                                  final int iconAppTargetSdk,
                                                  final RectF outIconBounds,
                                                  final float[] outScale,
                                                  final UserHandle user) {
    float scale = 1f;
    Drawable icon = mIcon;
    if (Utilities.ATLEAST_OREO) {
      ...
      scale = getNormalizer().getScale(icon, outIconBounds, dr.getIconMask(),
                                       outShape);
      /*if (!outShape[0] && (icon instanceof NonAdaptiveIconDrawable)) {
          ...
          ((ColorDrawable)
      dr.getBackground()).setColor(mWrapperBackgroundColor);
  ...
  /**
   * Adds the {@param badge} on top of {@param target} using the badge
   * dimensions.
   */
  ...
  public Bitmap createIconBitmap(final Drawable icon, final float scale) {
    ...
    if (Utilities.ATLEAST_OREO && icon instanceof AdaptiveIconDrawable) {
      int offset = Math.max((int)Math.ceil(BLUR_FACTOR * textureWidth),
                            Math.max(left, top));
  ...
  public BitmapInfo
  createShortcutIcon(final ShortcutInfoCompat shortcutInfo,
                     final boolean badged,
                     final @Nullable Provider<Bitmap> fallbackIconProvider) {
    Drawable unbadgedDrawable =
        DeepShortcutManager.getInstance(mContext).getShortcutIconDrawable(
            shortcutInfo, mFillResIconDpi);
    ...
    result.icon = BitmapRenderer.createHardwareBitmap(
        mIconBitmapSize, mIconBitmapSize, (c) -> {
  ...
  getShortcutInfoBadge(final ShortcutInfoCompat shortcutInfo,
  ...
  /**
   * An extension of {@link BitmapDrawable} which returns the bitmap pixel size
   * as intrinsic size. This allows the badging to be done based on the action
   * bitmap size rather than the scaled bitmap size.
   */
  private static class FixedSizeBitmapDrawable extends BitmapDrawable {

    public FixedSizeBitmapDrawable(final Bitmap bitmap) { super(null, bitmap); }
