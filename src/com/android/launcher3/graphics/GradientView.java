...
/**
 * Draws a translucent radial gradient background from an initial state with
 * progress 0.0 to a final state with progress 1.0;
 */
public class GradientView
    extends View implements WallpaperColorInfo.OnChangeListener {
  ...
  public GradientView(final Context context, final AttributeSet attrs) {
    ...
    boolean hideDockGradient =
        Utilities.getZimPrefs(context).getDockGradientStyle();
    this.mAlphaStart =
        launcher.getDeviceProfile().isVerticalBarLayout() || hideDockGradient
            ? 0
            : 100;
  ...
  protected void updateColors() {
    this.mColor1 = ColorUtils.setAlphaComponent(
        mWallpaperColorInfo.getMainColor(), mAlphaColors);
    this.mColor2 = ColorUtils.setAlphaComponent(
        mWallpaperColorInfo.getSecondaryColor(), mAlphaColors);
    if (mWidth + mHeight > 0) {
      createRadialShader();
    }
  }
  ...
  protected void createRadialShader() {
    ...
    float posScreenBottom =
        (radius - mHeight) / radius; // center lives below screen

    RadialGradient shaderNoScrim = new RadialGradient(
        mWidth * 0.5f, mHeight * gradientCenterY, radius,
        new int[] {mColor1, mColor1, mColor2},
        new float[] {0f, posScreenBottom, 1f}, Shader.TileMode.CLAMP);
  ...
  protected void onDraw(final Canvas canvas) {
    ...
    float linearProgress =
        head + (mProgress * (Utilities.ATLEAST_MARSHMALLOW ? 1f : 0.85f) *
                (1f - head));
    float startMaskY =
        (1f - linearProgress) * mHeight - mMaskHeight * linearProgress;
    float interpolatedAlpha =
        (255 - mAlphaStart) * mAccelerator.getInterpolation(mProgress);
    ...
    if (DEBUG) {
      ...
      canvas.drawLine(0, startMaskY + mMaskHeight, mWidth,
                      startMaskY + mMaskHeight, mDebugPaint);
    }
  }

  public Bitmap createDitheredAlphaMask() {
    Bitmap dst =
        Bitmap.createBitmap(mMaskWidth, mMaskHeight, Bitmap.Config.ALPHA_8);
    Canvas c = new Canvas(dst);
    Paint paint = new Paint(Paint.DITHER_FLAG);
    LinearGradient lg =
        new LinearGradient(0, 0, 0, mMaskHeight,
                           new int[] {0x00FFFFFF,
