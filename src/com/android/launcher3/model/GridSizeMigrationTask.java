 ...
 * one column), as a result of restoring from a larger device or device density
 * change.
 */
public class GridSizeMigrationTask {
  ...
  private static final String KEY_MIGRATION_SRC_WORKSPACE_SIZE =
      "migration_src_workspace_size";
  private static final String KEY_MIGRATION_SRC_HOTSEAT_COUNT =
      "migration_src_hotseat_count";

  // These are carefully selected weights for various item types (Math.random?),
  // to allow for the least absurd migration experience.
  ...
  public final ArrayList<ContentProviderOperation> mUpdateOperations =
      new ArrayList<>();
  ...
  protected GridSizeMigrationTask(final Context context,
  ...
  protected GridSizeMigrationTask(final Context context,
                                  final InvariantDeviceProfile idp,
                                  final HashSet<String> validPackages,
                                  final int srcHotseatSize,
                                  final int destHotseatSize) {
    mContext = context;
    mIdp = idp;
    mValidPackages = validPackages;
  ...
  public boolean applyOperations() throws Exception {
    ...
    if (!mEntryToRemove.isEmpty()) {
      ...
      mContext.getContentResolver().delete(
          ...
          Utilities.createDbSelectionQuery(LauncherSettings.Favorites._ID,
                                           mEntryToRemove),
          null);
    }
  ...
  /**
   * To migrate hotseat, we load all the entries in order (LTR or RTL) and
   * arrange them in the order in the new hotseat while keeping an empty space
   * for all-apps. If the number of entries is more than what can fit in the new
   * hotseat, we drop the entries with least weight. For weight calculation
   * {@see #WT_SHORTCUT}, {@see #WT_APPLICATION} & {@see #WT_FOLDER_FACTOR}.
  ...
  protected boolean migrateHotseat() throws Exception {
    ...
    int requiredCount =
        FeatureFlags.NO_ALL_APPS_ICON ? mDestHotseatSize : mDestHotseatSize - 1;
    ...
    for (DbEntry entry : items) {
      if (entry.screenId != newScreenId) {
        ...
        // These values does not affect the item position, but we should set
        // them to something other than -1.
      ...
      if (!FeatureFlags.NO_ALL_APPS_ICON &&
          mIdp.isAllAppsButtonRank(newScreenId)) {
        newScreenId++;
      }
    }
  ...
  protected boolean migrateWorkspace() throws Exception {
    ...
    if (!mCarryOver.isEmpty()) {
      ...
      do {
        ...
        // {@link #mCarryOver}, to prevent an infinite loop. If no item could be
        // removed, break the loop and abort migration by throwing an exception.
        ...
        if (placement.finalPlacedItems.size() > 0) {
          ...
          throw new Exception(
              "None of the items can be placed on an empty screen");
        }
      ...
      for (int i = 0; i < count; i++) {
        ...
        mUpdateOperations.add(
            ContentProviderOperation.newInsert(uri).withValues(v).build());
      }
    }
    return applyOperations();
  }
   ...
   * Strategy:
   *   1) For all possible combinations of row and column, pick the one which
   * causes the least data loss: {@link #tryRemove(int, int, int, ArrayList,
   * float[])} 2) Maintain a list of all lost items before this screen, and add
   * any new item lost from this screen to that list as well. 3) If all those
   * items from the above list can be placed on this screen, place them
   *      (otherwise they are placed on a new screen).
   */
  protected void migrateScreen(final long screenId) {
    // If we are migrating the first screen, do not touch the first row.
    int startY = (FeatureFlags.QSB_ON_FIRST_SCREEN &&
    ...
    // removeWt represents the cost function for loss of items during migration,
    // and moveWt represents the cost function for repositioning the items.
    // moveWt is only considered if removeWt is same for two different
    // configurations. Start with Float.MAX_VALUE (assuming full data) and pick
    // the configuration with least cost.
    ...
    for (int x = 0; x < mSrcX; x++) {
      ...
      for (int y = mSrcY - 1; y >= startY; y--) {
        // Use a deep copy when trying out a particular combination as it can
        // change the underlying object.
        ArrayList<DbEntry> itemsOnScreen =
            tryRemove(x, y, startY, deepCopy(items), outLoss);

        if ((outLoss[0] < removeWt) ||
            ((outLoss[0] == removeWt) && (outLoss[1] < moveWt))) {
          removeWt = outLoss[0];
          moveWt = outLoss[1];
    ...
    // The remaining items in {@link #itemMap} are those which didn't get
    // placed.
    ...
    if (!mCarryOver.isEmpty() && removeWt == 0) {
      // No new items were removed in this step. Try placing all the items on
      // this screen.
      ...
      OptimalPlacementSolution placement = new OptimalPlacementSolution(
          occupied, deepCopy(mCarryOver), startY, true);
      placement.find();
   ...
   * @param outLoss array of size 2. The first entry is filled with weight loss,
   *     and the second
   * with the overall item movement.
   */
  private ArrayList<DbEntry> tryRemove(final int col, final int row,
                                       final int startY,
                                       final ArrayList<DbEntry> items,
                                       final float[] outLoss) {
    GridOccupancy occupied = new GridOccupancy(mTrgX, mTrgY);
    occupied.markCells(0, 0, mTrgX, startY, true);
  ...
  private class OptimalPlacementSolution {
    ...
    // If set to true, item movement are not considered in move cost, leading to
    // a more linear placement.
    ...
    public OptimalPlacementSolution(final GridOccupancy occupied,
                                    final ArrayList<DbEntry> itemsToPlace,
                                    final int startY) {
    ...
    public OptimalPlacementSolution(final GridOccupancy occupied,
                                    final ArrayList<DbEntry> itemsToPlace,
                                    final int startY,
                                    final boolean ignoreMove) {
      this.occupied = occupied;
      this.itemsToPlace = itemsToPlace;
      this.ignoreMove = ignoreMove;
      this.startY = startY;

      // Sort the items such that larger widgets appear first followed by 1x1
      // items
      Collections.sort(this.itemsToPlace);
    }

    public void find() { find(0, 0, 0, new ArrayList<DbEntry>()); }

    /**
     * Recursively finds a placement for the provided items.
     *
     * @param index       the position in {@link #itemsToPlace} to start looking
     *     at.
     * @param weightLoss  total weight loss upto this point
     * @param moveCost    total move cost upto this point
     * @param itemsPlaced all the items already placed upto this point
     */
    public void find(final int index, final float weightLoss,
                     final float moveCost,
                     final ArrayList<DbEntry> itemsPlaced) {
      if ((weightLoss >= lowestWeightLoss) ||
        ...
        // Keep a deep copy of current configuration as it can change during
        // recursion.
      ...
      ArrayList<DbEntry> itemsIncludingMe =
          new ArrayList<>(itemsPlaced.size() + 1);
      itemsIncludingMe.addAll(itemsPlaced);
      itemsIncludingMe.add(me);

      if (me.spanX > 1 || me.spanY > 1) {
        // If the current item is a widget (and it greater than 1x1), try to
        // place it at all possible positions. This is because a widget placed
        // at one position can affect the placement of a different widget.
        int myW = me.spanX;
        int myH = me.spanY;
        ...
        for (int y = startY; y < mTrgY; y++) {
          for (int x = 0; x < mTrgX; x++) {
            ...
            if (myW > me.minSpanX &&
                occupied.isRegionVacant(x, y, myW - 1, myH)) {
              me.spanX--;
              occupied.markCells(me, true);
              // 1 extra move cost
            ...
            if (myH > me.minSpanY &&
                occupied.isRegionVacant(x, y, myW, myH - 1)) {
              me.spanY--;
              occupied.markCells(me, true);
              // 1 extra move cost
              find(index + 1, weightLoss, newMoveCost + 1, itemsIncludingMe);
        ...
        // Finally also try a solution when this item is not included. Trying it
        // in the end causes it to get skipped in most cases due to higher
        // weight loss, and prevents unnecessary deep copies of various
        // configurations.
        find(index + 1, weightLoss + me.weight, moveCost, itemsPlaced);
      } else {
        // Since this is a 1x1 item and all the following items are also 1x1,
        // just place it at 'the most appropriate position' and hope for the
        // best. The most appropriate position: one with lease straight line
        // distance
          ...
          for (int x = 0; x < mTrgX; x++) {
            if (!occupied.cells[x][y]) {
              int dist = ignoreMove ? 0
                                    : ((me.cellX - x) * (me.cellX - x) +
                                       (me.cellY - y) * (me.cellY - y));
              if (dist < newDistance) {
                newX = x;
                newY = y;
                newDistance = dist;
        ...
        if (newX < mTrgX && newY < mTrgY) {
          ...
          //  1) there was at least one space, i.e., we were able to place this
          //  item 2) if the next item has the same weight (all items are
          //  already sorted), as
          //     if it has lower weight, that solution will automatically get
          //     discarded.
          //  3) ignoreMove false otherwise, move cost is ignored and the weight
          //  will
          //      anyway be same.
          ...
          find(itemsToPlace.size(), weightLoss + me.weight, moveCost,
               itemsPlaced);
        }
      }
    }
  ...
  private ArrayList<DbEntry> loadHotseatEntries() {
    Cursor c = mContext.getContentResolver().query(
        LauncherSettings.Favorites.CONTENT_URI,
        new String[] {Favorites._ID,       // 0
                      Favorites.ITEM_TYPE, // 1
                      Favorites.INTENT,    // 2
                      Favorites.SCREEN},   // 3
        Favorites.CONTAINER + " = " + Favorites.CONTAINER_HOTSEAT, null, null,
        null);
  ...
  protected ArrayList<DbEntry> loadWorkspaceEntries(final long screen) {
    Cursor c = queryWorkspace(new String[] {Favorites._ID,                // 0
                                            Favorites.ITEM_TYPE,          // 1
                                            Favorites.CELLX,              // 2
                                            Favorites.CELLY,              // 3
                                            Favorites.SPANX,              // 4
                              ...
                              Favorites.CONTAINER + " = " +
                                  Favorites.CONTAINER_DESKTOP + " AND " +
                                  Favorites.SCREEN + " = " + screen);
    ...
    final int indexIntent = c.getColumnIndexOrThrow(Favorites.INTENT);
    final int indexAppWidgetProvider =
        c.getColumnIndexOrThrow(Favorites.APPWIDGET_PROVIDER);
    final int indexAppWidgetId =
        c.getColumnIndexOrThrow(Favorites.APPWIDGET_ID);
    ...
    while (c.moveToNext()) {
      ...
      try {
        ...
        case Favorites.ITEM_TYPE_APPWIDGET: {
          ...
          entry.weight = Math.max(WT_WIDGET_MIN,
                                  WT_WIDGET_FACTOR * entry.spanX * entry.spanY);

          int widgetId = c.getInt(indexAppWidgetId);
          LauncherAppWidgetProviderInfo pInfo =
              AppWidgetManagerCompat.getInstance(mContext)
   ...
   */
  private int getFolderItemsCount(final long folderId) {
    Cursor c = queryWorkspace(new String[] {Favorites._ID, Favorites.INTENT},
  ...
  protected Cursor queryWorkspace(final String[] columns, final String where) {
    return mContext.getContentResolver().query(
        LauncherSettings.Favorites.CONTENT_URI, columns, where, null, null,
        null);
  }
   ...
   */
  private void verifyIntent(final String intentStr) throws Exception {
   ...
   */
  private void verifyPackage(final String packageName) throws Exception {
  ...
  protected static class DbEntry
      extends ItemInfo implements Comparable<DbEntry> {
    ...
    /**
     * Comparator such that larger widgets come first,  followed by all 1x1
     * items based on their weights.
     */
    @Override
    public int compareTo(final DbEntry another) {
    ...
    public boolean columnsSame(final DbEntry org) {
      return org.cellX == cellX && org.cellY == cellY && org.spanX == spanX &&
          org.spanY == spanY && org.screenId == screenId;
    }

    public void addToContentValues(final ContentValues values) {
  ...
  private static ArrayList<DbEntry> deepCopy(final ArrayList<DbEntry> src) {
  ...
  private static String getPointString(final int x, final int y) {
    return String.format(Locale.ENGLISH, "%d,%d", x, y);
  }

  public static void markForMigration(final Context context, final int gridX,
                                      final int gridY, final int hotseatSize) {
    Utilities.getPrefs(context)
        .edit()
        .putString(KEY_MIGRATION_SRC_WORKSPACE_SIZE,
                   getPointString(gridX, gridY))
   ...
   */
  public static boolean migrateGridIfNeeded(final Context context) {
    SharedPreferences prefs = Utilities.getPrefs(context);
    InvariantDeviceProfile idp = LauncherAppState.getIDP(context);

    String gridSizeString = getPointString(idp.numColumns, idp.numRows);

    if (gridSizeString.equals(
    ...
    try {
      ...
      // Hotseat
      int srcHotseatCount =
          prefs.getInt(KEY_MIGRATION_SRC_HOTSEAT_COUNT, idp.numHotseatIcons);
      if (srcHotseatCount != idp.numHotseatIcons) {
        // Migrate hotseat.

        dbChanged = new GridSizeMigrationTask(
                        context, LauncherAppState.getIDP(context),
      ...
      Point sourceSize = parsePoint(
          prefs.getString(KEY_MIGRATION_SRC_WORKSPACE_SIZE, gridSizeString));

      if (new MultiStepMigrationTask(validPackages, context)
  ...
  protected static HashSet<String> getValidPackages(final Context context) {
    // Initialize list of valid packages. This contain all the packages which
    // are already on the device and packages which are being installed. Any
    // item which doesn't belong to this set is removed. Since the loader
    // removes such items anyway, removing these items here doesn't cause any
    // extra data loss and gives us more free space on the grid for better
    // migration.
    HashSet<String> validPackages = new HashSet<>();
    for (PackageInfo info : context.getPackageManager().getInstalledPackages(
   ...
   */
  public static LongArrayMap<Object>
  removeBrokenHotseatItems(final Context context) throws Exception {
  ...
  /**
   * Task to run grid migration in multiple steps when the size difference is
   * more than 1.
   */
  protected static class MultiStepMigrationTask {
    ...
    public MultiStepMigrationTask(final HashSet<String> validPackages,
                                  final Context context) {
      mValidPackages = validPackages;
      mContext = context;
    }

    public boolean migrate(final Point sourceSize, final Point targetSize)
        throws Exception {
      boolean dbChanged = false;
      if (!targetSize.equals(sourceSize)) {
        if (sourceSize.x < targetSize.x) {
          // Source is smaller that target, just expand the grid without actual
          // migration.
        ...
        // Migrate the workspace grid, such that the points differ by max 1 in x
        // and y each on every step.
        while (!targetSize.equals(sourceSize)) {
          // Get the next size, such that the points differ by max 1 in x and y
          // each
    ...
    protected boolean runStepTask(final Point sourceSize, final Point nextSize)
        throws Exception {
      return new GridSizeMigrationTask(mContext,
                                       LauncherAppState.getIDP(mContext),
                                       mValidPackages, sourceSize, nextSize)
          .migrateWorkspace();
    }
  }
}
