...
public class BgDataModel {
  ...
  /**
   * List of all the folders and shortcuts directly on the home screen (no
   * widgets or shortcuts within folders).
   */
  ...
  /**
   * Maps all launcher activities to the id's of their shortcuts (if they have
   * any).
   */
  public final MultiHashMap<ComponentKey, String> deepShortcutMap =
      new MultiHashMap<>();
  ...
  public synchronized void dump(final String prefix, final FileDescriptor fd,
  ...
  private synchronized void dumpProto(final String prefix,
    ...
    if (Arrays.asList(args).contains("--debug")) {
      for (int i = 0; i < targetList.size(); i++) {
        writer.println(prefix +
                       DumpTargetWrapper.getDumpTargetStr(targetList.get(i)));
      }
      return;
    } else {
      LauncherDumpProto.LauncherImpression proto =
          new LauncherDumpProto.LauncherImpression();
  ...
  public synchronized void removeItem(final Context context,
                                      final ItemInfo... items) {
    removeItem(context, Arrays.asList(items));
  }

  public synchronized void
  removeItem(final Context context, final Iterable<? extends ItemInfo> items) {
    for (ItemInfo item : items) {
      ...
      case LauncherSettings.Favorites.ITEM_TYPE_DEEP_SHORTCUT: {
        ...
        if ((count == null || --count.value == 0) &&
            !InstallShortcutReceiver.getPendingShortcuts(context).contains(
                pinnedShortcut)) {
  ...
  public synchronized void addItem(final Context context, final ItemInfo item,
  ...
  /**
   * Clear all the deep shortcuts for the given package, and re-add the new
   * shortcuts.
   */
  public synchronized void
  updateDeepShortcutMap(final String packageName, final UserHandle user,
