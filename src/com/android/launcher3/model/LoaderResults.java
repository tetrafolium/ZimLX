...
/**
 * Helper class to handle results of {@link
...
private static final int ITEMS_CHUNK =
...
public LoaderResults(final LauncherAppState app, final BgDataModel dataModel,
                     final AllAppsList allAppsList, final int pageToBindFirst,
                     final WeakReference<Callbacks> callbacks) {
	...
	mCallbacks =
		callbacks == null ? new WeakReference<Callbacks>(null) : callbacks;
}
...
public void bindWorkspace() {
	...
	final int currentScreen;
	{
		int currScreen = mPageToBindFirst != PagedView.INVALID_RESTORE_PAGE
		           ? mPageToBindFirst
		           : callbacks.getCurrentWorkspaceScreen();
		if (currScreen >= orderedScreenIds.size()) {
			// There may be no workspace screens (just hotseat items and an empty
			// page).
	...
	final long currentScreenId = validFirstPage
	...
	// In case of validFirstPage, only bind the first screen, and defer binding
	// the remaining screens after first onDraw (and an optional the fade
	// animation whichever happens later). This ensures that the first screen is
	// immediately visible (eg. during rotation) In case of !validFirstPage,
	// bind all pages one after other.
	...
	if (validFirstPage) {
		r = ()->{
			...
			if (callbacks15 != null) {
				// We are loading synchronously, which means, some of the pages will
				// be bound after first draw. Inform the callbacks that page binding
				// is not complete, and schedule the remaining pages.
 ...
 */
public static <T extends ItemInfo> void
filterCurrentWorkspaceItems(final long currentScreenId,
	...
	// Order the set of items by their containers first, this allows use to walk
	// through the list sequentially, build up a list of containers that are in
	// the specified screen, as well as all items in those containers.
	Set<Long> itemsOnScreen = new HashSet<>();
	Collections.sort(
		allWorkspaceItems,
		(Comparator<ItemInfo>)(lhs, rhs)
		->Utilities.longCompare(lhs.container, rhs.container));
 ...
 */
private void
sortWorkspaceItemsSpatially(final ArrayList<ItemInfo> workspaceItems) {
	...
	Collections.sort(workspaceItems, new Comparator<ItemInfo>() {
			...
			public int compare(final ItemInfo lhs, final ItemInfo rhs) {
			        if (lhs.container == rhs.container) {
			                // Within containers, order by their spatial position in that
			                // container
			                switch ((int)lhs.container) {
					case LauncherSettings.Favorites.CONTAINER_DESKTOP: {
						long lr = (lhs.screenId * screenCellCount + lhs.cellY * screenCols +
						           lhs.cellX);
						long rr = (rhs.screenId * screenCellCount + rhs.cellY * screenCols +
						           rhs.cellX);
...
private void
bindWorkspaceItems(final ArrayList<ItemInfo> workspaceItems,
