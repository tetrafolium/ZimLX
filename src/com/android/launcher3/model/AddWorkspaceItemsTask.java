	...
	synchronized (dataModel) {
		...
		for (ItemInfo item : filteredItems) {
			...
			Pair<Long, int[]> coords = findSpaceForItem(
				app, dataModel, workspaceScreens, addedWorkspaceScreensFinal,
			...
			getModelWriter().addItemToDatabase(
				itemInfo, LauncherSettings.Favorites.CONTAINER_DESKTOP, screenId,
...
protected void updateScreens(final Context context,
 ...
 * called after the workspace has been loaded. We identify a shortcut by its
 * intent.
 */
protected boolean shortcutExists(final BgDataModel dataModel,
		...
		for (ItemInfo item : dataModel.itemsIdMap) {
			if (item instanceof ShortcutInfo) {
				...
				if (item.getIntent() != null && info.user.equals(user)) {
					...
					if (isLauncherAppTarget && info.isPromise() &&
					    info.hasStatusFlag(ShortcutInfo.FLAG_AUTOINSTALL_ICON) &&
					    info.getTargetComponent() != null && compPkgName != null &&
					    compPkgName.equals(
 ...
 */
protected Pair<Long, int[]>
findSpaceForItem(final LauncherAppState app, final BgDataModel dataModel,
                 final ArrayList<Long> workspaceScreens,
	...
	if (preferredScreenIndex < screenCount) {
		...
		found = findNextAvailableIconSpaceInScreen(app, screenItems.get(screenId),
		                                           cordinates, spanX, spanY);
	}
	...
	if (!found) {
		...
		for (int screen = 1; screen < screenCount; screen++) {
			...
			if (findNextAvailableIconSpaceInScreen(app, screenItems.get(screenId),
			                                       cordinates, spanX, spanY)) {
	...
	if (!found) {
		...
		screenId = LauncherSettings.Settings
		           .call(app.getContext().getContentResolver(),
...
private boolean findNextAvailableIconSpaceInScreen(
	...
	GridOccupancy occupied =
		new GridOccupancy(profile.numColumns, profile.numRows);
	if (occupiedPos != null) {
