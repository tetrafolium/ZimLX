...
public LoaderTask(final LauncherAppState app, final AllAppsList bgAllAppsList,
                  final BgDataModel dataModel, final LoaderResults results) {
	mApp = app;
...
private void sendFirstScreenActiveInstallsBroadcast() {
	...
	long firstScreen =
		mBgDataModel.workspaceScreens.isEmpty()
	...
	filterCurrentWorkspaceItems(
		firstScreen, allItems, firstScreenItems,
...
public void run() {
	...
	try (LauncherModel.LoaderTransaction transaction =
		...
		// Notify the installer packages of packages with active installs on the
		// first screen.
		TraceHelper.partitionSection(TAG,
...
private void loadWorkspace() {
	...
	final MultiHashMap<UserHandle, String> pendingPackages =
		new MultiHashMap<>();
	...
	if (clearDb) {
		...
		LauncherSettings.Settings.call(
			contentResolver, LauncherSettings.Settings.METHOD_CREATE_EMPTY_DB);
	}
	...
	synchronized (mBgDataModel) {
		...
		mBgDataModel.workspaceScreens.addAll(
			LauncherModel.loadWorkspaceScreensDb(context));

		Map<ShortcutKey, ShortcutInfoCompat> shortcutKeyToPinnedShortcuts =
			new HashMap<>();
		final LoaderCursor c = new LoaderCursor(
			contentResolver.query(LauncherSettings.Favorites.CONTENT_URI, null,
			                      null, null, null),
			mApp);
		...
		try {
			final int appWidgetIdIndex =
				c.getColumnIndexOrThrow(LauncherSettings.Favorites.APPWIDGET_ID);
			final int appWidgetProviderIndex = c.getColumnIndexOrThrow(
				LauncherSettings.Favorites.APPWIDGET_PROVIDER);
			final int spanXIndex =
				c.getColumnIndexOrThrow(LauncherSettings.Favorites.SPANX);
			final int spanYIndex =
				c.getColumnIndexOrThrow(LauncherSettings.Favorites.SPANY);
			final int rankIndex =
				c.getColumnIndexOrThrow(LauncherSettings.Favorites.RANK);
			final int optionsIndex =
				c.getColumnIndexOrThrow(LauncherSettings.Favorites.OPTIONS);
			final int titleAliasIndex =
				c.getColumnIndexOrThrow(LauncherSettings.Favorites.TITLE_ALIAS);
			final int customIconEntryIndex = c.getColumnIndexOrThrow(
				LauncherSettings.Favorites.CUSTOM_ICON_ENTRY);
			final int swipeUpActionEntryIndex =
				c.getColumnIndexOrThrow(LauncherSettings.Favorites.SWIPE_UP_ACTION);
			...
			for (UserHandle user : mUserManager.getUserProfiles()) {
				...
				if (userUnlocked) {
					...
					if (mShortcutManager.wasLastCallSuccess()) {
						...
						// lock state changes too frequently. For the purpose of the
						// loading shortcuts, consider the user is still locked.
						userUnlocked = false;
					}
				}
				...
				try {
					...
					case LauncherSettings.Favorites.ITEM_TYPE_DEEP_SHORTCUT:
				      ...
				      : 0;
						...
						targetPkg =
							cn == null ? intent.getPackage() : cn.getPackageName();
						...
						if (!Process.myUserHandle().equals(c.user)) {
							if (c.itemType ==
								...
								c.markDeleted(
									"Legacy shortcuts are only allowed for default user");
								continue;
							...
							if (mLauncherApps.isActivityEnabledForProfile(cn, c.user)) {
								...
								if (c.hasRestoreFlag(ShortcutInfo.FLAG_AUTOINSTALL_ICON)) {
									...
									if (intent != null) {
										c.restoreFlag = 0;
										c.updater()
										.put(LauncherSettings.Favorites.INTENT,
										     intent.toUri(0))
										.commit();
						...
						if (c.restoreFlag != 0) {
							...
							info = c.getAppShortcutInfo(intent, allowMissingTarget,
							                            useLowResIcon);
						} else if (c.itemType ==
							...
							if (unlockedUsers.get(c.serialNumber)) {
								...
								Provider<Bitmap> fallbackIconProvider =
									...
									public Bitmap get() {
										...
										return c.loadIcon(finalInfo) ? finalInfo.iconBitmap
						       : null;
									}
								};
								LauncherIcons li = LauncherIcons.obtain(context);
								li.createShortcutIcon(pinnedShortcut, true /* badged */,
								                      fallbackIconProvider)
								.applyTo(info);
								li.recycle();
								if (pmHelper.isAppSuspended(pinnedShortcut.getPackage(),
								                            info.user)) {
						...
						if (info != null) {
							...
							info.onLoadCustomizations(
								titleAlias, swipeUpAction,
					...
					case LauncherSettings.Favorites.ITEM_TYPE_CUSTOM_APPWIDGET:
						...
						final AppWidgetProviderInfo provider =
							widgetProvidersMap.get(new ComponentKey(
										       ComponentName.unflattenFromString(savedProvider),
										       c.user));

						final boolean isProviderReady = isValidProvider(provider);
						if (!isSafeMode && !customWidget && wasProviderReady &&
						    !isProviderReady) {
							c.markDeleted("Deleting widget that isn't installed anymore: " +
							              provider);
						} else {
							if (isProviderReady) {
								appWidgetInfo =
								...
								appWidgetInfo =
									new LauncherAppWidgetInfo(appWidgetId, component);
								appWidgetInfo.restoreStatus = c.restoreFlag;
								SessionInfo si =
									installingPkgs.get(component.getPackageName());
								Integer installProgress =
									si == null ? null : (int)(si.getProgress() * 100);
		...
		if (c.commitDeleted()) {
			...
			ArrayList<Long> deletedFolderIds =
				(ArrayList<Long>)LauncherSettings.Settings
				.call(contentResolver,
				      LauncherSettings.Settings.METHOD_DELETE_EMPTY_FOLDERS)
				.getSerializable(LauncherSettings.Settings.EXTRA_VALUE);
			for (long folderId : deletedFolderIds) {
				mBgDataModel.workspaceItems.remove(
					mBgDataModel.folders.get(folderId));
		...
		FolderIconPreviewVerifier verifier =
			new FolderIconPreviewVerifier(mApp.getInvariantDeviceProfile());
		// Sort the folder items and make sure all items in the preview are high
		// resolution.
		...
		if (!isSdCardReady && !pendingPackages.isEmpty()) {
			context.registerReceiver(
				...
				new IntentFilter(Intent.ACTION_BOOT_COMPLETED), null,
				new Handler(LauncherModel.getWorkerLooper()));
		}

		// Remove any empty screens
		ArrayList<Long> unusedScreens =
			new ArrayList<>(mBgDataModel.workspaceScreens);
		...
		if (unusedScreens.size() != 0) {
			...
			LauncherModel.updateWorkspaceScreenOrder(context,
			                                         mBgDataModel.workspaceScreens);
		}
	}
}
...
private void loadAllApps() {
	...
	if (FeatureFlags.LAUNCHER3_PROMISE_APPS_IN_ALL_APPS) {
		...
		for (PackageInstaller.SessionInfo info :
			...
			mBgAllAppsList.addPromiseApp(
				mApp.getContext(),
				PackageInstallerCompat.PackageInstallInfo.fromInstallingState(
					info));
		}
	}
...
private void loadDeepShortcuts() {
	...
	mBgDataModel.hasShortcutHostPermission =
		mShortcutManager.hasHostPermission();
