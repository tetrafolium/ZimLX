 ...
 * installed/updated/removed) or when a user availability changes.
 */
...
public static final int OP_USER_AVAILABILITY_CHANGE =
	7;                               // user available/unavailable
...
public PackageUpdatedTask(final int op, final UserHandle user,
...
public void execute(final LauncherAppState app, final BgDataModel dataModel,
	...
	case OP_ADD: {
		...
		for (int i = 0; i < N; i++) {
			...
			if (Utilities.ATLEAST_OREO && prefs.getAutoAddInstalled() &&
				...
				SessionCommitReceiver.queueAppIconAddition(context, packages[i],
				                                           mUser);
			} else if (!Utilities.ATLEAST_OREO &&
			           !Process.myUserHandle().equals(mUser)) {
				// Automatically add homescreen icon for work profile apps for below O
				// device.
				SessionCommitReceiver.queueAppIconAddition(context, packages[i],
				                                           mUser);
			}
	...
	final ArrayMap<ComponentName, AppInfo> addedOrUpdatedApps =
		new ArrayMap<>();
	if (!addedOrModified.isEmpty()) {
		scheduleCallbackTask(
			(callbacks)->callbacks.bindAppsAddedOrUpdated(addedOrModified));
	...
	if (mOp == OP_ADD || flagOp != FlagOp.NO_OP) {
					...
					if (cn != null && matcher.matches(si, cn)) {
						...
						if (si.isPromise() && isNewApkAvailable) {
							...
							if (si.itemType == Favorites.ITEM_TYPE_DEEP_SHORTCUT) {
								List<ShortcutInfoCompat> shortcut =
							...
							if (si.hasStatusFlag(ShortcutInfo.FLAG_AUTOINSTALL_ICON)) {
								...
								if (!isTargetValid) {
									...
									Intent intent =
										new PackageManagerHelper(context).getAppLaunchIntent(
											cn.getPackageName(), mUser);
									if (intent != null) {
								...
								removedShortcuts.put(si.id, true);
								FileLog.e(TAG,
								          "Restored shortcut no longer valid " + si.intent);
								continue;
							} else {
								si.status = ShortcutInfo.DEFAULT;
								infoUpdated = true;
		...
		if (!removedShortcuts.isEmpty()) {
			deleteAndBindComponentsRemoved(
				ItemInfoMatcher.ofItemIds(removedShortcuts, false));
		}
	...
	if (mOp == OP_REMOVE) {
		...
		// Update removedComponents as some components can get removed during
		// package update
	...
	if (!removedPackages.isEmpty() || !removedComponents.isEmpty()) {
		...
		InstallShortcutReceiver.removeFromInstallQueue(context, removedPackages,
		                                               mUser);
	}
	...
	if (Utilities.ATLEAST_OREO && mOp == OP_ADD) {
		// Load widgets for the new package. Changes due to app updates are
		// handled through AppWidgetHost events, this is just to initialize the
		// long-press options.
		for (int i = 0; i < N; i++) {
			dataModel.widgetsModel.update(app,
