...
public class LoaderCursor extends CursorWrapper {
  ...
  public LoaderCursor(final Cursor c, final LauncherAppState app) {
    ...
    customIconIndex =
        getColumnIndexOrThrow(LauncherSettings.Favorites.CUSTOM_ICON);
    iconPackageIndex =
    ...
    profileIdIndex =
        getColumnIndexOrThrow(LauncherSettings.Favorites.PROFILE_ID);
  ...
  protected boolean loadIcon(final ShortcutInfo info) {
    ...
    try (LauncherIcons li = LauncherIcons.obtain(mContext)) {
      li.createIconBitmap(BitmapFactory.decodeByteArray(data, 0, data.length))
          .applyTo(info);
      return true;
    } catch (Exception e) {
  ...
  public ShortcutInfo getRestoredItemInfo(final Intent intent) {
    ...
    info.contentDescription =
        mUserManager.getBadgedLabelForUser(info.title, info.user);
    info.itemType = itemType;
   ...
   */
  public ShortcutInfo getAppShortcutInfo(final Intent intent,
                                         final boolean allowMissingTarget,
    ...
    LauncherActivityInfo lai =
        LauncherAppsCompat.getInstance(mContext).resolveActivity(newIntent,
                                                                 user);
  ...
  /**
   * Returns a {@link ContentWriter} which can be used to update the current
   * item.
   */
  ...
  public void commitRestoredItems() {
    if (restoredRows.size() > 0) {
      ...
      mContext.getContentResolver().update(
          LauncherSettings.Favorites.CONTENT_URI, values,
          Utilities.createDbSelectionQuery(LauncherSettings.Favorites._ID,
                                           restoredRows),
          null);
    }
  ...
  /**
   * Adds the {@param info} to {@param dataModel} if it does not overlap with
   * any other item, otherwise marks it for deletion.
   */
  public void checkAndAddItem(final ItemInfo info,
   ...
   * items
   */
  protected boolean checkItemPlacement(final ItemInfo item,
    ...
    if (item.container == LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
      ...
      if (!FeatureFlags.NO_ALL_APPS_ICON &&
          !Utilities.getZimPrefs(mContext).getHideDockButton() &&
          !prefs.getDockHide() &&
      ...
      if (hotseatOccupancy != null) {
        ...
        final GridOccupancy occupancy =
            new GridOccupancy(hotseatSize, hotseatRows);
        occupancy.cells[hotseatX][hotseatY] = true;
        occupied.put((long)LauncherSettings.Favorites.CONTAINER_HOTSEAT,
                     occupancy);
        return true;
      }
    ...
    if (item.container == LauncherSettings.Favorites.CONTAINER_DESKTOP &&
            item.cellX < 0 ||
        item.cellY < 0 || item.cellX + item.spanX > countX ||
        item.cellY + item.spanY > countY) {
      Log.e(TAG, "Error loading shortcut " + item + " into cell (" +
                     containerIndex + "-" + item.screenId + ":" + item.cellX +
                     "," + item.cellY + ") out of screen bounds ( " + countX +
                     "x" + countY + ")");
      return false;
    }
    ...
    if (occupancy.isRegionVacant(item.cellX, item.cellY, item.spanX,
                                 item.spanY)) {
      ...
      Log.e(TAG, "Error loading shortcut " + item + " into cell (" +
                     containerIndex + "-" + item.screenId + ":" + item.cellX +
                     "," + item.cellX + "," + item.spanX + "," + item.spanY +
                     ") already occupied");
