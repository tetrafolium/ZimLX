...
/**
 * Helper for identifying when a stylus touches a view while the primary stylus
 * button is pressed. This can occur in {@value MotionEvent#ACTION_DOWN} or
 * {@value MotionEvent#ACTION_MOVE}.
 */
public class StylusEventHelper {
  ...
  /**
   * Constructs a helper for listening to stylus button presses and releases.
   * Ensure that {
   ...
   */
  public StylusEventHelper(final StylusButtonListener listener,
                           final View view) {
    mListener = listener;
    mView = view;
    if (mView != null) {
   ...
   */
  private static boolean isStylusButtonPressed(final MotionEvent event) {
   ...
   */
  public boolean inStylusButtonPressed() { return mIsButtonPressed; }

  /**
   * Implement this interface to receive callbacks for a stylus button press and
   * release.
   */
  public interface StylusButtonListener {
    ...
    /**
     * Called when the stylus button is released after a button press. This is
     * also called if the event is canceled or the stylus is lifted off the
     * screen.
     *
