...
public class NotificationMainView
    extends FrameLayout implements SwipeDetector.Listener {
  ...
  // This is used only to track the notification view, so that it can be
  // properly logged.
  ...
  public NotificationMainView(final Context context) { this(context, null, 0); }
  ...
  public NotificationMainView(final Context context, final AttributeSet attrs,
    ...
    mContentTranslateAnimator =
        ObjectAnimator.ofFloat(this, CONTENT_TRANSLATION, 0);
  }
  ...
  protected void onFinishInflate() {
    ...
    ColorDrawable colorBackground =
        (ColorDrawable)mTextAndBackground.getBackground();
    mBackgroundColor = Color.RED; // colorBackground.getColor();
    RippleDrawable rippleBackground = new RippleDrawable(
        ColorStateList.valueOf(Themes.getAttrColor(
            getContext(), android.R.attr.colorControlHighlight)),
  ...
  /**
   * Sets the content of this view, animating it after a new icon shifts up if
   * necessary.
   */
  public void applyNotificationInfo(final NotificationInfo mainNotification,
                                    final boolean animate) {
    mNotificationInfo = mainNotification;
    NotificationListener listener =
        NotificationListener.getInstanceIfConnected();
    if (listener != null) {
      listener.setNotificationsShown(
          new String[] {mNotificationInfo.notificationKey});
    }
    ...
    if (!TextUtils.isEmpty(title) && !TextUtils.isEmpty(text)) {
      ...
      mTitleView.setText(TextUtils.isEmpty(title) ? text.toString()
                                                  : title.toString());
      mTextView.setVisibility(GONE);
    }
    mIconView.setBackground(
        mNotificationInfo.getIconForBackground(getContext(), mBackgroundColor));
    if (mNotificationInfo.intent != null) {
      setOnClickListener(mNotificationInfo);
    }
    setContentTranslation(0);
    // Add a dummy ItemInfo so that logging populates the correct container and
    // item types instead of DEFAULT_CONTAINERTYPE and DEFAULT_ITEMTYPE,
    // respectively.
    setTag(NOTIFICATION_ITEM_INFO);
    if (animate) {
      ObjectAnimator.ofFloat(mTextAndBackground, ALPHA, 0, 1)
          .setDuration(150)
          .start();
    }
  }
  ...
  public NotificationInfo getNotificationInfo() { return mNotificationInfo; }
  ...
  public void onChildDismissed() {
    ...
    launcher.getUserEventDispatcher().logActionOnItem(
        ...
        LauncherLogProto.Action.Direction
            .RIGHT, // Assume all swipes are right for logging.
  ...
  public boolean onDrag(final float displacement, final float velocity) {
    setContentTranslation(
        canChildBeDismissed()
            ? displacement
            : OverScroll.dampedScroll(displacement, getWidth()));
  ...
  public void onDragEnd(final float velocity, final boolean fling) {
    ...
    long duration = SwipeDetector.calculateDuration(
        velocity, (endTranslation - startTranslation) / getWidth());

    mContentTranslateAnimator.removeAllListeners();
    mContentTranslateAnimator.setDuration(duration).setInterpolator(
