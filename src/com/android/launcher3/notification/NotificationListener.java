 ...
 * {@link NotificationsChangedListener} when notifications are posted or
 * canceled, as well and when this service first connects. An instance of
 * NotificationListener, and its methods for getting notifications, can be
 * obtained via {@link #getInstanceIfConnected()}.
 */
...
private static StatusBarNotificationsChangedListener
        sStatusBarNotificationsChangedListener;
 ...
 */
private final Map<String, NotificationGroup> mNotificationGroupMap =
	new HashMap<>();
/**
...
private final Handler.Callback mWorkerCallback = new Handler.Callback() {
	...
	public boolean handleMessage(final Message message) {
		...
		case MSG_NOTIFICATION_FULL_REFRESH:
			...
			mUiHandler.obtainMessage(message.what, activeNotifications)
			.sendToTarget();
			break;
		}
		return true;
	...
	public boolean handleMessage(final Message message) {
		...
		case MSG_NOTIFICATION_REMOVED:
			if (sNotificationsChangedListener != null) {
				...
				sNotificationsChangedListener.onNotificationRemoved(pair.first,
				                                                    pair.second);
			}
			break;
		case MSG_NOTIFICATION_FULL_REFRESH:
...
public static void
setNotificationsChangedListener(final NotificationsChangedListener listener) {
...
public void onNotificationPosted(final StatusBarNotification sbn) {
	...
	if (sbn == null) {
		// There is a bug in platform where we can get a null notification; just
		// ignore it.
		return;
	}
	mWorkerHandler
...
public void onNotificationRemoved(final StatusBarNotification sbn) {
	...
	mWorkerHandler
	.obtainMessage(MSG_NOTIFICATION_REMOVED,
	               packageUserKeyAndNotificationKey)
	.sendToTarget();
	if (sStatusBarNotificationsChangedListener != null) {
	...
	NotificationGroup notificationGroup =
		mNotificationGroupMap.get(sbn.getGroupKey());
	String key = sbn.getKey();
	if (notificationGroup != null) {
		notificationGroup.removeChildKey(key);
		if (notificationGroup.isEmpty()) {
			if (key.equals(mLastKeyDismissedByLauncher)) {
				// Only cancel the group notification if launcher dismissed the last
				// child.
...
private void updateGroupKeyIfNecessary(final StatusBarNotification sbn) {
	...
	if (oldGroupKey == null || !oldGroupKey.equals(newGroupKey)) {
		...
		if (oldGroupKey != null &&
		    mNotificationGroupMap.containsKey(oldGroupKey)) {
	...
	if (sbn.isGroup() && newGroupKey != null) {
		// Maintain group info so we can cancel the summary when the last child is
		// canceled.
		NotificationGroup notificationGroup =
			mNotificationGroupMap.get(newGroupKey);
		if (notificationGroup == null) {
 ...
 * background thread.
 */
public List<StatusBarNotification>
getNotificationsForKeys(final List<NotificationKeyData> keys) {
	StatusBarNotification[] notifications =
		NotificationListener.this.getActiveNotifications(
			NotificationKeyData.extractKeysOnly(keys).toArray(
...
filterNotifications(final StatusBarNotification[] notifications) {
...
void onNotificationRemoved(PackageUserKey removedPackageUserKey,
                           NotificationKeyData notificationKey);
void
onNotificationFullRefresh(List<StatusBarNotification> activeNotifications);
}
