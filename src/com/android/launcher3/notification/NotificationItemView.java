...
public class NotificationItemView {
  ...
  public NotificationItemView(final PopupContainerWithArrow container) {
    ...
    mSwipeDetector.setDetectableScrollConditions(SwipeDetector.DIRECTION_BOTH,
                                                 false);
  ...
  public void addGutter() {
    if (mGutter == null) {
      mGutter =
          mContainer.inflateAndAdd(R.layout.notification_gutter, mContainer);
    }
  }
  ...
  public void updateHeader(final int notificationCount, final int iconColor) {
    mHeaderCount.setText(
        notificationCount <= 1 ? "" : String.valueOf(notificationCount));
    if (Color.alpha(iconColor) > 0) {
      if (mNotificationHeaderTextColor == Notification.COLOR_DEFAULT) {
        mNotificationHeaderTextColor = IconPalette.resolveContrastColor(
            mContext, iconColor,
