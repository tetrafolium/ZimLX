...
public abstract class PagedView<T extends View & PageIndicator>
  ...
  // versions will be assigned to the corresponding member variables below.
  ...
  @ViewDebug.ExportedProperty(category = "launcher") protected int mCurrentPage;
  ...
  // range. Otherwise it is equal to the scaled overscroll position. We use a
  // separate value so as to prevent the screens from continuing to translate
  ...
  public PagedView(final Context context) { this(context, null); }

  public PagedView(final Context context, final AttributeSet attrs) {
  ...
  public PagedView(final Context context, final AttributeSet attrs,
    ...
    TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PagedView,
                                                  defStyle, 0);
    mPageIndicatorViewId =
        a.getResourceId(R.styleable.PagedView_pageIndicator, -1);
    a.recycle();
  ...
  protected void setDefaultInterpolator(final Interpolator interpolator) {
    mDefaultInterpolator = interpolator;
    mScroller.setInterpolator(mDefaultInterpolator);
  }

  public void initParentViews(final View parent) {
  ...
  public T getPageIndicator() { return mPageIndicator; }

  /**
   * Returns the index of the currently displayed page. When in free scroll
   * mode, this is the page that the user was on before entering free scroll
   * mode (e.g. the home screen page they long-pressed on to enter the
   * overview). Try using {@link #getPageNearestToCenterOfScreen()} to get the
   * page the user is currently scrolling over.
   */
  ...
  public View getPageAt(final int index) { return getChildAt(index); }

  protected int indexToPage(final int index) { return index; }
  ...
  public void abortScrollerAnimation(final boolean resetNextPage) {
  ...
  private void forceFinishScroller(final boolean resetNextPage) {
  ...
  private int validateNewPage(final int newPage) {
  ...
  /**
   * Should be called whenever the page changes. In the case of a scroll, we
  ...
  protected boolean isPageInTransition() { return mIsPageInTransition; }
   ...
   * override this to provide custom behavior during animation.
   */
  protected void onPageEndTransition() { mWasInOverscroll = false; }

  protected int getUnboundedScrollX() { return mUnboundedScrollX; }
  ...
  public void scrollTo(final int x, final int y) {
    ...
    if (mFreeScroll) {
      // If the scroller is trying to move to a location beyond the maximum
      // allowed in the free scroll mode, we make sure to end the scroll
      // operation.
  ...
  // reuse it
  protected boolean computeScrollHelper() { return computeScrollHelper(true); }
  ...
  protected boolean computeScrollHelper(final boolean shouldInvalidate) {
  ...
  public int getExpectedHeight() { return getMeasuredHeight(); }
  ...
  public int getExpectedWidth() { return getMeasuredWidth(); }
  ...
  protected void onMeasure(final int widthMeasureSpec,
    ...
    // that when we zoom out (and scale down), the view is still contained in
    // the parent
    ...
    // about extra space due to insets.
  ...
  protected void onLayout(final boolean changed, final int left, final int top,
    ...
    // If the transition is running defer updating max scroll, as some empty
    // pages could still be present, and a max scroll change could cause sudden
    // jumps in scroll.
      ...
      transition.addTransitionListener(
          new LayoutTransition.TransitionListener() {
            ...
            public void startTransition(
                final LayoutTransition transition, final ViewGroup container,
   ...
   * childCount
   */
  protected boolean getPageScrolls(final int[] outPageScrolls,
                                   final boolean layoutChildren,
                                   final ComputePageScrollsLogic scrollLogic) {
    final int childCount = getChildCount();
    ...
    final int verticalCenter =
        (getPaddingTop() + getMeasuredHeight() + mInsets.top - mInsets.bottom -
         getPaddingBottom()) /
        2;

    final int scrollOffsetLeft = mInsets.left + getPaddingLeft();
    final int scrollOffsetRight =
        getWidth() - getPaddingRight() - mInsets.right;
    boolean pageScrollChanged = false;

    for (int i = startIndex, childLeft = scrollOffsetLeft; i != endIndex;
         i += delta) {
      ...
      if (scrollLogic.shouldIncludeView(child)) {
        ...
        // In case the pages are of different width, align the page to left or
        // right edge based on the orientation.
        final int pageScroll =
            mIsRtl ? (childLeft - scrollOffsetLeft)
  ...
  protected int getChildGap() { return 0; }

  private void updateMaxScrollX() { mMaxScrollX = computeMaxScrollX(); }
  ...
  public void setPageSpacing(final int pageSpacing) {
  ...
  private void dispatchPageCountChanged() {
    ...
    // transforms / alphas in accordance with any scroll effects.
    invalidate();
  }
  ...
  protected int getChildOffset(final int index) {
   ...
   */
  @Override
  public void focusableViewAvailable(final View focused) {
  ...
  @Override
  public void
  requestDisallowInterceptTouchEvent(final boolean disallowIntercept) {
  ...
  private boolean isTouchPointInViewportWithBuffer(final int x, final int y) {
    sTmpRect.set(-getMeasuredWidth() / 2, 0, 3 * getMeasuredWidth() / 2,
                 getMeasuredHeight());
  ...
  public boolean onInterceptTouchEvent(final MotionEvent ev) {
    ...
    case MotionEvent.ACTION_DOWN: {
      ...
      if (finishedScrolling) {
        ...
        if (isTouchPointInViewportWithBuffer((int)mDownMotionX,
                                             (int)mDownMotionY)) {
          mTouchState = TOUCH_STATE_SCROLLING;
        } else {
          mTouchState = TOUCH_STATE_REST;
  ...
  public boolean isHandlingTouch() { return mTouchState != TOUCH_STATE_REST; }
  ...
  protected float getScrollProgress(final int screenCenter, final View v,
                                    final int page) {
  ...
  public int getScrollForPage(final int index) {
  ...
  // its baseline position. This method returns the magnitude of this stray at
  // any given time.
  public int getLayoutTransitionOffsetForPage(final int index) {
    if (mPageScrolls == null || index >= mPageScrolls.length || index < 0) {
      return 0;
    } else {
  ...
  protected void dampedOverScroll(final float amount) {
  ...
  protected void overScroll(final float amount) { dampedOverScroll(amount); }

  protected void enableFreeScroll(final boolean settleOnPageInFreeScroll) {
    setEnableFreeScroll(true);
    mSettleOnPageInFreeScroll = settleOnPageInFreeScroll;
  }

  private void setEnableFreeScroll(final boolean freeScroll) {
  ...
  protected void setEnableOverscroll(final boolean enable) {
  ...
  public boolean onTouchEvent(final MotionEvent ev) {
    ...
    case MotionEvent.ACTION_MOVE:
      if (mTouchState == TOUCH_STATE_SCROLLING) {
        ...
        // Only scroll and update mLastMotionX if we have moved some discrete
        // amount.  We keep the remainder because we are actually testing if
        // we've moved from the last scrolled position (which is discrete).
    ...
    case MotionEvent.ACTION_UP:
      if (mTouchState == TOUCH_STATE_SCROLLING) {
        ...
        boolean isSignificantMove =
            Math.abs(deltaX) > pageWidth * SIGNIFICANT_MOVE_THRESHOLD;
        ...
        if (!mFreeScroll) {
          // In the case that the page is moved far to one direction and then is
          // flung in the opposite direction, we use a threshold to determine
          // whether we should just return to the starting page, or if we should
          // skip one further.
          boolean returnToOriginalPage = false;
          if (Math.abs(deltaX) >
                  pageWidth * RETURN_TO_ORIGINAL_PAGE_THRESHOLD &&
          ...
          // short-circuit our test for a large move if a fling has been
          // registered. That is, a large move to the left and fling to the
          // right will register as a fling to the right.
          ...
          mScroller.setInterpolator(mDefaultInterpolator);
          mScroller.fling(initialScrollX, getScrollY(), vX, 0,
          ...
          int lastPageScroll =
              getScrollForPage(!mIsRtl ? getPageCount() - 1 : 0);
          if (mSettleOnPageInFreeScroll && unscaledScrollX > 0 &&
              unscaledScrollX < mMaxScrollX) {
            // If scrolling ends in the half of the added space that is closer
            // to the end, settle to the end. Otherwise snap to the nearest
            // page. If flinging past one of the ends, don't change the velocity
            // as it will get stopped at the end anyway.
            final int finalX =
                unscaledScrollX < firstPageScroll / 2
                    ? 0
  ...
  public boolean onGenericMotionEvent(final MotionEvent event) {
    if ((event.getSource() & InputDevice.SOURCE_CLASS_POINTER) != 0) {
      ...
      case MotionEvent.ACTION_SCROLL: {
        // Handle mouse (or ext. device) by shifting the page depending on the
        // scroll
        final float vscroll;
        final float hscroll;
  ...
  private void acquireVelocityTrackerAndAddMovement(final MotionEvent ev) {
  ...
  private int getPageNearestToCenterOfScreen(final int scaledScrollX) {
  ...
  private float distanceInfluenceForSnapDuration(final float f) {
    f -= 0.5f; // center the values about 0.
    f *= 0.3f * Math.PI / 2.0f;
    return (float)Math.sin(f);
  }

  protected boolean snapToPageWithVelocity(final int whichPage,
                                           final int velocity) {
    ...
    if (Math.abs(velocity) < mMinFlingVelocity) {
      // If the velocity is low enough, then treat this more as an automatic
      // page advance as opposed to an apparent physical response to flinging
    ...
    // overall snap duration. This is a function of the actual distance that
    // needs to be traveled; we keep this value close to half screen size in
    // order to reduce the variance in snap duration as a function of the
    // distance the page needs to travel.
    float distanceRatio =
        Math.min(1f, 1.0f * Math.abs(delta) / (2 * halfScreenSize));
    float distance =
        halfScreenSize +
        halfScreenSize * distanceInfluenceForSnapDuration(distanceRatio);
    ...
    // derivative of the scroll interpolator at zero, ie. 5. We use 4 to make it
    // a little slower.
  ...
  public boolean snapToPage(final int whichPage) {
    return snapToPage(whichPage, PAGE_SNAP_ANIMATION_DURATION);
  }

  public boolean snapToPageImmediately(final int whichPage) {
    return snapToPage(whichPage, PAGE_SNAP_ANIMATION_DURATION, true, null);
  }

  public boolean snapToPage(final int whichPage, final int duration) {
  ...
  protected boolean snapToPage(final int whichPage, final int duration,
                               final boolean immediate,
                               final TimeInterpolator interpolator) {
    whichPage = validateNewPage(whichPage);
  ...
  protected boolean snapToPage(final int whichPage, final int delta,
                               final int duration) {
    return snapToPage(whichPage, delta, duration, false, null);
  }

  protected boolean snapToPage(final int whichPage, final int delta,
                               final int duration, final boolean immediate,
                               final TimeInterpolator interpolator) {
    if (mFirstLayout) {
  ...
  public CharSequence getAccessibilityClassName() {
    ...
    // provide same accessibility info as ScrollView, inform the service to
    // handle use the same way.
    return ScrollView.class.getName();
  }

  protected boolean isPageOrderFlipped() { return false; }
  ...
  onInitializeAccessibilityNodeInfo(final AccessibilityNodeInfo info) {
    ...
    // Accessibility-wise, PagedView doesn't support long click, so disabling
    // it. Besides disabling the accessibility long-click, this also prevents
    // this view from getting accessibility focus.
  ...
  protected boolean canAnnouncePageDescription() { return true; }
  ...
  protected float getDownMotionX() { return mDownMotionX; }

  protected float getDownMotionY() { return mDownMotionY; }
  ...
  public int[] getVisibleChildrenRange() {
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (left <= visibleRight &&
          (left + child.getMeasuredWidth()) >= visibleLeft) {
        if (leftChild == -1) {
          leftChild = i;
        }
        rightChild = i;
  ...
  public boolean isScrollerFinished() { return mScroller.isFinished(); }
}
