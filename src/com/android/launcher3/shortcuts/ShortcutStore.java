...
public final Handler mWorkerThread =
	new Handler(LauncherModel.getWorkerLooper(), this);
...
public Store(final ShortcutKey shortcutKey,
...
public boolean handleMessage(final Message message) {
	if (message.what == 0) {
		List<ShortcutKey> keys =
			message.obj != null ? (List)message.obj : Collections.EMPTY_LIST;
...
private ShortcutInfo getInfo(final ShortcutKey shortcutKey) {
	...
	List queryForFullDetails =
		DeepShortcutManager.getInstance(mContext).queryForFullDetails(
			shortcutKey.componentName.getPackageName(),
			Collections.singletonList(shortcutKey.getId()), shortcutKey.user);
	if (queryForFullDetails.isEmpty()) {
		return null;
	}
	ShortcutInfo shortcutInfo = new ShortcutInfo(
		(ShortcutInfoCompat)queryForFullDetails.get(0), mContext);
	try {
		obtain = LauncherIcons.obtain(mContext);
		try {
			obtain
			.createShortcutIcon((ShortcutInfoCompat)queryForFullDetails.get(0),
			                    true, null)
...
public interface OnUpdateListener { void onShortcutsUpdated(); }
}
