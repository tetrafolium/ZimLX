...
public class ShortcutInfoCompatBackport extends ShortcutInfoCompat {
  ...
  public ShortcutInfoCompatBackport(final Context context,
                                    final Resources resources,
                                    final String packageName,
                                    final ComponentName activity,
                                    final XmlResourceParser parseXml)
    ...
    mEnabled = !xmlData.containsKey("enabled") ||
               xmlData.get("enabled").toLowerCase().equals("true");
    ...
    mShortLabel = xmlData.containsKey("shortcutShortLabel")
                      ? resources.getString(Integer.valueOf(
                            xmlData.get("shortcutShortLabel").substring(1)))
                      : "";

    mLongLabel = xmlData.containsKey("shortcutLongLabel")
                     ? resources.getString(Integer.valueOf(
                           xmlData.get("shortcutLongLabel").substring(1)))
                     : mShortLabel;

    mDisabledMessage =
        xmlData.containsKey("shortcutDisabledMessage")
            ? resources.getString(Integer.valueOf(
                  xmlData.get("shortcutDisabledMessage").substring(1)))
            : "";
    ...
    do {
      if (parseXml.nextToken() == XmlPullParser.START_TAG) {
        ...
        if (xmlName.equals("intent")) {
          ...
          extras.clear();
          for (int i = 0; i < parseXml.getAttributeCount(); i++) {
            xmlDataIntent.put(parseXml.getAttributeName(i),
                              parseXml.getAttributeValue(i));
          }
        } else if (xmlName.equals("extra")) {
          xmlDataExtras.clear();
          for (int i = 0; i < parseXml.getAttributeCount(); i++) {
            xmlDataExtras.put(parseXml.getAttributeName(i),
                              parseXml.getAttributeValue(i));
          }
          if (xmlDataExtras.containsKey("name") &&
              xmlDataExtras.containsKey("value")) {
    ...
    String targetPackage =
        useTargetPackage ? xmlDataIntent.get("targetPackage") : mPackageName;
    ...
    if (xmlDataIntent.containsKey("targetClass")) {
      mIntent.setComponent(
          new ComponentName(targetPackage, xmlDataIntent.get("targetClass")));
    }
  ...
  public Drawable getIcon(final int density) {
    try {
      ...
      return mContext.getResources().getDrawableForDensity(
