...
/**
 * An extension of {@link GridSizeMigrationTask} which migrates only one screen
 * and deletes all carry-forward items.
 */
public class LossyScreenMigrationTask extends GridSizeMigrationTask {
  ...
  protected LossyScreenMigrationTask(final Context context,
  ...
  protected Cursor queryWorkspace(final String[] columns, final String where) {
    return mDb.query(Favorites.TABLE_NAME, columns, where, null, null, null,
                     null);
  }
  ...
  public void migrateScreen0() {
    ...
    if (!mEntryToRemove.isEmpty()) {
      mDb.delete(
          ...
          Utilities.createDbSelectionQuery(Favorites._ID, mEntryToRemove),
          null);
    }
  }
}
