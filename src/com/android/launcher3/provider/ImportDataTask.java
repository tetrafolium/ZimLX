...
/**
 * Utility class to import data from another Launcher which is based on
 * Launcher3 schema.
 */
public class ImportDataTask {

public static final String KEY_DATA_IMPORT_SRC_PKG = "data_import_src_pkg";
public static final String KEY_DATA_IMPORT_SRC_AUTHORITY =
	"data_import_src_authority";
...
private ImportDataTask(final Context context, final String sourceAuthority) {
	mContext = context;
	mOtherScreensUri = Uri.parse("content://" + sourceAuthority + "/" +
	                             WorkspaceScreens.TABLE_NAME);
	mOtherFavoritesUri =
		Uri.parse("content://" + sourceAuthority + "/" + Favorites.TABLE_NAME);
}
...
throws Exception {
	...
	String sourceAuthority =
		devicePrefs.getString(KEY_DATA_IMPORT_SRC_AUTHORITY, "");

	if (TextUtils.isEmpty(sourcePackage) ||
	    TextUtils.isEmpty(sourceAuthority)) {
		return false;
	}

	// Synchronously clear the migration flags. This ensures that we do not try
	// migration again and thus prevents potential crash loops due to migration
	// failure.
	devicePrefs.edit()
	...
	if (!Settings
	    .call(context.getContentResolver(),
	          Settings.METHOD_WAS_EMPTY_DB_CREATED)
...
public boolean importWorkspace() throws Exception {
	ArrayList<Long> allScreens = LauncherDbUtils.getScreenIdsFromCursor(
		mContext.getContentResolver().query(
			mOtherScreensUri, null, null, null,
	...
	for (int i = 0; i < count; i++) {
		...
		screenOps.add(
			...
			.newInsert(LauncherSettings.WorkspaceScreens.CONTENT_URI)
			.withValues(v)
			.build());
	}
	...
	GridSizeMigrationTask.markForMigration(mContext, mMaxGridSizeX,
	                                       mMaxGridSizeY, mHotseatSize);
 ...
 * 2) For home screen entries, maps the screen id based on {@param
 * screenIdMap} 3) In the end fills any holes in hotseat with items from
 * default hotseat layout.
 */
private void importWorkspaceItems(final long firsetScreenId,
                                  final LongSparseArray<Long> screenIdMap)
throws Exception {
	String profileId = Long.toString(
		UserManagerCompat.getInstance(mContext).getSerialNumberForUser(
	...
	if (FeatureFlags.QSB_ON_FIRST_SCREEN) {
		try (
			Cursor c = mContext.getContentResolver().query(
				mOtherFavoritesUri, null,
				// get items on the first row of the first screen
				"profileId = ? AND container = -100 AND screen = ? AND cellY = 0",
				new String[] {profileId, Long.toString(firsetScreenId)}, null)) {
			// First row of first screen is not empty
			createEmptyRowOnFirstScreen = c.moveToNext();
		}
	}

	ArrayList<ContentProviderOperation> insertOperations =
		new ArrayList<>(BATCH_INSERT_SIZE);
	...
	try (Cursor c = mContext.getContentResolver().query(
		     ...
		     // Get the items sorted by container, so that the folders are
		     // loaded before the corresponding items.
		     Favorites.CONTAINER)) {
		...
		final int iconPackageIndex =
			c.getColumnIndexOrThrow(Favorites.ICON_PACKAGE);
		final int iconResourceIndex =
			c.getColumnIndexOrThrow(Favorites.ICON_RESOURCE);
		...
		while (c.moveToNext()) {
			...
			case Favorites.CONTAINER_DESKTOP: {
				...
				screen = newScreenId;
				if (createEmptyRowOnFirstScreen &&
			...
			default:
				if (!mValidFolders.get(container)) {
					FileLog.d(TAG,
					          String.format(
							  "Skipping item %d, type %d not in a valid folder %d",
							  id, type, container));
					continue;
				}
			}
			...
			case Favorites.ITEM_TYPE_APPWIDGET: {
				...
				values.put(Favorites.APPWIDGET_PROVIDER,
				           c.getString(widgetProviderIndex));
				break;
			}
			...
			default:
				FileLog.d(TAG, String.format("Skipping item %d, not a valid type %d",
				                             id, type));
				continue;
			}
			...
			if (container == Favorites.CONTAINER_HOTSEAT) {
				if (intent == null) {
					FileLog.d(TAG, String.format(
							  "Skipping item %d, null intent on hotseat", id));
					continue;
				}
			...
			insertOperations.add(
				ContentProviderOperation.newInsert(Favorites.CONTENT_URI)
				.withValues(values)
				.build());
			if (container < 0) {
	...
	FileLog.d(TAG,
	          totalItemsOnWorkspace + " items imported from external source");
	...
	LongArrayMap<Object> hotseatItems =
		GridSizeMigrationTask.removeBrokenHotseatItems(mContext);
	...
	if (hotseatItems.size() < myHotseatCount) {
		...
		HotseatParserCallback parserCallback = new HotseatParserCallback(
			...
			myHotseatCount);
		new HotseatLayoutParser(mContext, parserCallback)
		.loadLayout(null, new ArrayList<Long>());
...
HotseatParserCallback(final HashSet<String> existingApps,
                      final LongArrayMap<Object> existingItems,
                      final ArrayList<ContentProviderOperation> outOps,
                      final int startItemId, final int requiredSize) {
...
public long insertAndCheck(final SQLiteDatabase db,
	...
	values.put(Favorites.SCREEN, screen);
	mOutOps.add(ContentProviderOperation.newInsert(Favorites.CONTENT_URI)
	            .withValues(values)
	            .build());
	return 0;
}
