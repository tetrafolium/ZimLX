 ...
 * immediately after restore. This helps keep the model consistent if the
 * launcher updates between restore and first startup.
 */
public class RestoreDbTask {
   ...
   * cannot be restored.
   * 2. Marks all entries as restored. The flags are updated during first load
   * or as the restored apps get installed.
   * 3. If the user serial for primary profile is different than that of the
   * previous device, update the entries to the new profile id.
   */
  private void sanitizeDB(final DatabaseHelper helper, final SQLiteDatabase db)
      throws Exception {
    long oldProfileId = getDefaultProfileId(db);
    // Delete all entries which do not belong to the main user
    int itemsDeleted = db.delete(Favorites.TABLE_NAME, "profileId != ?",
                                 new String[] {Long.toString(oldProfileId)});
    if (itemsDeleted > 0) {
      FileLog.d(TAG, itemsDeleted +
                         " items belonging to a managed profile, were deleted");
    }

    // Mark all items as restored.
    boolean keepAllIcons =
        Utilities.isPropertyEnabled(LogConfig.KEEP_ALL_ICONS);
    ContentValues values = new ContentValues();
    values.put(Favorites.RESTORED,
               ShortcutInfo.FLAG_RESTORED_ICON |
    ...
    if (Utilities.longCompare(oldProfileId, myProfileId) != 0) {
      FileLog.d(TAG, "Changing primary user id from " + oldProfileId + " to " +
                         myProfileId);
  ...
  /**
   * Updates profile id of all entries and changes the default value for the
   * column.
   */
  protected void migrateProfileId(final SQLiteDatabase db,
                                  final long newProfileId) {
  ...
  protected long getDefaultProfileId(final SQLiteDatabase db) throws Exception {
    try (Cursor c = db.rawQuery("PRAGMA table_info (favorites)", null)) {
      ...
      throw new InvalidObjectException(
          "Table does not have a profile id column");
    }
  }
}
