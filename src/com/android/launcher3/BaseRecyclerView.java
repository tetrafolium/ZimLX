 ...
 * <ul>
 *   <li> NOT intercept a touch unless the scrolling velocity is below a
 * predefined threshold. <li> Enable fast scroller.
 * </ul>
 */
public abstract class BaseRecyclerView extends RecyclerView {
  ...
  public BaseRecyclerView(final Context context) { this(context, null); }
  ...
  public void bindFastScrollbar() {
    ...
    mScrollbar.setRecyclerView(this,
                               parent.findViewById(R.id.fast_scroller_popup));
    onUpdateScrollbar(0);
  }

  public RecyclerViewFastScroller getScrollbar() { return mScrollbar; }

  public int getScrollBarTop() { return getPaddingTop(); }
   ...
   *   AvailableScrollBarHeight = Total height of the visible view - thumb
   * height
   */
  protected int getAvailableScrollBarHeight() {
    int availableScrollBarHeight =
        getScrollbarTrackHeight() - mScrollbar.getThumbHeight();
    return availableScrollBarHeight;
  }

  /**
   * Updates the scrollbar thumb offset to match the visible scroll of the
   * recycler view.  It does this by mapping the available scroll area of the
   * recycler view to the available space for the scroll bar.
   *
   * @param scrollY the current scroll y
   */
  protected void
  synchronizeScrollBarThumbOffsetToViewScroll(final int scrollY,
    ...
    // accounts for the view padding, while the scrollBarY is drawn right up to
    // the background padding (ignoring padding)
  ...
  public boolean shouldContainerScroll(final MotionEvent ev,
                                       final View eventSource) {
    int[] point = new int[2];
    point[0] = (int)ev.getX();
    point[1] = (int)ev.getY();
    Utilities.mapCoordInSelfToDescendant(mScrollbar, eventSource, point);
    // IF the MotionEvent is inside the thumb, container should not be pulled
    // down.
    ...
    // probably not enough items to scroll, THEN it's okay for the container to
    // be pulled down.
   ...
   */
  public boolean supportsFastScrolling() { return true; }
