 ...
 */
private static Pair<Integer, WallpaperColorsCompat>
parseValue(final String value) {
	String[] parts = value.split(",");
	Integer wallpaperId = Integer.parseInt(parts[1]);
	if (parts.length == 2) {
		// There is no wallpaper color info present, eg when live wallpaper has no
		// preview.
	...
	return Pair.create(wallpaperId, new WallpaperColorsCompat(
				   primary, secondary, tertiary, hints));
}
...
private void reloadColors() {
	JobInfo job =
		new JobInfo
		.Builder(Utilities.WALLPAPER_COMPAT_JOB_ID,
...
public static class ColorExtractionService
	extends JobService implements Runnable {
...
private static int calculateDarkHints(final Bitmap source) {
	...
	int darkPixels = 0;
	source.getPixels(pixels, 0 /* offset */, source.getWidth(), 0 /* x */,
	                 0 /* y */, source.getWidth(), source.getHeight());
...
/**
 * Extracts the wallpaper colors and sends the result back through the
 * receiver.
 */
@Override
public void run() {
	...
	if (info != null) {
		...
		if (Utilities.ATLEAST_NOUGAT) {
			try (ParcelFileDescriptor fd = wm.getWallpaperFile(FLAG_SYSTEM)) {
				...
				Rect region =
					new Rect(0, 0, decoder.getWidth(), decoder.getHeight());
	...
	if (drawable != null && drawable.getIntrinsicWidth() > 0 &&
		...
		final int requestedArea =
			drawable.getIntrinsicWidth() * drawable.getIntrinsicHeight();
		double scale = 1;
		if (requestedArea > MAX_WALLPAPER_EXTRACTION_AREA) {
			scale =
				Math.sqrt(MAX_WALLPAPER_EXTRACTION_AREA / (double)requestedArea);
		}
		bitmap =
			Bitmap.createBitmap((int)(drawable.getIntrinsicWidth() * scale),
	...
	if (bitmap != null) {
		...
		for (Palette.Swatch swatch : palette.getSwatches()) {
			colorsToOccurrences.add(
				new Pair(swatch.getRgb(), swatch.getPopulation()));
		}

		Collections.sort(colorsToOccurrences,
		                 new Comparator<Pair<Integer, Integer> >() {
					@Override
					public int compare(final Pair<Integer, Integer> a,
