...
public class AlphabeticIndexCompat {
  ...
  public AlphabeticIndexCompat(final Context context) {
    ...
    if (context.getResources().getConfiguration().locale.getLanguage().equals(
  ...
  public String computeSectionName(final CharSequence cs) {
    ...
    String sectionName =
        mBaseIndex.getBucketLabel(mBaseIndex.getBucketIndex(s));
  ...
  /**
   * Base class to support Alphabetic indexing if not supported by the
   * framework.
  ...
  /**
   * Reflected libcore.icu.AlphabeticIndex implementation, falls back to the
   * base alphabetic index.
   */
  private static class AlphabeticIndexV16 extends BaseIndex {
    ...
    public AlphabeticIndexV16(final Context context) throws Exception {
      ...
      mGetBucketIndexMethod =
          clazz.getDeclaredMethod("getBucketIndex", String.class);
      mGetBucketLabelMethod =
          clazz.getDeclaredMethod("getBucketLabel", int.class);
      mAlphabeticIndex =
          clazz.getConstructor(Locale.class).newInstance(curLocale);
