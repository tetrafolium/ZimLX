...
public class AutoInstallsLayout {
  /**
   * Marker action used to discover a package which defines launcher
   * customization
   */
   ...
   */
  private static final String FORMATTED_LAYOUT_RES_WITH_HOSTEAT =
      "default_layout_%dx%d_h%s";
  ...
  // the last row / column, -2 represents the second last, and so on.
  ...
  public AutoInstallsLayout(final Context context,
                            final AppWidgetHost appWidgetHost,
                            final LayoutParserCallback callback,
                            final Resources res, final int layoutId,
  ...
  static AutoInstallsLayout get(final Context context,
                                final AppWidgetHost appWidgetHost,
                                final LayoutParserCallback callback) {
  ...
  static AutoInstallsLayout get(final Context context, final String pkg,
    ...
    String layoutName =
        String.format(Locale.ENGLISH, FORMATTED_LAYOUT_RES_WITH_HOSTEAT,
    ...
    if (layoutId == 0) {
      Log.d(TAG, "Formatted layout: " + layoutName +
                     " not found. Trying the default layout");
    ...
    return new AutoInstallsLayout(context, appWidgetHost, callback, targetRes,
                                  layoutId, TAG_WORKSPACE);
  }

  protected static void beginDocument(final XmlPullParser parser,
  ...
  private static String convertToDistanceFromEnd(final String value,
                                                 final int endValue) {
   ...
   */
  protected static String getAttributeValue(final XmlResourceParser parser,
   ...
   */
  protected static int getAttributeResourceValue(final XmlResourceParser parser,
                                                 final String attribute,
                                                 final int defaultValue) {
    int value = parser.getAttributeResourceValue(
        "http://schemas.android.com/apk/res-auto", attribute, defaultValue);
  ...
  @Thunk
  static void copyInteger(final ContentValues from, final ContentValues to,
                          final String key) {
   ...
   * desktop.
   */
  public int loadLayout(final SQLiteDatabase db,
                        final ArrayList<Long> screenIds) {
    mDb = db;
    try {
  ...
  /**
   * Parses the layout and returns the number of elements added on the
   * homescreen.
   */
  protected int parseLayout(final int layoutId, final ArrayList<Long> screenIds)
      throws XmlPullParserException, IOException {
  ...
  /**
   * Parses container and screenId attribute from the current tag, and puts it
   * in the out.
   *
   * @param out array of size 2.
   */
  protected void parseContainerAndScreen(final XmlResourceParser parser,
                                         final long[] out) {
    if (HOTSEAT_CONTAINER_NAME.equals(
            getAttributeValue(parser, ATTR_CONTAINER))) {
      out[0] = Favorites.CONTAINER_HOTSEAT;
      // Hack: hotseat items are stored using screen ids
      long rank = Long.parseLong(getAttributeValue(parser, ATTR_RANK));
      out[1] =
          (FeatureFlags.NO_ALL_APPS_ICON || rank < mIdp.getAllAppsButtonRank())
              ? rank
              : (rank + 1);
    } else {
   ...
   */
  protected int parseAndAddNode(final XmlResourceParser parser,
                                final ArrayMap<String, TagParser> tagParserMap,
                                final ArrayList<Long> screenIds)
      throws XmlPullParserException, IOException {
    ...
    mValues.put(Favorites.CELLX,
                convertToDistanceFromEnd(getAttributeValue(parser, ATTR_X),
                                         mColumnCount));
    mValues.put(
        Favorites.CELLY,
  ...
  protected long addShortcut(final String title, final Intent intent,
                             final int type) {
  ...
  protected class AppShortcutParser implements TagParser {
    ...
    @Override
    public long parseAndAdd(final XmlResourceParser parser) {
     ...
     */
    protected long invalidPackageOrClass(final XmlResourceParser parser) {
    ...
    public long parseAndAdd(final XmlResourceParser parser) {
      ...
      final Intent intent =
          new Intent(Intent.ACTION_MAIN, null)
              ...
              .setFlags(Intent.FLAG_ACTIVITY_NEW_TASK |
                        Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
      return addShortcut(mContext.getString(R.string.package_state_unknown),
  ...
  protected class ShortcutParser implements TagParser {
    ...
    public ShortcutParser(final Resources iconRes) { mIconRes = iconRes; }
    ...
    public long parseAndAdd(final XmlResourceParser parser) {
      ...
      mValues.put(LauncherSettings.Favorites.ICON,
                  Utilities.flattenBitmap(
                      li.createBadgedIconBitmap(icon, Process.myUserHandle(),
                                                Build.VERSION.SDK_INT)
                          .icon));
      li.recycle();

      mValues.put(Favorites.ICON_PACKAGE,
                  mIconRes.getResourcePackageName(iconId));
      mValues.put(Favorites.ICON_RESOURCE, mIconRes.getResourceName(iconId));

      intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK |
                      Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
      return addShortcut(mSourceRes.getString(titleResId), intent,
                         Favorites.ITEM_TYPE_SHORTCUT);
    }

    protected Intent parseIntent(final XmlResourceParser parser) {
   ...
   * spanY. Options child nodes: <extra key=... value=... /> It adds a pending
   * widget which allows the widget to come later. If there are extras, those
   * are passed to widget options during bind.
   * The config activity for the widget (if present) is not shown, so any
   * optional configurations should be passed as extras and the widget should
   * support reading these widget options.
   */
  protected class PendingWidgetParser implements TagParser {
    ...
    protected long verifyAndInsert(final ComponentName cn,
                                   final Bundle extras) {
  ...
  protected class FolderParser implements TagParser {
    ...
    public FolderParser() { this(getFolderElementsMap()); }

    public FolderParser(final ArrayMap<String, TagParser> elements) {
