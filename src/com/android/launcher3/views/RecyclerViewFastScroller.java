...
public class RecyclerViewFastScroller extends View {
  ...
  private static final Property<RecyclerViewFastScroller, Integer> TRACK_WIDTH =
      new Property<RecyclerViewFastScroller, Integer>(Integer.class, "width") {
        ...
        @Override
        public void set(final RecyclerViewFastScroller scrollBar,
                        final Integer value) {
  ...
  // This is the offset from the top of the scrollbar when the user first starts
  // touching.  To prevent jumping, this offset is applied as the user scrolls.
  ...
  public RecyclerViewFastScroller(final Context context,
    ...
    mWidth = mMinWidth =
        res.getDimensionPixelSize(R.dimen.fastscroll_track_min_width);
    ...
    mDeltaThreshold =
        res.getDisplayMetrics().density * SCROLL_DELTA_THRESHOLD_DP;

    TypedArray ta = context.obtainStyledAttributes(
        attrs, R.styleable.RecyclerViewFastScroller, defStyleAttr, 0);
    mCanThumbDetach = ta.getBoolean(
        R.styleable.RecyclerViewFastScroller_canThumbDetach, false);
    ta.recycle();
  }

  public void setRecyclerView(final BaseRecyclerView rv,
                              final TextView popupView) {
    ...
    mRv.addOnScrollListener(
        mOnScrollListener = new RecyclerView.OnScrollListener() {
          ...
          public void onScrolled(final RecyclerView recyclerView, final int dx,
            ...
            // TODO(winsonc): If we want to animate the section heads while
            // scrolling, we can
            //                initiate that here if the recycler view scroll
            //                state is not RecyclerView.SCROLL_STATE_IDLE.
    ...
    mPopupView.setBackground(new FastScrollThumbDrawable(
        mThumbPaint, Utilities.isRtl(getResources())));
  }

  public void reattachThumbToScroll() { mIsThumbDetached = false; }
  ...
  public int getThumbOffsetY() { return mThumbOffsetY; }
  ...
  public int getThumbHeight() { return mThumbHeight; }

  public boolean isDraggingThumb() { return mIsDragging; }

  public boolean isThumbDetached() { return mIsThumbDetached; }

  /**
   * Handles the touch event and determines whether to show the fast scroller
   * (or updates it if it is already showing).
   */
  public boolean handleTouchEvent(final MotionEvent ev, final Point offset) {
    ...
    case MotionEvent.ACTION_DOWN:
      ...
      if ((Math.abs(mDy) < mDeltaThreshold &&
        ...
        // now the touch events are being passed to the {@link WidgetCell} until
        // the touch sequence goes over the touch slop.
        mRv.stopScroll();
      }
      if (isNearThumb(x, y)) {
        mTouchOffsetY = mDownY - mThumbOffsetY;
      } else if (FeatureFlags.LAUNCHER3_DIRECT_SCROLL &&
                 mRv.supportsFastScrolling() && isNearScrollBar(mDownX)) {
    ...
    case MotionEvent.ACTION_MOVE:
      ...
      // if we have exceeded some fixed movement
      mIgnoreDragGesture |=
          Math.abs(y - mDownY) > mConfig.getScaledPagingTouchSlop();
  ...
  private void calcTouchOffsetAndPrepToFastScroll(final int downY,
                                                  final int lastY) {
    mIsDragging = true;
    if (mCanThumbDetach) {
  ...
  private void updateFastScrollSectionNameAndThumbOffset(final int lastY,
                                                         final int y) {
  ...
  private void updatePopupY(final int lastTouchY) {
    ...
    float top = lastTouchY - (FAST_SCROLL_OVERLAY_Y_OFFSET_FACTOR * height) +
                mRv.getScrollBarTop();
    top = Utilities.boundToRange(
        top, mMaxWidth, mRv.getScrollbarTrackHeight() - mMaxWidth - height);
  ...
  public boolean isHitInParent(final float x, final float y,
                               final Point outOffset) {
    if (mThumbOffsetY < 0) {
      return false;
    }
  ...
  public boolean hasOverlappingRendering() {
    // There is actually some overlap between the track and the thumb. But since
    // the track alpha is so low, it does not matter.
    return false;
  }
}
