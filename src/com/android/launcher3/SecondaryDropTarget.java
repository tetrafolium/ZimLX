 ...
 */
public class SecondaryDropTarget
    extends ButtonDropTarget implements OnAlarmListener {
  ...
  protected void setupUi(final int action) {
    ...
    if (action == UNINSTALL) {
      mHoverColor =
          getResources().getColor(R.color.uninstall_target_hover_tint);
  ...
  public Target getDropTargetForLogging() {
    ...
    t.controlType = mCurrentAccessibilityAction == UNINSTALL
                        ? ControlType.UNINSTALL_TARGET
  ...
  private ComponentName getUninstallTarget(final ItemInfo item) {
    ...
    if (intent != null) {
      LauncherActivityInfo info =
          LauncherAppsCompat.getInstance(mLauncher).resolveActivity(intent,
                                                                    user);
      if (info != null && (info.getApplicationInfo().flags &
  ...
  public void completeDrop(final DragObject d) {
    ComponentName target =
        performDropAction(getViewUnderDrag(d.dragInfo), d.dragInfo);
  ...
  /**
   * Verifies that the view is an reconfigurable widget and returns the
   * corresponding widget Id, otherwise return {@code INVALID_APPWIDGET_ID}
   */
  private int getReconfigurableWidgetId(final View view) {
    ...
    if ((LauncherAppWidgetProviderInfo
             .fromProviderInfo(getContext(), widgetInfo)
             .getWidgetFeatures() &
         WIDGET_FEATURE_RECONFIGURABLE) == 0) {
      return INVALID_APPWIDGET_ID;
    }
  ...
  /**
   * Performs the drop action and returns the target component for the
   * dragObject or null if the action was not performed.
   */
  protected ComponentName performDropAction(final View view,
    ...
    if (cn == null) {
      ...
      // explaining that. We may give them the option of disabling apps this
      // way.
      Toast
          .makeText(mLauncher, R.string.uninstall_system_app_text,
                    Toast.LENGTH_SHORT)
          .show();
      return null;
    }
    try {
      Intent i =
          Intent
              .parseUri(mLauncher.getString(R.string.delete_package_intent), 0)
              .setData(Uri.fromParts("package", cn.getPackageName(),
                                     cn.getClassName()))
  ...
  /**
   * A wrapper around {@link DragSource} which delays the {@link
   * #onDropCompleted} action until
  ...
  private class DeferredOnComplete implements DragSource, OnResumeCallback {
    ...
    public DeferredOnComplete(final DragSource original,
    ...
    public void onLauncherResume() {
      ...
      if (LauncherAppsCompat.getInstance(mContext).getApplicationInfo(
              mPackageName, PackageManager.MATCH_UNINSTALLED_PACKAGES,
