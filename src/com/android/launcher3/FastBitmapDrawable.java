...
public class FastBitmapDrawable extends Drawable {
  ...
  // Since we don't need 256^2 values for combinations of both the brightness
  // and saturation, we reduce the value space to a smaller value V, which
  // reduces the number of cached ColorMatrixColorFilters that we need to keep
  // to V^2
  ...
  private static final Property<FastBitmapDrawable, Float> SCALE =
      new Property<FastBitmapDrawable, Float>(Float.TYPE, "scale") {
        ...
        @Override
        public void set(final FastBitmapDrawable fastBitmapDrawable,
                        final Float value) {
  ...
  // The saturation and brightness are values that are mapped to
  // REDUCED_FILTER_VALUE_SPACE and as a result, can be used to compose the key
  // for the cached ColorMatrixColorFilters
  private int mDesaturation = 0;
  private int mBrightness = 0;
  private int mAlpha = 255;
  private int mPrevUpdateKey = Integer.MAX_VALUE;

  public FastBitmapDrawable(final Bitmap b) { this(b, Color.TRANSPARENT); }
  ...
  public void draw(final Canvas canvas) {
    if (mScaleAnimation != null) {
      ...
      canvas.scale(mScale, mScale, bounds.exactCenterX(),
                   bounds.exactCenterY());
  ...
  private void setDesaturation(final float desaturation) {
    int newDesaturation =
        (int)Math.floor(desaturation * REDUCED_FILTER_VALUE_SPACE);
  ...
  /**
   * Sets the brightness of this icon, 0 [no add. brightness] -> 1
   * [2bright2furious]
   */
  private void setBrightness(final float brightness) {
    int newBrightness =
        (int)Math.floor(brightness * REDUCED_FILTER_VALUE_SPACE);
  ...
  protected void updateFilter() {
    ...
    if (mDesaturation > 0) {
      ...
      // Compose a key with a fully saturated icon if we are just animating
      // brightness
      key = (1 << 16) | mBrightness;

      // We found that in L, ColorFilters cause drawing artifacts with shadows
      // baked into icons, so just use a PorterDuff filter when we aren't
      ...
      if (filter == null) {
        ...
        if (usePorterDuffFilter) {
          filter = new PorterDuffColorFilter(
              Color.argb(brightnessI, 255, 255, 255), PorterDuff.Mode.SRC_ATOP);
        } else {
