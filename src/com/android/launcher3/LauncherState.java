...
public class LauncherState {
  ...
  /**
   * Set of elements indicating various workspace elements which change
   * visibility across states Note that workspace is not included here as in
   * that case, we animate individual pages
   */
  ...
  public static final int ALL_APPS_HEADER_EXTRA = 1
                                                  << 3; // e.g. app predictions
   ...
   */
  public static final LauncherState NORMAL = new LauncherState(
      0, ContainerType.WORKSPACE, 0,
      FLAG_DISABLE_RESTORE | FLAG_WORKSPACE_ICONS_CAN_BE_DRAGGED |
          FLAG_HIDE_BACK_BUTTON | FLAG_HAS_SYS_UI_SCRIM);
  ...
  /**
   * Used for containerType in {@link
  ...
  /**
   * True if the workspace pages should not be clipped relative to the workspace
  ...
  /**
   * True if the back button should be hidden when in this state (assuming no
   * floating views are open, launcher has window focus, etc).
   */
  public final boolean hideBackButton;

  public final boolean hasSysUiScrim;

  public LauncherState(final int id, final int containerType,
                       final int transitionDuration, final int flags) {
    ...
    this.workspaceIconsCanBeDragged =
        (flags & FLAG_WORKSPACE_ICONS_CAN_BE_DRAGGED) != 0;
   ...
   */
  public float getVerticalProgress(final Launcher launcher) { return 1f; }

  public float getScrimProgress(final Launcher launcher) {
    return getVerticalProgress(launcher);
  }

  public float getWorkspaceScrimAlpha(final Launcher launcher) { return 0; }

  public float getWorkspaceBlurAlpha(final Launcher launcher) { return 0; }
  ...
  /**
   * Called when the start transition ends and the user settles on this
   * particular state.
   */
  ...
  protected static void dispatchWindowStateChanged(final Launcher launcher) {
    launcher.getWindow().getDecorView().sendAccessibilityEvent(
