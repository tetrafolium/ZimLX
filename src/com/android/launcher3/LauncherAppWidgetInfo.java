...
public class LauncherAppWidgetInfo extends ItemInfo {
  ...
  /**
   * Indicates that the widget UI is not yet ready, and user needs to set it up
   * again.
   */
  ...
  /**
   * Indicates that the widget has been allocated an Id. The id is still not
   * valid, as it has not been bound yet.
   */
  public static final int FLAG_ID_ALLOCATED = 16;

  /**
   * Indicates that the widget does not need to show config activity, even if it
   * has a configuration screen. It can also optionally have some extras which
   * are sent during bind.
   */
  ...
  /**
   * Indicates that this is a locally defined widget and hence has no system
   * allocated id.
   */
  ...
  /**
   * Nonnull for pending widgets. We use this to get the icon and title for the
   * widget.
   */
  ...
  public LauncherAppWidgetInfo(final int appWidgetId,
    ...
    // Since the widget isn't instantiated yet, we don't know these values. Set
    // them to -1 to indicate that they should be calculated based on the layout
    // and minWidth/minHeight
    spanX = -1;
    spanY = -1;
  ...
  public boolean isCustomWidget() { return appWidgetId <= CUSTOM_WIDGET_ID; }
   ...
   * widgets).
   */
  void onBindAppWidget(final Launcher launcher,
