...
public abstract class ButtonDropTarget extends TextView
  ...
  /**
   * An item must be dragged at least this many pixels before this drop target
   * is enabled.
   */
  ...
  public ButtonDropTarget(final Context context, final AttributeSet attrs,
    ...
    mBottomDragPadding =
        resources.getDimensionPixelSize(R.dimen.drop_target_drag_padding);
    mDragDistanceThreshold =
        resources.getDimensionPixelSize(R.dimen.drag_distanceThreshold);
  }
  ...
  protected void setDrawable(final int resId) {
    ...
    // corresponding to drawableLeft and drawableStart.
    if (mTextVisible) {
  ...
  public final void onDragEnter(final DragObject d) {
    if (!d.accessibleDrag && !mTextVisible) {
      ...
      TextView message = (TextView)LayoutInflater.from(getContext())
                             .inflate(R.layout.drop_target_tool_tip, null);
  ...
  private void animateTextColor(final int targetColor) {
    ...
    mCurrentColorAnim.play(
        ObjectAnimator.ofArgb(this, "textColor", targetColor));
  ...
  public boolean isDropEnabled() {
    return mActive && (mAccessibleDrag ||
                       mLauncher.getDragController().getDistanceDragged() >=
                           mDragDistanceThreshold);
  }
  ...
  public void onDrop(final DragObject d, final DragOptions options) {
    ...
    dragLayer.animateView(d.dragView, from, to, scale, 1f, 1f, 0.1f, 0.1f,
                          DRAG_VIEW_DROP_DURATION, Interpolators.DEACCEL_2,
                          Interpolators.LINEAR, onAnimationEndRunnable,
  ...
  @Override
  public void
  getHitRectRelativeToDragLayer(final android.graphics.Rect outRect) {
  ...
  public int getTextColor() { return getTextColors().getDefaultColor(); }
  ...
  public void setTextVisible(final boolean isVisible) {
    ...
    if (mTextVisible != isVisible || !TextUtils.equals(newText, getText())) {
      ...
      setText(newText);
      if (mTextVisible) {
        setCompoundDrawablesRelativeWithIntrinsicBounds(mDrawable, null, null,
                                                        null);
      } else {
        setCompoundDrawablesRelativeWithIntrinsicBounds(null, mDrawable, null,
                                                        null);
      }
    }
  }
  ...
  public boolean isTextTruncated(final int availableWidth) {
    availableWidth -=
        (getPaddingLeft() + getPaddingRight() + mDrawable.getIntrinsicWidth() +
         getCompoundDrawablePadding());
    CharSequence displayedText = TextUtils.ellipsize(
        mText, getPaint(), availableWidth, TextUtils.TruncateAt.END);
