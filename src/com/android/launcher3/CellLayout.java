...
public class CellLayout extends ViewGroup {
  ...
  @ViewDebug.ExportedProperty(category = "launcher") @Thunk int mCellWidth;
  @ViewDebug.ExportedProperty(category = "launcher") @Thunk int mCellHeight;
  private int mFixedCellWidth;
  private int mFixedCellHeight;

  @ViewDebug.ExportedProperty(category = "launcher") private int mCountX;
  @ViewDebug.ExportedProperty(category = "launcher") private int mCountY;
  ...
  // screens. They are used as circular arrays, indexed by mDragOutlineCurrent.
  ...
  public CellLayout(final Context context) { this(context, null); }

  public CellLayout(final Context context, final AttributeSet attrs) {
  ...
  public CellLayout(final Context context, final AttributeSet attrs,
    ...
    TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CellLayout,
                                                  defStyle, 0);
    mContainerType =
        a.getInteger(R.styleable.CellLayout_containerType, WORKSPACE);
    a.recycle();

    // A ViewGroup usually does not draw, but CellLayout needs to draw a
    // rectangle to show the user where a dragged item will land when dropped.
    ...
    mReorderPreviewAnimationMagnitude =
        (REORDER_PREVIEW_MAGNITUDE * grid.iconSizePx);
    ...
    // trail behind the drag path. Set up all the animations that are used to
    // implement this fading.
    final int duration = res.getInteger(R.integer.config_dragOutlineFadeTime);
    final float fromAlphaValue = 0;
    final float toAlphaValue =
        (float)res.getInteger(R.integer.config_dragOutlineMaxAlpha);

    Arrays.fill(mDragOutlineAlphas, fromAlphaValue);

    for (int i = 0; i < mDragOutlineAnims.length; i++) {
      final InterruptibleInOutAnimator anim = new InterruptibleInOutAnimator(
          this, duration, fromAlphaValue, toAlphaValue);
      ...
      anim.getAnimator().addUpdateListener(new AnimatorUpdateListener() {
        public void onAnimationUpdate(final ValueAnimator animation) {
          ...
          // If an animation is started and then stopped very quickly, we can
          // still get spurious updates we've cleared the tag. Guard against
          // this.
          if (outline == null) {
            if (LOGD) {
      ...
      // The animation holds a reference to the drag outline bitmap as long is
      // it's running. This way the bitmap can be GCed when the animations are
      // complete.
    ...
    mShortcutsAndWidgets =
        new ShortcutAndWidgetContainer(context, mContainerType);
    mShortcutsAndWidgets.setCellDimensions(mCellWidth, mCellHeight, mCountX,
                                           mCountY);
  ...
  public void enableAccessibleDrag(final boolean enable, final int dragType) {
    mUseTouchHelper = enable;
    if (!enable) {
      ViewCompat.setAccessibilityDelegate(this, null);
      setImportantForAccessibility(IMPORTANT_FOR_ACCESSIBILITY_NO);
      getShortcutsAndWidgets().setImportantForAccessibility(
  ...
  public boolean onInterceptTouchEvent(final MotionEvent ev) {
    return mUseTouchHelper || (mInterceptTouchListener != null &&
  ...
  public boolean onTouchEvent(final MotionEvent ev) {
    ...
    // Stylus button press on a home screen should not switch between overview
    // mode and the home screen mode, however, once in overview mode stylus
    // button press should be enabled to allow rearranging the different home
    // screens. So check what mode the workspace is in, and only perform stylus
    // button presses while in overview mode.
  ...
  public void enableHardwareLayer(final boolean hasLayer) {
    mShortcutsAndWidgets.setLayerType(
        hasLayer ? LAYER_TYPE_HARDWARE : LAYER_TYPE_NONE, sPaint);
  }

  public void setCellDimensions(final int width, final int height) {
  ...
  public void setGridSize(final int x, final int y) {
    mCountX = x;
    mCountY = y;
    mOccupied = new GridOccupancy(mCountX, mCountY);
    mTmpOccupied = new GridOccupancy(mCountX, mCountY);
  ...
  public void setDropPending(final boolean pending) { mDropPending = pending; }

  public boolean isDropPending() { return mDropPending; }

  void setIsDragOverlapping(final boolean isDragOverlapping) {
    if (mIsDragOverlapping != isDragOverlapping) {
      mIsDragOverlapping = isDragOverlapping;
      mBackground.setState(mIsDragOverlapping ? BACKGROUND_STATE_ACTIVE
  ...
  @Override
  protected void
  dispatchSaveInstanceState(final SparseArray<Parcelable> container) {
  ...
  /**
   * Wrap the SparseArray in another Parcelable so that the item ids do not
   * conflict with our our internal resource ids
   */
  private ParcelableSparseArray
  getJailedArray(final SparseArray<Parcelable> container) {
  ...
  public boolean getIsDragOverlapping() { return mIsDragOverlapping; }

  @Override
  protected void onDraw(final Canvas canvas) {
    // When we're large, we are either drawn in a "hover" state (ie when
    // dragging an item to a neighboring page) or with just a normal background
    // (if backgroundAlpha > 0.0f) When we're small, we are either drawn
    // normally or in the "accepts drops" state (during a drag). However, we
    // also drag the mini hover background *over* one of those two backgrounds
    ...
    if (mFolderLeaveBehind.delegateCellX >= 0 &&
        mFolderLeaveBehind.delegateCellY >= 0) {
  ...
  public void addFolderBackground(final PreviewBackground bg) {
    mFolderBackgrounds.add(bg);
  }
  public void removeFolderBackground(final PreviewBackground bg) {
    mFolderBackgrounds.remove(bg);
  }

  public void setFolderLeaveBehindCell(final int x, final int y) {
    View child = getChildAt(x, y);
    mFolderLeaveBehind.setup(mLauncher, null, child.getMeasuredWidth(),
                             child.getPaddingTop());
  ...
  public void restoreInstanceState(final SparseArray<Parcelable> states) {
    try {
      ...
      // Mismatched viewId / viewType preventing restore. Skip restore on
      // production builds.
  ...
  public boolean acceptsWidget() { return mContainerType == WORKSPACE; }

  public boolean isHotseat() { return mContainerType == HOTSEAT; }

  public boolean addViewToCellLayout(final View child, final int index,
                                     final int childId,
                                     final LayoutParams params,
                                     final boolean markCells) {
   ...
   */
  public void pointToCellExact(final int x, final int y, final int[] result) {
   ...
   * @param result Array of 2 ints to hold the x and y coordinate of the cell
   */
  void pointToCellRounded(final int x, final int y, final int[] result) {
   ...
   */
  void cellToPoint(final int cellX, final int cellY, final int[] result) {
    final int hStartPadding = getPaddingLeft();
    final int vStartPadding = getPaddingTop();
  ...
  /**
   * Given a cell coordinate, return the point that represents the center of the
   * cell
   *
   * @param cellX X coordinate of the cell
   * @param cellY Y coordinate of the cell
   ...
   * @param result Array of 2 ints to hold the x and y coordinate of the point
   */
  void cellToCenterPoint(final int cellX, final int cellY, final int[] result) {
   ...
   * @param result Array of 2 ints to hold the x and y coordinate of the point
   */
  void regionToCenterPoint(final int cellX, final int cellY, final int spanX,
                           final int spanY, final int[] result) {
    final int hStartPadding = getPaddingLeft();
    final int vStartPadding = getPaddingTop();
   ...
   */
  void regionToRect(final int cellX, final int cellY, final int spanX,
                    final int spanY, final Rect result) {
    final int hStartPadding = getPaddingLeft();
    final int vStartPadding = getPaddingTop();
    final int left = hStartPadding + cellX * mCellWidth;
    final int top = vStartPadding + cellY * mCellHeight;
    result.set(left, top, left + (spanX * mCellWidth),
               top + (spanY * mCellHeight));
  }

  public float getDistanceFromCell(final float x, final float y,
                                   final int[] cell) {
  ...
  protected void onLayout(final boolean changed, final int l, final int t,
    ...
    mBackground.setBounds(left - mTempRect.left - getPaddingLeft(),
  ...
  /**
   * Returns the amount of space left over after subtracting padding and cells.
   * This space will be very small, a few pixels at most, and is a result of
   * rounding down when calculating the cell width in {@link
   * DeviceProfile#calculateCellWidth(int, int)}.
   */
  ...
  public Drawable getScrimBackground() { return mBackground; }
  ...
  public boolean animateChildToPosition(final View child, final int cellX,
                                        final int cellY, final int duration,
                                        final int delay,
                                        final boolean permanent,
                                        final boolean adjustOccupied) {
    ...
    if (clc.indexOfChild(child) != -1) {
      ...
      if (adjustOccupied) {
        ...
        occupied.markCells(lp.cellX, lp.cellY, lp.cellHSpan, lp.cellVSpan,
                           false);
  ...
  void visualizeDropLocation(final View v,
                             final DragPreviewProvider outlineProvider,
                             final int cellX, final int cellY, final int spanX,
                             final int spanY, final boolean resize,
                             final DropTarget.DragObject dragObject) {
    final int oldDragCellX = mDragCell[0];
    final int oldDragCellY = mDragCell[1];

    if (outlineProvider == null ||
        outlineProvider.generatedDragOutline == null) {
      return;
    }
    ...
    if (cellX != oldDragCellX || cellY != oldDragCellY) {
      ...
      if (resize) {
        ...
        if (v != null && dragOffset == null) {
          ...
          // offsets added by the view's parent.
          MarginLayoutParams lp = (MarginLayoutParams)v.getLayoutParams();
          left += lp.leftMargin;
          top += lp.topMargin;

          // Offsets due to the size difference between the View and the
          // dragOutline. There is a size difference to account for the outer
          // blur, which may lie outside the bounds of the view.
          top += ((mCellHeight * spanY) - dragOutline.getHeight()) / 2;
          // We center about the x axis
          left += ((mCellWidth * spanX) - dragOutline.getWidth()) / 2;
        } else {
          if (dragOffset != null && dragRegion != null) {
            // Center the drag region *horizontally* in the cell and apply a
            // drag outline offset
            left +=
                dragOffset.x + ((mCellWidth * spanX) - dragRegion.width()) / 2;
      ...
      if (dragObject.stateAnnouncer != null) {
        dragObject.stateAnnouncer.announce(
            getItemMoveDescription(cellX, cellY));
      }
    }
  }

  @SuppressLint("StringFormatMatches")
  public String getItemMoveDescription(final int cellX, final int cellY) {
    if (mContainerType == HOTSEAT) {
      return getContext().getString(R.string.move_to_hotseat_position,
                                    Math.max(cellX, cellY) + 1);
    } else {
      return getContext().getString(R.string.move_to_empty_cell, cellY + 1,
                                    cellX + 1);
    }
  }
   ...
   * @param result Array in which to place the result, or null (in which case a
   *     new array will
   *        be allocated)
   * @return The X, Y cell of a vacant area that can contain this object,
   *         nearest the requested location.
   */
  int[] findNearestVacantArea(final int pixelX, final int pixelY,
                              final int minSpanX, final int minSpanY,
                              final int spanX, final int spanY,
                              final int[] result, final int[] resultSpan) {
    return findNearestArea(pixelX, pixelY, minSpanX, minSpanY, spanX, spanY,
                           true, result, resultSpan);
  }
  ...
  private void recycleTempRects(final Stack<Rect> used) {
    while (!used.isEmpty()) {
   ...
   *         nearest the requested location.
   */
  private int[] findNearestArea(final int pixelX, final int pixelY,
                                final int minSpanX, final int minSpanY,
                                final int spanX, final int spanY,
                                final boolean ignoreOccupied,
                                final int[] result, final int[] resultSpan) {
    lazyInitTempRectStack();

    // For items with a spanX / spanY > 1, the passed in point (pixelX, pixelY)
    // corresponds to the center of the item, but we are searching based on the
    // top-left cell, so we translate the point over to correspond to the
    // top-left.
    ...
    inner:
      for (int x = 0; x < countX - (minSpanX - 1); x++) {
        ...
        if (ignoreOccupied) {
          ...
          // We know that the item will fit at _some_ acceptable size, now let's
          // see how big we can make it. We'll alternate between incrementing x
          // and y spans until we hit a limit.
          boolean incX = true;
        ...
        // We verify that the current rect is not a sub-rect of any of our
        // previous candidates. In this case, the current rect is disqualified
        // in favour of the containing rect.
   ...
   * @param spanX Horizontal span of the object.
   * @param spanY Vertical span of the object.
   * @param direction The favored direction in which the views should move from
   *     x, y
   ...
   *     specified block (cellX,
   *        cellY, spanX, spanY) are occupied. This is used when try to move a
   * group of views.
   * @param result Array in which to place the result, or null (in which case a
   *     new array will
   *        be allocated)
   ...
   *         nearest the requested location.
   */
  private int[] findNearestArea(final int cellX, final int cellY,
                                final int spanX, final int spanY,
                                final int[] direction,
                                final boolean[][] occupied,
                                final boolean[][] blockOccupied,
                                final int[] result) {
    // Keep track of best-scoring drop area
    final int[] bestXY = result != null ? result : new int[2];
    ...
    inner:
      for (int x = 0; x < countX - (spanX - 1); x++) {
        ...
        // The direction score is just the dot product of the two candidate
        // direction and that passed in.
  ...
  private boolean addViewToTempLocation(final View v,
                                        final Rect rectOccupiedByPotentialDrop,
                                        final int[] direction,
                                        final ItemConfiguration currentState) {
   ...
   * complex edges of the cluster and determining how those edges interact with
   * other views. The edges essentially define a fine-grained boundary around
   * the cluster of views -- like a more precise version of a bounding box.
   */
  private class ViewCluster {
    ...
    public ViewCluster(final ArrayList<View> views,
                       final ItemConfiguration config) {
    ...
    void computeEdge(final int which) {
    ...
    boolean isViewTouchingEdge(final View v, final int whichEdge) {
    ...
    void shift(final int whichEdge, final int delta) {
      for (View v : views) {
    ...
    public void sortConfigurationForEdgePush(final int edge) {
  ...
  private boolean
  pushViewsToTempLocation(final ArrayList<View> views,
                          final Rect rectOccupiedByPotentialDrop,
                          final int[] direction, final View dragView,
                          final ItemConfiguration currentState) {
    ...
    // Determine the edge of the cluster that will be leading the push and how
    // far the cluster must be shifted.
    if (direction[0] < 0) {
    ...
    // will revert to the initial state. The process of finding a solution
    // modifies the configuration in place, hence the need for revert in the
    // failure case.
    currentState.save();

    // The pushing algorithm is simplified by considering the views in the order
    // in which they would be pushed by the cluster. For example, if the cluster
    // is leading with its left edge, we consider sort the views by their right
    ...
    while (pushDistance > 0 && !fail) {
      ...
      // The cluster has been completed, now we move the whole thing over in the
    ...
    // Due to the nature of the algorithm, the only check required to verify a
    // valid solution is to ensure that completed shifted cluster lies
    // completely within the cell layout.
    ...
    // In either case, we set the occupied array as marked for the location of
    // the views
  ...
  private boolean addViewsToTempLocation(final ArrayList<View> views,
                                         final Rect rectOccupiedByPotentialDrop,
                                         final int[] direction,
                                         final View dragView,
                                         final ItemConfiguration currentState) {
    ...
    GridOccupancy blockOccupied =
        new GridOccupancy(boundingRect.width(), boundingRect.height());
    int top = boundingRect.top;
    int left = boundingRect.left;
    // We mark more precisely which parts of the bounding rect are truly
    // occupied, allowing for interlocking.
    for (View v : views) {
      CellAndSpan c = currentState.map.get(v);
      blockOccupied.markCells(c.cellX - left, c.cellY - top, c.spanX, c.spanY,
                              true);
    }
  ...
  // mechanic by trying to push items in each of the cardinal directions, in an
  // order based on the direction vector passed.
  private boolean
  attemptPushInDirection(final ArrayList<View> intersectingViews,
                         final Rect occupied, final int[] direction,
                         final View ignoreView,
                         final ItemConfiguration solution) {
    if ((Math.abs(direction[0]) + Math.abs(direction[1])) > 1) {
      ...
      // If the direction vector has a single non-zero component, we push first
  ...
  private boolean rearrangementExists(final int cellX, final int cellY,
                                      final int spanX, final int spanY,
                                      final int[] direction,
                                      final View ignoreView,
                                      final ItemConfiguration solution) {
    ...
    // First we try to find a solution which respects the push mechanic. That
    // is, we try to find a solution such that no displaced item travels through
    // another item without also displacing that item.
    if (attemptPushInDirection(mIntersectingViews, mOccupiedRect, direction,
                               ignoreView, solution)) {
      return true;
    }

    // Next we try moving the views as a block, but without requiring the push
    // mechanic.
  ...
  /*
   * Returns a pair (x, y), where x,y are in {-1, 0, 1} corresponding to vector
   * between the provided point and the provided cell
   */
  private void computeDirectionVector(final float deltaX, final float deltaY,
                                      final int[] result) {
  ...
  findReorderSolution(final int pixelX, final int pixelY, final int minSpanX,
                      final int minSpanY, final int spanX, final int spanY,
                      final int[] direction, final View dragView,
                      final boolean decX, final ItemConfiguration solution) {
    // Copy the current state into the solution. This solution will be
    // manipulated as necessary.
    copyCurrentStateToSolution(solution, false);
    // Copy the current occupied array into the temporary occupied array. This
    // array will be manipulated as necessary to find a solution.
    mOccupied.copyTo(mTmpOccupied);

    // We find the nearest cell into which we would place the dragged item,
    // assuming there's nothing in its way.
    ...
    if (!success) {
      // We try shrinking the widget down to size in an alternating pattern,
      // shrink 1 in x, then 1 in y etc.
      if (spanX > minSpanX && (minSpanY == spanY || decX)) {
        return findReorderSolution(pixelX, pixelY, minSpanX, minSpanY,
                                   spanX - 1, spanY, direction, dragView, false,
                                   solution);
      } else if (spanY > minSpanY) {
        return findReorderSolution(pixelX, pixelY, minSpanX, minSpanY, spanX,
                                   spanY - 1, direction, dragView, true,
                                   solution);
      }
  ...
  private void copyCurrentStateToSolution(final ItemConfiguration solution,
                                          final boolean temp) {
  ...
  private void copySolutionToTempState(final ItemConfiguration solution,
                                       final View dragView) {
    mTmpOccupied.clear();
  ...
  private void animateItemsToSolution(final ItemConfiguration solution,
                                      final View dragView,
                                      final boolean commitDragView) {
  ...
  private void
  beginOrAdjustReorderPreviewAnimations(final ItemConfiguration solution,
                                        final View dragView, final int delay,
                                        final int mode) {
    int childCount = mShortcutsAndWidgets.getChildCount();
    for (int i = 0; i < childCount; i++) {
      View child = mShortcutsAndWidgets.getChildAt(i);
      ...
      if (c != null && !skip) {
        ReorderPreviewAnimation rha =
            new ReorderPreviewAnimation(child, mode, lp.cellX, lp.cellY,
                                        c.cellX, c.cellY, c.spanX, c.spanY);
        rha.animate();
      }
    }
  }

  // Class which represents the reorder preview animations. These animations
  // show that an item is in a temporary state, and hint at where the item will
  // return to.
  class ReorderPreviewAnimation {
    ...
    public ReorderPreviewAnimation(final View child, final int mode,
                                   final int cellX0, final int cellY0,
                                   final int cellX1, final int cellY1,
                                   final int spanX, final int spanY) {
      ...
      finalScale =
          (mChildScale - (CHILD_DIVIDEND / child.getWidth())) * initScale;
      ...
      if (dX == dY && dX == 0) {
        ...
        if (dY == 0) {
          finalDeltaX +=
              -dir * Math.signum(dX) * mReorderPreviewAnimationMagnitude;
        } else if (dX == 0) {
          finalDeltaY +=
    ...
    void setInitialAnimationValues(final boolean restoreOriginalValues) {
    ...
    void animate() {
      boolean noMovement =
          (finalDeltaX == initDeltaX) && (finalDeltaY == initDeltaY);
      ...
      // Animations are disabled in power save mode, causing the repeated
      // animation to jump spastically between beginning and end states. Since
      // this looks bad, we don't repeat the animation in power save mode.
    ...
    void completeAnimationImmediately() {
      ...
      setInitialAnimationValues(true);
      a = LauncherAnimUtils
              .ofPropertyValuesHolder(child, new PropertyListBuilder()
                                                 .scale(initScale)
  ...
  private void commitTempPlacement() {
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      // We do a null check here because the item info can be null in the case
      // of the AllApps button in the hotseat.
      if (info != null) {
        final boolean requiresDbUpdate =
            (info.cellX != lp.tmpCellX || info.cellY != lp.tmpCellY ||
        ...
        if (requiresDbUpdate) {
          mLauncher.getModelWriter().modifyItemInDatabase(
              info, container, screenId, info.cellX, info.cellY, info.spanX,
              info.spanY);
        }
      }
    }
  ...
  private void setUseTempCoords(final boolean useTempCoords) {
    int childCount = mShortcutsAndWidgets.getChildCount();
    for (int i = 0; i < childCount; i++) {
      LayoutParams lp =
          (LayoutParams)mShortcutsAndWidgets.getChildAt(i).getLayoutParams();
      lp.useTmpCoords = useTempCoords;
    }
  }

  private ItemConfiguration findConfigurationNoShuffle(
      final int pixelX, final int pixelY, final int minSpanX,
      final int minSpanY, final int spanX, final int spanY, final View dragView,
      final ItemConfiguration solution) {
    int[] result = new int[2];
  ...
  direction vector needs to match with the notion of the dragView pushing other
  views, we have to employ a slightly more subtle notion of the direction
  vector. The question is what two points is the vector between? The center of
  the dragView and its desired destination? Not quite, as this doesn't
  necessarily coincide with the interaction of the dragView and items occupying
  those cells. Instead we use some heuristics to often lock the vector to up,
  down, left or right, which helps make pushing feel right.
  */
  private void getDirectionVectorForDrop(final int dragViewCenterX,
                                         final int dragViewCenterY,
                                         final int spanX, final int spanY,
                                         final View dragView,
                                         final int[] resultDirection) {
    ...
    regionToRect(targetDestination[0], targetDestination[1], spanX, spanY,
                 dragRect);
    dragRect.offset(dragViewCenterX - dragRect.centerX(),
                    dragViewCenterY - dragRect.centerY());

    Rect dropRegionRect = new Rect();
    getViewsIntersectingRegion(targetDestination[0], targetDestination[1],
                               spanX, spanY, dragView, dropRegionRect,
                               mIntersectingViews);
    ...
    regionToRect(dropRegionRect.left, dropRegionRect.top,
                 dropRegionRect.width(), dropRegionRect.height(),
                 dropRegionRect);
  ...
  private void
  getViewsIntersectingRegion(final int cellX, final int cellY, final int spanX,
                             final int spanY, final View dragView,
                             final Rect boundingRect,
                             final ArrayList<View> intersectingViews) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (child == dragView)
        continue;
      LayoutParams lp = (LayoutParams)child.getLayoutParams();
      r1.set(lp.cellX, lp.cellY, lp.cellX + lp.cellHSpan,
  ...
  boolean isNearestDropLocationOccupied(final int pixelX, final int pixelY,
                                        final int spanX, final int spanY,
                                        final View dragView,
                                        final int[] result) {
    result = findNearestArea(pixelX, pixelY, spanX, spanY, result);
    getViewsIntersectingRegion(result[0], result[1], spanX, spanY, dragView,
                               null, mIntersectingViews);
  ...
  boolean createAreaForResize(final int cellX, final int cellY, final int spanX,
                              final int spanY, final View dragView,
                              final int[] direction, final boolean commit) {
    ...
    ItemConfiguration swapSolution =
        findReorderSolution(pixelXY[0], pixelXY[1], spanX, spanY, spanX, spanY,
                            direction, dragView, true, new ItemConfiguration());

    setUseTempCoords(true);
    if (swapSolution != null && swapSolution.isSolution) {
      // If we're just testing for a possible location (MODE_ACCEPT_DROP), we
      // don't bother committing anything or animating anything as we just want
  ...
  int[] performReorder(final int pixelX, final int pixelY, final int minSpanX,
                       final int minSpanY, final int spanX, final int spanY,
                       final View dragView, final int[] result,
                       final int[] resultSpan, final int mode) {
    // First we determine if things have moved enough to cause a different
    // layout
    result = findNearestArea(pixelX, pixelY, spanX, spanY, result);
    ...
    // recompute the direction vector, since we want the solution to match the
    // preview, and it's possible that the exact position of the item has
    // changed to result in a new reordering outcome.
    ...
    ItemConfiguration swapSolution = findReorderSolution(
        pixelX, pixelY, minSpanX, minSpanY, spanX, spanY, mDirectionVector,
        dragView, true, new ItemConfiguration());

    // We attempt the approach which doesn't shuffle views at all
    ItemConfiguration noShuffleSolution =
        findConfigurationNoShuffle(pixelX, pixelY, minSpanX, minSpanY, spanX,
                                   spanY, dragView, new ItemConfiguration());

    ItemConfiguration finalSolution = null;

    // If the reorder solution requires resizing (shrinking) the item being
    // dropped, we instead favor a solution in which the item is not resized,
    // but
  ...
  void setItemPlacementDirty(final boolean dirty) {
    mItemPlacementDirty = dirty;
  }
  boolean isItemPlacementDirty() { return mItemPlacementDirty; }
  ...
  private static class ItemConfiguration extends CellAndSpan {
    ...
    void add(final View v, final CellAndSpan cs) {
      map.put(v, cs);
      savedMap.put(v, new CellAndSpan());
      sortedViews.add(v);
    }

    int area() { return spanX * spanY; }

    void getBoundingRectForViews(final ArrayList<View> views,
                                 final Rect outRect) {
      boolean first = true;
      for (View v : views) {
  ...
  public int[] findNearestArea(final int pixelX, final int pixelY,
                               final int spanX, final int spanY,
                               final int[] result) {
    return findNearestArea(pixelX, pixelY, spanX, spanY, spanX, spanY, false,
                           result, null);
  }

  boolean existsEmptyCell() { return findCellForSpan(null, 1, 1); }

  /**
   * Finds the upper-left coordinate of the first rectangle in the grid that can
   * hold a cell of the specified dimensions. If intersectX and intersectY are
   * not -1, then this method will only return coordinates for rectangles that
   * contain the cell (intersectX, intersectY)
   *
   * @param cellXY The array that will contain the position of a vacant cell if
   *     such a cell
   *               can be found.
   ...
   *     otherwise.
   */
  public boolean findCellForSpan(final int[] cellXY, final int spanX,
                                 final int spanY) {
    if (cellXY == null) {
      cellXY = new int[2];
    }
   ...
   * first), or it may have begun on another layout.
   */
  void onDragEnter() { mDragging = true; }
   ...
   */
  void onDragExit() {
    // This can actually be called when we aren't in a drag, e.g. when adding a
    // new item to this layout via the customize drawer. Guard against that
    // case.
    if (mDragging) {
      mDragging = false;
    }
   ...
   */
  public void cellToRect(final int cellX, final int cellY, final int cellHSpan,
                         final int cellVSpan, final Rect resultRect) {
  ...
  generateLayoutParams(final ViewGroup.LayoutParams p) {
  ...
  public static class LayoutParams extends ViewGroup.MarginLayoutParams {
    ...
    /**
     * Indicates whether the item will set its x, y, width and height parameters
     * freely, or whether these will be computed based on cellX, cellY,
     * cellHSpan and cellVSpan.
     */
    public boolean isLockedToGrid = true;

    /**
     * Indicates whether this item can be reordered. Always true except in the
     * case of the the AllApps button and QSB place holder.
     */
    ...
    public LayoutParams(final int cellX, final int cellY, final int cellHSpan,
                        final int cellVSpan) {
    ...
    public void setup(final int cellWidth, final int cellHeight,
                      final boolean invertHorizontally, final int colCount) {
      setup(cellWidth, cellHeight, invertHorizontally, colCount, 1.0f, 1.0f);
    }

    /**
     * Use this method, as opposed to {@link #setup(int, int, boolean, int)}, if
     * the view needs to be scaled.
     *
     * ie. In multi-window mode, we setup widgets so that they are measured and
     * laid out using their full/invariant device profile sizes.
     */
    public void setup(final int cellWidth, final int cellHeight,
                      final boolean invertHorizontally, final int colCount,
                      final float cellScaleX, final float cellScaleY) {
      if (isLockedToGrid) {
        ...
        width = (int)(myCellHSpan * cellWidth / cellScaleX - leftMargin -
                      rightMargin);
        height = (int)(myCellVSpan * cellHeight / cellScaleY - topMargin -
                       bottomMargin);
  ...
  // 2. When long clicking on an empty cell in a CellLayout, we save information
  // about the
  //    cellX and cellY coordinates and which page was clicked. We then set this
  //    as a tag on the CellLayout that was long clicked
  public static final class CellInfo extends CellAndSpan {
    public final View cell;
    final long screenId;
    final long container;

    public CellInfo(final View v, final ItemInfo info) {
      cellX = info.cellX;
      cellY = info.cellY;
      spanX = info.spanX;
      spanY = info.spanY;
  ...
  /**
   * Returns whether an item can be placed in this CellLayout (after rearranging
   * and/or resizing if necessary).
   */
    ...
    for (int cellX = 0; cellX < getCountX(); cellX++) {
      for (int cellY = 0; cellY < getCountY(); cellY++) {
        ...
        if (findReorderSolution(cellPoint[0], cellPoint[1], itemInfo.minSpanX,
                                itemInfo.minSpanY, itemInfo.spanX,
                                itemInfo.spanY, mDirectionVector, null, true,
