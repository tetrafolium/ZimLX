...
public class WorkspaceTouchListener
	extends GestureTouchListener implements OnTouchListener, Runnable {

/**
 * STATE_PENDING_PARENT_INFORM is the state between longPress performed & the
 * next motionEvent. This next event is used to send an ACTION_CANCEL to
 * Workspace, to that it clears any temporary scroll state. After that, the
 * state is set to COMPLETED, and we just eat up all subsequent motion events.
 */
...
public WorkspaceTouchListener(final Launcher launcher,
	...
	if (mLongPressState == STATE_REQUESTED) {
		if (canHandleLongPress()) {
			...
			mLauncher.getUserEventDispatcher().logActionOnContainer(
				Action.Touch.LONGPRESS, Action.Direction.NONE,
