...
public abstract class BaseActivity
    extends Activity implements UserEventDelegate {

  public static final int INVISIBLE_BY_STATE_HANDLER = 1 << 0;
  public static final int INVISIBLE_BY_APP_TRANSITIONS = 1 << 1;
  public static final int INVISIBLE_BY_PENDING_FLAGS = 1 << 2;

  // This is not treated as invisibility flag, but adds as a hint for an
  // incomplete transition. When the wallpaper animation runs, it replaces this
  // flag with a proper invisibility flag, INVISIBLE_BY_PENDING_FLAGS only for
  // the duration of that animation.
  public static final int PENDING_INVISIBLE_BY_WALLPAPER_ANIMATION = 1 << 3;

  private static final int INVISIBLE_FLAGS = INVISIBLE_BY_STATE_HANDLER |
  ...
  private final ArrayList<OnDeviceProfileChangeListener> mDPChangeListeners =
      new ArrayList<>();
  private final ArrayList<MultiWindowModeChangedListener>
      mMultiWindowModeChangedListeners = new ArrayList<>();
  ...
  /**
   * State flag indicating if the user is active or the actitvity when to
   * background as a result of user action.
   *
   * @see #isUserActive()
   */
  ...
  @Retention(SOURCE)
  @IntDef(flag = true, value = {ACTIVITY_STATE_STARTED, ACTIVITY_STATE_RESUMED,
                                ACTIVITY_STATE_USER_ACTIVE})
  public @interface ActivityFlags {}

  @ActivityFlags private int mActivityFlags;

  // When the recents animation is running, the visibility of the Launcher is
  // managed by the animation
  @InvisibilityFlags private int mForceInvisible;

  public DeviceProfile getDeviceProfile() { return mDeviceProfile; }
  ...
  public final UserEventDispatcher getUserEventDispatcher() {
    if (mUserEventDispatcher == null) {
      mUserEventDispatcher =
          UserEventDispatcher.newInstance(this, mDeviceProfile, this);
    }
  ...
  public void onMultiWindowModeChanged(final boolean isInMultiWindowMode,
    ...
    for (int i = mMultiWindowModeChangedListeners.size() - 1; i >= 0; i--) {
      mMultiWindowModeChangedListeners.get(i).onMultiWindowModeChanged(
          isInMultiWindowMode);
    }
  }
  ...
  protected void onPause() {
    ...
    // Reset the overridden sysui flags used for the task-swipe launch
    // animation, we do this here instead of at the end of the animation because
    // the start of the new activity does not happen immediately, which would
    // cause us to reset to launcher's sysui flags and then back to the new app
    // (causing a flash)
  ...
  public void addOnDeviceProfileChangeListener(
      final OnDeviceProfileChangeListener listener) {
  ...
  public void addMultiWindowModeChangedListener(
      final MultiWindowModeChangedListener listener) {
    mMultiWindowModeChangedListeners.add(listener);
  }

  public void removeMultiWindowModeChangedListener(
      final MultiWindowModeChangedListener listener) {
    mMultiWindowModeChangedListeners.remove(listener);
  }

  /**
   * Used to set the override visibility state, used only to handle the
   * transition home with the recents animation.
   *
  ...
  /**
   * @return Wether this activity should be considered invisible regardless of
   *     actual visibility.
   */
  ...
  protected void dumpMisc(final PrintWriter writer) {
    writer.println(" deviceProfile isTransposed=" +
