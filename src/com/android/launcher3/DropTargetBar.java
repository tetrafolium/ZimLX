 ...
 */
public class DropTargetBar
    extends FrameLayout implements DragListener, Insettable {
  ...
  public void setInsets(final Rect insets) {
    ...
    if (grid.isVerticalBarLayout()) {
      ...
      int gap;
      if (grid.isTablet) {
        // XXX: If the icon size changes across orientations, we will have to
        // take
        ...
        gap = grid.desiredWorkspaceLeftRightMarginPx -
              grid.defaultWidgetPadding.right;
      }
  ...
  protected void onLayout(final boolean changed, final int left, final int top,
    ...
    if (mIsVertical) {
      int gap = getResources().getDimensionPixelSize(
          R.dimen.drop_target_vertical_gap);
      int start = gap;
      int end;
      ...
      for (ButtonDropTarget button : mDropTargets) {
        if (button.getVisibility() != GONE) {
          ...
          button.layout(start - halfWidth, 0, start + halfWidth,
   ...
   * animation has completed, instead of hiding immediately when the drag has
   * ended.
   */
  protected void deferOnDragEnd() { mDeferOnDragEnd = true; }
  ...
  public ButtonDropTarget[] getDropTargets() { return mDropTargets; }
}
