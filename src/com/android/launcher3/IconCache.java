...
public class IconCache {
  ...
  public IconCache(final Context context, final InvariantDeviceProfile inv) {
    ...
    // Always prefer RGB_565 config for low res. If the bitmap has transparency,
    // it will automatically be loaded as ALPHA_8888.
  ...
  public Drawable getFullResIcon(final LauncherActivityInfo info,
    ...
    if (mIconProvider instanceof ZimIconProvider)
      return ((ZimIconProvider)mIconProvider)
          .getIcon(info, itemInfo, mIconDpi, flattenDrawable);
    return mIconProvider.getIcon(info, mIconDpi, flattenDrawable);
  }

  protected BitmapInfo makeDefaultIcon(final UserHandle user) {
    try (LauncherIcons li = LauncherIcons.obtain(mContext)) {
      return li.createBadgedIconBitmap(getFullResDefaultActivityIcon(), user,
                                       VERSION.SDK_INT);
    }
  }
   ...
   */
  public synchronized void remove(final ComponentName componentName,
   ...
   */
  private void removeFromMemCacheLocked(final String packageName,
                                        final UserHandle user) {
   ...
   * DB.
   */
  public synchronized void updateIconsForPkg(final String packageName,
                                             final UserHandle user) {
   ...
   * DB.
   */
  public synchronized void removeIconsForPkg(final String packageName,
                                             final UserHandle user) {
  ...
  public void updateDbIcons(final Set<String> ignorePackagesForMainUser) {
    ...
    for (UserHandle user : mUserManager.getUserProfiles()) {
      ...
      // #onPackageIconsUpdated} is called by the icon cache when the job is
      // complete.
  ...
  /**
   * Updates the persistent DB, such that only entries corresponding to {@param
   * apps} remain in the DB and are updated.
   * @return The set of packages for which icons have updated.
   */
  private void updateDBIcons(final UserHandle user,
                             final List<LauncherActivityInfo> apps,
    ...
    try {
      ...
      while (c.moveToNext()) {
        ...
        if ((info.applicationInfo.flags & ApplicationInfo.FLAG_IS_DATA_ONLY) !=
            0) {
   ...
   * @param replaceExisting if true, it will recreate the bitmap even if it
   *     already exists in
   *                        the memory. This is useful then the previous bitmap
   * was created using old data.
   */
  @Thunk
  synchronized void
  addIconToDBAndMemCache(final LauncherActivityInfo app, final PackageInfo info,
                         final long userSerial, final boolean replaceExisting) {
    final ComponentKey key =
        new ComponentKey(app.getComponentName(), app.getUser());
    ...
    entry.contentDescription =
        mUserManager.getBadgedLabelForUser(entry.title, app.getUser());
    mCache.put(key, entry);

    Bitmap lowResIcon = generateLowResIcon(entry.icon);
    ContentValues values = newContentValues(
        entry.icon, lowResIcon, entry.color, entry.title.toString(),
        app.getApplicationInfo().packageName);
    addIconToDB(values, app.getComponentName(), info, userSerial);
  }

  /**
   * Updates {@param values} to contain versioning information and adds it to
   * the DB.
   ...
   */
  public IconLoadRequest
  updateIconInBackground(final ItemInfoUpdateReceiver caller,
    ...
    IconLoadRequest request =
        new IconLoadRequest(mWorkerHandler, this::onIconRequestEnd) {
          @Override
          public void run() {
  ...
  public synchronized void updateTitleAndIcon(final AppInfo application) {
    CacheEntry entry =
        cacheLocked(application.componentName, Provider.of(null),
  ...
  getTitleAndIcon(final ItemInfoWithIcon info,
  ...
  public synchronized void getTitleAndIcon(final ItemInfoWithIcon info,
    ...
    // then we should still look in the cache for restored app icons.
    if (info.getTargetComponent() == null) {
      ...
      getTitleAndIcon(info,
                      new ActivityInfoProvider(info.getIntent(), info.user),
                      true, useLowResIcon);
    }
  }
  ...
  getTitleAndIcon(final @NonNull ItemInfoWithIcon infoInOut,
                  final
                  @NonNull Provider<LauncherActivityInfo> activityInfoProvider,
                  final boolean usePkgIcon, final boolean useLowResIcon) {
    CacheEntry entry =
        cacheLocked(infoInOut.getTargetComponent(), activityInfoProvider,
   ...
   */
  public synchronized void
  getTitleAndIconForApp(final PackageItemInfo infoInOut,
                        final boolean useLowResIcon) {
    CacheEntry entry = getEntryForPackageLocked(infoInOut.packageName,
                                                infoInOut.user, useLowResIcon);
    applyCacheEntry(entry, infoInOut);
  }

  private void applyCacheEntry(final CacheEntry entry,
                               final ItemInfoWithIcon info) {
  ...
  /**
   * Retrieves the entry from the cache. If the entry is not present, it creates
   * a new entry. This method is not thread safe, it must be called from a
   * synchronized method.
   */
  protected CacheEntry
  cacheLocked(final @NonNull ComponentName componentName,
              final @NonNull Provider<LauncherActivityInfo> infoProvider,
              final UserHandle user, final boolean usePackageIcon,
              final boolean useLowResIcon) {
    Preconditions.assertWorkerThread();
    ...
    if (entry == null || (entry.isLowResIcon && !useLowResIcon)) {
      ...
      if (!getEntryFromDB(cacheKey, entry, useLowResIcon) ||
        ...
        if (info != null) {
          ...
          li.createBadgedIconBitmap(getFullResIcon(info), info.getUser(),
                                    info.getApplicationInfo().targetSdkVersion)
              .applyTo(entry);
          li.recycle();
        } else {
          if (usePackageIcon) {
          ...
          if (entry.icon == null) {
            if (DEBUG)
              Log.d(TAG,
                    "using default icon for " + componentName.toShortString());
      ...
      if (TextUtils.isEmpty(entry.title)) {
        ...
        if (info != null) {
          ...
          entry.contentDescription =
              mUserManager.getBadgedLabelForUser(entry.title, user);
        }
      }
    }
    return entry;
  ...
  /**
   * Adds a default package entry in the cache. This entry is not persisted and
   * will be removed when the cache is flushed.
   */
  public synchronized void cachePackageInstallInfo(final String packageName,
                                                   final UserHandle user,
                                                   final Bitmap icon,
  ...
  private static ComponentKey getPackageKey(final String packageName,
                                            final UserHandle user) {
    ComponentName cn =
   ...
   * various components. This method is not thread safe, it must be called from
   * a synchronized method.
   */
  private CacheEntry getEntryForPackageLocked(final String packageName,
                                              final UserHandle user,
                                              final boolean useLowResIcon) {
    Preconditions.assertWorkerThread();
    ComponentKey cacheKey = getPackageKey(packageName, user);
    ...
    if (entry == null || (entry.isLowResIcon && !useLowResIcon)) {
      ...
      if (!getEntryFromDB(cacheKey, entry, useLowResIcon)) {
        try {
          int flags = Process.myUserHandle().equals(user)
                          ? 0
          ...
          LauncherIcons li = LauncherIcons.obtain(mContext);
          // Load the full res icon for the application, but if useLowResIcon is
          // set, then only keep the low resolution icon instead of the larger
          // full-sized icon
          ...
          ContentValues values =
              newContentValues(iconInfo.icon, lowResIcon, entry.color,
  ...
  private boolean getEntryFromDB(final ComponentKey cacheKey,
                                 final CacheEntry entry, final boolean lowRes) {
    Cursor c = null;
    try {
      c = mIconDb.query(
          new String[] {lowRes ? IconDB.COLUMN_ICON_LOW_RES
                               : IconDB.COLUMN_ICON,
                        IconDB.COLUMN_ICON_COLOR, IconDB.COLUMN_LABEL},
          IconDB.COLUMN_COMPONENT + " = ? AND " + IconDB.COLUMN_USER + " = ?",
          new String[] {cacheKey.componentName.flattenToString(),
                        Long.toString(mUserManager.getSerialNumberForUser(
                            cacheKey.user))});
      if (c.moveToNext()) {
        entry.icon =
            loadIconNoResize(c, 0, lowRes ? mLowResOptions : mHighResOptions);
        ...
        if (entry.title == null) {
          ...
          entry.contentDescription =
              mUserManager.getBadgedLabelForUser(entry.title, cacheKey.user);
        }
        return true;
      }
    } catch (SQLiteException e) {
  ...
  /**
   * A runnable that updates invalid icons and adds missing icons in the DB for
   * the provided LauncherActivityInfo list. Items are updated/added one at a
   * time, so that the worker thread doesn't get blocked.
   */
  @Thunk
  class SerializedIconUpdateTask implements Runnable {
    ...
    @Thunk
    SerializedIconUpdateTask(final long userSerial,
                             final HashMap<String, PackageInfo> pkgInfoMap,
    ...
    public void run() {
      if (!mAppsToUpdate.isEmpty()) {
        ...
        if (mAppsToUpdate.isEmpty() && !mUpdatedPackages.isEmpty()) {
          ...
          LauncherAppState.getInstance(mContext)
              ...
              .onPackageIconsUpdated(
                  mUpdatedPackages,
                  mUserManager.getUserForSerialNumber(mUserSerial));
        }
        ...
        PackageInfo info =
            mPkgInfoMap.get(app.getComponentName().getPackageName());
        // We do not check the mPkgInfoMap when generating the mAppsToAdd.
        // Although every app should have package info, this is not guaranteed
        // by the api
        if (info != null) {
          addIconToDBAndMemCache(app, info, mUserSerial,
                                 false /*replace existing*/);
  ...
  private static final class IconDB extends SQLiteCacheHelper {
    ...
    public IconDB(final Context context, final int iconPixelSize) {
      super(context, LauncherFiles.APP_ICONS_DB,
            (RELEASE_VERSION << 16) + iconPixelSize, TABLE_NAME);
    }
    ...
    protected void onCreateTable(final SQLiteDatabase db) {
      db.execSQL(
          ...
          COLUMN_VERSION + " INTEGER NOT NULL DEFAULT 0, " + COLUMN_ICON +
          " BLOB, " + COLUMN_ICON_LOW_RES + " BLOB, " + COLUMN_ICON_COLOR +
          " INTEGER NOT NULL DEFAULT 0, " + COLUMN_LABEL + " TEXT, " +
  ...
  private ContentValues newContentValues(final Bitmap icon,
                                         final Bitmap lowResIcon,
                                         final int iconColor,
                                         final String label,
    ...
    values.put(IconDB.COLUMN_SYSTEM_STATE,
               mIconProvider.getIconSystemState(packageName));
  ...
  private Bitmap generateLowResIcon(final Bitmap icon) {
    return Bitmap.createScaledBitmap(
        icon, icon.getWidth() / LOW_RES_SCALE_FACTOR,
        icon.getHeight() / LOW_RES_SCALE_FACTOR, true);
  }

  private static Bitmap loadIconNoResize(final Cursor c, final int iconIndex,
