...
// Constants that affects the interpolation curve between statically defined
// device profile buckets.
private static float KNEARESTNEIGHBOR = 3;
private static float WEIGHT_POWER = 5;

// used to offset float not being able to express extremely small weights in
// extreme cases.
...
private InvariantDeviceProfile(final InvariantDeviceProfile p) {
	this(p.name, p.minWidthDps, p.minHeightDps, p.numRows, p.numColumns,
	     p.numFolderRows, p.numFolderColumns, p.iconSize, p.landscapeIconSize,
	     p.iconTextSize, p.numHotseatIcons, p.defaultLayoutId,
	     p.demoModeLayoutId);
}

private InvariantDeviceProfile(final String n, final float w, final float h,
                               final int r, final int c, final int fr,
                               final int fc, final float is, final float lis,
                               final float its, final int hs, final int dlId,
                               final int dmlId) {
	name = n;
	minWidthDps = w;
	minHeightDps = h;
...
public InvariantDeviceProfile(final Context context) {
	...
	minWidthDps =
		Utilities.dpiFromPx(Math.min(smallestSize.x, smallestSize.y), dm);
	minHeightDps =
		Utilities.dpiFromPx(Math.min(largestSize.x, largestSize.y), dm);
	...
	landscapeHotseatIconSizeOriginal =
		interpolatedDeviceProfileOut.landscapeIconSize;
	...
	landscapeAllAppsIconSizeOriginal =
		interpolatedDeviceProfileOut.landscapeIconSize;
	...
	new IconScale(Utilities.getZimPrefs(context), "hotseatIconSize", "iconSize",
	              this);

	// Initialize these *after* the icon scale has been applied, this ensures we
	// load icons of proper resolution
	iconBitmapSize = Utilities.pxFromDp(
		max(max(iconSize, allAppsIconSize), hotseatIconSize), dm);
	...
	landscapeProfile = new DeviceProfile(
		context, this, smallestSize, largestSize, largeSide, smallSide,
		true /* isLandscape */, false /* isMultiWindowMode */);
	portraitProfile = new DeviceProfile(
		context, this, smallestSize, largestSize, smallSide, largeSide,
		false /* isLandscape */, false /* isMultiWindowMode */);
	...
	if (context.getResources().getConfiguration().smallestScreenWidthDp >=
		...
		defaultWallpaperSize =
			new Point(max(smallSide * 2, largeSide), largeSide);
	}
}
...
getPredefinedDeviceProfiles(final Context context) {
	ArrayList<InvariantDeviceProfile> profiles = new ArrayList<>();
	try (XmlResourceParser parser =
		     context.getResources().getXml(R.xml.device_profiles)) {
		...
		while (((type = parser.next()) != XmlPullParser.END_TAG ||
			...
			if ((type == XmlPullParser.START_TAG) &&
				...
				float iconSize =
					a.getFloat(R.styleable.InvariantDeviceProfile_zIconSize, 0);
				profiles.add(new InvariantDeviceProfile(
						     a.getString(R.styleable.InvariantDeviceProfile_name),
						     a.getFloat(R.styleable.InvariantDeviceProfile_minWidthDps, 0),
						     a.getFloat(R.styleable.InvariantDeviceProfile_minHeightDps, 0),
						     numRows, numColumns,
						     a.getInt(R.styleable.InvariantDeviceProfile_numFolderRows,
						              numRows),
						     a.getInt(R.styleable.InvariantDeviceProfile_numFolderColumns,
						              numColumns),
						     iconSize,
						     a.getFloat(R.styleable.InvariantDeviceProfile_landscapeIconSize,
						                iconSize),
						     a.getFloat(R.styleable.InvariantDeviceProfile_iconTextSize, 0),
						     a.getInt(R.styleable.InvariantDeviceProfile_numHotseatIcons,
						              numColumns),
						     a.getResourceId(
							     R.styleable.InvariantDeviceProfile_defaultLayoutId, 0),
						     a.getResourceId(
							     R.styleable.InvariantDeviceProfile_demoModeLayoutId, 0)));
				a.recycle();
			}
		}
...
private int getLauncherIconDensity(final int requiredSize) {
	...
	int[] densityBuckets =
		new int[] {DisplayMetrics.DENSITY_LOW,    DisplayMetrics.DENSITY_MEDIUM,
 ...
 */
private void applyPartnerDeviceProfileOverrides(final Context context,
...
/**
 * Returns the closest device profiles ordered by closeness to the specified
 * width and height
 */
// Package private visibility for testing.
ArrayList<InvariantDeviceProfile>
findClosestDeviceProfiles(final float width, final float height,
...
invDistWeightedInterpolate(final float width, final float height,
                           final ArrayList<InvariantDeviceProfile> points) {
	float weights = 0;
	...
	for (int i = 0; i < points.size() && i < KNEARESTNEIGHBOR; ++i) {
		...
		float w =
			weight(width, height, p.minWidthDps, p.minHeightDps, WEIGHT_POWER);
		weights += w;
...
public int getAllAppsButtonRank() {
	if (FeatureFlags.IS_DOGFOOD_BUILD && FeatureFlags.NO_ALL_APPS_ICON) {
		throw new IllegalAccessError(
			      "Accessing all apps rank when all-apps is disabled");
	}
...
private float weight(final float x0, final float y0, final float x1,
                     final float y1, final float pow) {
...
/**
 * As a ratio of screen height, the total distance we want the parallax effect
 * to span horizontally
 */
private static float wallpaperTravelToScreenWidthRatio(final int width,
                                                       final int height) {
	float aspectRatio = width / (float)height;

	// At an aspect ratio of 16/10, the wallpaper parallax effect should
	// span 1.5 * screen width At an aspect ratio of 10/16, the wallpaper
	// parallax effect should span 1.2 * screen width We will use these two data
	// points to extrapolate how much the wallpaper parallax effect to span (ie
	// travel) at any aspect ratio:
	...
	// following two formulas, where the coefficient on x is the aspect ratio
	// (width/height):
	//   (16/10)x + y = 1.5
	//   (10/16)x + y = 1.2
	// We solve for x and y and end up with a final formula:
	final float x = (WALLPAPER_WIDTH_TO_SCREEN_RATIO_LANDSCAPE -
	                 WALLPAPER_WIDTH_TO_SCREEN_RATIO_PORTRAIT) /
	                (ASPECT_RATIO_LANDSCAPE - ASPECT_RATIO_PORTRAIT);
	final float y =
		WALLPAPER_WIDTH_TO_SCREEN_RATIO_PORTRAIT - x * ASPECT_RATIO_PORTRAIT;
