...
public abstract class AbstractFloatingView
    extends LinearLayout implements TouchController {

  @IntDef(flag = true,
          value = {TYPE_FOLDER, TYPE_ACTION_POPUP, TYPE_WIDGETS_BOTTOM_SHEET,
                   TYPE_WIDGET_RESIZE_FRAME, TYPE_WIDGETS_FULL_SHEET,
                   TYPE_ON_BOARD_POPUP, TYPE_DISCOVERY_BOUNCE,

                   TYPE_QUICKSTEP_PREVIEW, TYPE_TASK_MENU, TYPE_OPTIONS_POPUP,
  ...
  public static final int TYPE_ALL =
      ...
      TYPE_TASK_MENU | TYPE_OPTIONS_POPUP | TYPE_SETTINGS_SHEET;

  // Type of popups which should be kept open during launcher rebind
  public static final int TYPE_REBIND_SAFE =
      TYPE_WIDGETS_FULL_SHEET | TYPE_QUICKSTEP_PREVIEW | TYPE_ON_BOARD_POPUP |
      TYPE_DISCOVERY_BOUNCE;

  // Usually we show the back button when a floating view is open. Instead, hide
  // for these types.
  public static final int TYPE_HIDE_BACK_BUTTON =
      TYPE_ON_BOARD_POPUP | TYPE_DISCOVERY_BOUNCE;
  ...
  public AbstractFloatingView(final Context context, final AttributeSet attrs) {
    super(context, attrs);
  }
  ...
  public final void close(final boolean animate) {
    ...
    if (mIsOpen) {
      BaseActivity.fromContext(getContext())
  ...
  /**
   * @return Whether the back is consumed. If false, Launcher will handle the
   *     back as well.
   */
  ...
  protected void announceAccessibilityChanges() {
    ...
    sendCustomAccessibilityEvent(targetInfo.first, TYPE_WINDOW_STATE_CHANGED,
                                 targetInfo.second);

    if (mIsOpen) {
      sendAccessibilityEvent(TYPE_VIEW_FOCUSED);
    }
    BaseDraggingActivity.fromContext(getContext())
        .getDragLayer()
        .sendAccessibilityEvent(TYPE_WINDOW_CONTENT_CHANGED);
  }

  protected Pair<View, String> getAccessibilityTarget() { return null; }

  protected static <T extends AbstractFloatingView>
      T getOpenView(final BaseDraggingActivity activity,
                    final @FloatingViewType int type) {
    BaseDragLayer dragLayer = activity.getDragLayer();
    // Iterate in reverse order. AbstractFloatingView is added later to the
    // dragLayer, and will be one of the last views.
  ...
  public static void closeOpenContainer(final BaseDraggingActivity activity,
                                        final @FloatingViewType int type) {
  ...
  public static void closeOpenViews(final BaseDraggingActivity activity,
  ...
  public static void closeAllOpenViews(final BaseDraggingActivity activity,
                                       final boolean animate) {
  ...
  public static AbstractFloatingView
  getTopOpenView(final BaseDraggingActivity activity) {
    return getTopOpenViewWithType(activity, TYPE_ALL);
  }

  public static AbstractFloatingView
  getTopOpenViewWithType(final BaseDraggingActivity activity,
                         final @FloatingViewType int type) {
