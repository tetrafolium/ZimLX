 ...
 */
public class AllAppsGridAdapter
    extends RecyclerView.Adapter<AllAppsGridAdapter.ViewHolder> {
  ...
  // We use various dividers for various purposes.  They share enough attributes
  // to reuse layouts, but differ in enough attributes to require different view
  // types
  ...
  public static final int VIEW_TYPE_MASK_ICON =
      VIEW_TYPE_ICON | VIEW_TYPE_FOLDER;

  public interface BindViewCallback { void onBindView(ViewHolder holder); }
  ...
  public static class ViewHolder extends RecyclerView.ViewHolder {
    ...
    public ViewHolder(final View v) { super(v); }
  }
  ...
  public class AppsGridLayoutManager extends GridLayoutManager {
    ...
    public void onInitializeAccessibilityEvent(final AccessibilityEvent event) {
      ...
      // Ensure that we only report the number apps for accessibility not
      // including other adapter views
      final AccessibilityRecordCompat record =
          AccessibilityEventCompat.asRecord(event);
      record.setItemCount(mApps.getNumFilteredApps());
      record.setFromIndex(
          Math.max(0, record.getFromIndex() -
                          getRowsNotForAccessibility(record.getFromIndex())));
      record.setToIndex(
          Math.max(0, record.getToIndex() -
                          getRowsNotForAccessibility(record.getToIndex())));
    }
    ...
    public void onInitializeAccessibilityNodeInfoForItem(
        ...
        final View host, final AccessibilityNodeInfoCompat info) {
      super.onInitializeAccessibilityNodeInfoForItem(recycler, state, host,
                                                     info);

      ViewGroup.LayoutParams lp = host.getLayoutParams();
      AccessibilityNodeInfoCompat.CollectionItemInfoCompat cic =
      ...
      info.setCollectionItemInfo(
          AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(
              cic.getRowIndex() -
                  getRowsNotForAccessibility(glp.getViewAdapterPosition()),
              cic.getRowSpan(), cic.getColumnIndex(), cic.getColumnSpan(),
              cic.isHeading(), cic.isSelected()));
    }

    /**
     * Returns the number of rows before {@param adapterPosition}, including
     * this position which should not be counted towards the collection info.
     */
    private int getRowsNotForAccessibility(final int adapterPosition) {
      List<AdapterItem> items = mApps.getAdapterItems();
      adapterPosition =
          Math.max(adapterPosition, mApps.getAdapterItems().size() - 1);
      int extraRows = 0;
  ...
  // The intent to send off to the market app, updated each time the search
  // query changes.
  private Intent mMarketSearchIntent;

  public AllAppsGridAdapter(final Launcher launcher,
  ...
  /**
   * Sets the last search query that was made, used to show when there are no
   * results and to also seed the intent for searching the market.
   */
   ...
   */
  public GridLayoutManager getLayoutManager() { return mGridLayoutMgr; }
  ...
  public ViewHolder onCreateViewHolder(final ViewGroup parent,
    ...
    case VIEW_TYPE_ICON:
      ...
      // Ensure the all apps icon height matches the workspace icons in portrait
      // mode.
    ...
    case VIEW_TYPE_EMPTY_SEARCH:
      return new ViewHolder(mLayoutInflater.inflate(
          R.layout.all_apps_empty_search, parent, false));

    case VIEW_TYPE_SEARCH_MARKET:
      View searchMarketView = mLayoutInflater.inflate(
          R.layout.all_apps_search_market, parent, false);
      searchMarketView.setOnClickListener(
          v -> mLauncher.startActivitySafely(v, mMarketSearchIntent, null));
    ...
    case VIEW_TYPE_WORK_TAB_FOOTER:
      View footer =
          mLayoutInflater.inflate(R.layout.work_tab_footer, parent, false);
    ...
    case VIEW_TYPE_SEARCH_SUGGESTION:
      return new ViewHolder(mLayoutInflater.inflate(
          R.layout.all_apps_search_suggestion, parent, false));
  ...
  public void onBindViewHolder(final ViewHolder holder, final int position) {
    ...
    case VIEW_TYPE_EMPTY_SEARCH:
      ...
      emptyViewText.setGravity(mApps.hasNoFilteredResults()
                                   ? Gravity.CENTER
    ...
    case VIEW_TYPE_ALL_APPS_DIVIDER:
      ...
      break;
    case VIEW_TYPE_WORK_TAB_FOOTER:
      WorkModeSwitch workModeToggle =
          holder.itemView.findViewById(R.id.work_mode_toggle);
      workModeToggle.refresh();
      TextView managedByLabel =
          holder.itemView.findViewById(R.id.managed_by_label);
      boolean anyProfileQuietModeEnabled =
          UserManagerCompat.getInstance(managedByLabel.getContext())
              .isAnyProfileQuietModeEnabled();
      managedByLabel.setText(anyProfileQuietModeEnabled
                                 ? R.string.work_mode_off_label
                                 : R.string.work_mode_on_label);
      break;
    case VIEW_TYPE_FOLDER:
      ViewGroup container = (ViewGroup)holder.itemView;
      FolderIcon folderIcon =
          mApps.getAdapterItems().get(position).folderItem.getFolderIcon(
              mLauncher, container);
    ...
    case VIEW_TYPE_SEARCH_SUGGESTION:
      ...
      ((ImageView)group.findViewById(android.R.id.icon))
          .getDrawable()
          .setTint(color);
  ...
  private SearchProvider getSearchProvider() {
    return SearchProviderController.Companion.getInstance(mLauncher)
        .getSearchProvider();
  }
}
