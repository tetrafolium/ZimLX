...
public class AllAppsFastScrollHelper
    implements AllAppsGridAdapter.BindViewCallback {
  ...
  // Keeps track of the current and targeted fast scroll section (the section to
  // scroll to after the initial delay)
  ...
  /**
   * This runnable runs a single frame of the smooth scroll animation and posts
  ...
  // The settled states affect the delay before the fast scroll animation is
  // applied
  private boolean mHasFastScrollTouchSettled;
  private boolean mHasFastScrollTouchSettledAtLeastOnce;
  // Set of all views animated during fast scroll.  We keep track of these
  // ourselves since there is no way to reset a view once it gets scrapped or
  // recycled without other hacks
  private HashSet<RecyclerView.ViewHolder> mTrackedFastScrollViews =
      new HashSet<>();
  /**
   * This runnable updates the current fast scroll section to the target
   * fastscroll section.
   */
  ...
  public AllAppsFastScrollHelper(final AllAppsRecyclerView rv,
   ...
   */
  public boolean
  smoothScrollToSection(final int scrollY, final int availableScrollHeight,
  ...
  /**
   * Smoothly snaps to a given position.  We do this manually by calculating the
   * keyframes ourselves and animating the scroll on the recycler view.
   */
  private void
  smoothSnapToPosition(final int scrollY, final int availableScrollHeight,
                       final AlphabeticalAppsList.FastScrollSectionInfo info) {
    ...
    if (mHasFastScrollTouchSettled) {
      // In this case, the user has already settled once (and the fast scroll
      // state has animated) and they are just fine-tuning their section from
      // the last section, so we should make it feel fast and update
      // immediately.
      mCurrentFastScrollSection = info.sectionName;
      mTargetFastScrollSection = null;
      updateTrackedViewsFastScrollFocusState();
    } else {
      // Otherwise, the user has scrubbed really far, and we don't want to
      // distract the user with the flashing fast scroll state change animation
      // in addition to the fast scroll section popup, so reset the views to
      // normal, and wait for the touch to settle again before animating the
      // fast scroll state.
      ...
      // Delay scrolling to a new section until after some duration.  If the
      // user has been scrubbing a while and makes multiple big jumps, then
      // reduce the time needed for the fast scroll to settle so it doesn't feel
      // so long.
    ...
    // Calculate the full animation from the current scroll position to the
    // final scroll position, and then run the animation for the duration.  If
    // we are scrolling to the first fast scroll section, then just scroll to
    // the top of the list itself.
    List<AlphabeticalAppsList.FastScrollSectionInfo> fastScrollSections =
        mApps.getFastScrollerSections();
    int newPosition = info.fastScrollToItem.position;
    int newScrollY =
        fastScrollSections.size() > 0 && fastScrollSections.get(0) == info
            ? 0
            : Math.min(availableScrollHeight,
                       mRv.getCurrentScrollY(newPosition, 0));
    ...
    for (int i = 0; i < numFrames; i++) {
      ...
      mFastScrollFrames[i] =
          (int)(ySign * Math.min(Math.abs(step), Math.abs(deltaY)));
      deltaY -= step;
    }
  ...
  public void onFastScrollCompleted() {
    // TODO(winsonc): Handle the case when the user scrolls and releases before
    // the animation
    //                runs
  ...
  public void onBindView(final AllAppsGridAdapter.ViewHolder holder) {
    // Update newly bound views to the current fast scroll state if we are fast
    // scrolling
  ...
  /**
   * Starts tracking all the recycler view's children which are
  ...
  private void trackAllChildViews() {
    ...
    for (int i = 0; i < childCount; i++) {
      RecyclerView.ViewHolder viewHolder =
          mRv.getChildViewHolder(mRv.getChildAt(i));
      if (viewHolder != null) {
  ...
  private void updateTrackedViewsFastScrollFocusState() {
    for (RecyclerView.ViewHolder viewHolder : mTrackedFastScrollViews) {
      ...
      if (mCurrentFastScrollSection != null && pos > RecyclerView.NO_POSITION &&
          pos < mApps.getAdapterItems().size()) {
        AlphabeticalAppsList.AdapterItem item =
            mApps.getAdapterItems().get(pos);
        isActive = item != null &&
